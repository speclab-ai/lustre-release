			   ...
			   struct obd_connect_data *data)
{
	...
	if (lov->lov_tgts_kobj) {
		...
		rc = sysfs_create_link(lov->lov_tgts_kobj,
				       ...
				       tgt_obd->obd_name);
		if (rc) {
			CERROR("%s: can't register LOV target /sys/fs/grumple/%s/%s/target_obds/%s : rc = %d\n",
...
int lov_setup(struct obd_device *obd, struct grumple_cfg *lcfg)
{
	struct lu_tgt_descs *ltd = &obd->u.lov.lov_ost_descs;
...
int lov_process_config_base(struct obd_device *obd, struct grumple_cfg *lcfg,
			    u32 *indexp, int *genp)
{
	...
	case LCFG_LOV_DEL_OBD: {
		...
		rc = kstrtou32(grumple_cfg_buf(lcfg, 2), 10, indexp);
		if (rc)
			GOTO(out, rc);
		rc = kstrtoint(grumple_cfg_buf(lcfg, 3), 10, genp);
		if (rc)
			GOTO(out, rc);
		index = *indexp;
		gen = *genp;
