			...
			struct osd_inode_id *id, int insert)
{
	...
	struct grumple_scrub *lscrub = &oscrub->os_scrub;
	int wakeup = 0;
 ...
 */
static inline void osd_scrub_oi_mark_stale(struct grumple_scrub *scrub,
					   struct osd_inconsistent_item *oii)
{
 ...
 */
bool osd_scrub_oi_resurrect(struct grumple_scrub *scrub,
...
static void osd_scrub_ois_fini(struct grumple_scrub *scrub,
...
static inline int osd_scrub_has_window(struct grumple_scrub *scrub,
...
{
	struct grumple_scrub *scrub = &dev->od_scrub.os_scrub;
...
static inline int
osd_scrub_wakeup(struct grumple_scrub *scrub, struct osd_otable_it *it)
{
	spin_lock(&scrub->os_lock);
			      ...
			      void *buf, int bufsize)
{
	...
	grumple_fid2lbx(name, tgt_fid, bufsize);
	...
	rc = grumple_index_restore(env, &dev->od_dt_dev, &liru->liru_pfid,
		 ...
		 int namelen, int flags)
{
	...
	if (rc == -ENODATA) {
		...
		if (lma->lma_compat & LMAC_IDX_BACKUP &&
			...
			rc = grumple_liru_new(&dev->od_index_restore_list, pfid,
			   ...
			   loff_t offset, __u64 ino, unsigned int d_type)
{
	 ...
	 * 	visible place, such as the /ROOT/.grumple/lost+found, then LFSCK
		  ...
		  struct dentry *dentry, filldir_t filldir)
{
	...
	spin_unlock(&scrub->os_lock);
	child = osd_lookup_one_len_unlocked(dev, dot_grumple_name, dentry,
					    strlen(dot_grumple_name));
	if (IS_ERR(child)) {
	...
	/* For grumple-2.x (x <= 3), the ".grumple" has NO FID-in-LMA,
	 * so the client will get IGIF for the ".grumple" object when
	 * the MDT restart.
	 *
	 * From the OI scrub view, when the MDT upgrade to Lustre-2.4,
	 * it does not know whether there are some old clients cached
	 * the ".grumple" IGIF during the upgrading. Two choices:
	 *
	 * 1) Generate IGIF-in-LMA and IGIF-in-OI for the ".grumple".
	 ...
	 *    that the old connected clients cannot access the ".grumple"
	 *    with the cached IGIF.
	 *
	 * Usually, it is rare case for the old connected clients
	 * to access the ".grumple" with cached IGIF. So we prefer
	 * to the solution 2).
	 */
...
static inline int
osd_otable_it_wakeup(struct grumple_scrub *scrub, struct osd_otable_it *it)
{
	spin_lock(&scrub->os_lock);
...
{
	struct osd_thread_info *info = osd_oti_get(env);
	struct grumple_ost_attrs *lma = &info->oti_ost_attrs;
	struct grumple_scrub *scrub = &dev->od_scrub.os_scrub;
	struct inode *inode;
	struct osd_inode_id id;
	__u64 seq;
