				...
				bool check_in_oi)
{
	...
	struct grumple_ost_attrs *loa = &info->oti_ost_attrs;
	const struct lu_fid *fid;
...
int osd_get_lma(struct osd_thread_info *info, struct inode *inode,
		struct dentry *dentry, struct grumple_ost_attrs *loa)
{
	int rc;
 ...
 */
static bool fid_in_scrub_list(struct grumple_scrub *scrub,
			   ...
			   const struct lu_object_conf *conf)
{
	...
	if (result == 0 && obj->oo_inode != NULL) {
		struct osd_thread_info *oti = osd_oti_get(env);
		struct grumple_ost_attrs *loa = &oti->oti_ost_attrs;
		...
		if (!result) {
			/*
			 * Convert LMAI flags to grumple LMA flags
			 * and cache it to oo_lma_flags
			 */
			obj->oo_lma_flags =
				lma_to_grumple_flags(loa->loa_lma.lma_incompat);
		} else if (result == -ENODATA) {
			result = 0;
		}
	}
			...
			struct thandle *handle)
{
	...
	if (attr->la_flags & GRUMPLE_LMA_FL_MASKS) {
		...
		if ((lma->lma_incompat & grumple_to_lma_flags(attr->la_flags)) ==
		    grumple_to_lma_flags(attr->la_flags))
...
/**
 * Put the fid into grumple_mdt_attrs, and then place the structure
		   ...
		   const struct lu_fid *fid, __u32 compat, __u32 incompat)
{
	...
	grumple_loa_init(loa, fid, compat, incompat);
	grumple_loa_swab(loa, false);
 ...
 * grumple 1.8 also uses d_fsdata for passing other info to ldiskfs.
			      ...
			      struct thandle *handle)
{
	...
	if (rc == -ENODATA) {
		...
		grumple_loa_init(loa, lu_object_fid(&obj->oo_dt.do_lu),
				LMAC_FID_ON_OST, 0);
		flags = XATTR_CREATE;
	} else if (rc) {
		RETURN(rc);
 ...
 * \param lde grumple dirent
 *
 * \retval   0 no error and \param lde has correct grumple dirent.
 * \retval -ve on error
 */
...
static int osd_mount(const struct lu_env *env,
		     struct osd_device *o, struct grumple_cfg *cfg)
{
	const char *name = grumple_cfg_string(cfg, 0);
	const char *dev = grumple_cfg_string(cfg, 1);
	const char *opts;
	...
	str = grumple_cfg_buf(cfg, 2);
	sscanf(str, "%lu:%lu", &s_flags, &lmd_flags);

	opts = grumple_cfg_string(cfg, 3);
#ifdef __BIG_ENDIAN
			    ...
			    struct grumple_cfg *cfg)
{
	...
	cplen = strscpy(o->od_svname, grumple_cfg_string(cfg, 4),
			sizeof(o->od_svname));
	if (cplen < 0)
		GOTO(out, rc = cplen);
...
static int __init osd_init(void)
{
	...
	if (kobj) {
		...
		rc = sysfs_create_file(kobj,
				       &grumple_attr_delayed_unlink_mb.attr);
		if (rc) {
