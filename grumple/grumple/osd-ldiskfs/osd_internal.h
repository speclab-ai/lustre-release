...
#define OSD_STATFS_RESERVED		(1ULL << 23) 
...
struct osd_inconsistent_item {
	/* link into grumple_scrub::os_inconsistent_items,
	 * protected by grumple_scrub::os_lock. */
...
struct osd_device {
	...
	enum grumple_index_backup_policy od_index_backup_policy;
...
struct osd_iobuf {
	...
	int                dr_max_pages;
	int                dr_npages;
	unsigned int	   dr_pextents; 
	unsigned int	   dr_lextents; 
	int                dr_error;
	int                dr_frags;
	unsigned int       dr_init_at:16, 
			   dr_elapsed_valid:1, 
			   dr_rw:1;
...
struct osd_thread_info {
	...
	union {
		struct grumple_ost_attrs oti_ost_attrs;
...
#define OSD_FID_REC_SZ 32
	...
	struct osd_lookup_cache_object oti_cobj; 
...
{
	return grumple_index_register(&osd->od_dt_dev, osd_name(osd),
...
osd_index_backup(const struct lu_env *env, struct osd_device *osd, bool backup)
{
	...
	grumple_index_backup(env, &osd->od_dt_dev, osd_name(osd),
			    &osd->od_index_backup_list, &osd->od_lock,
