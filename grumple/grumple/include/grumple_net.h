 ...
 *
 * PortalRPC is the layer used by rest of grumple code to achieve network
 ...
 * grumple_msg + ldlm_request + mdt_body + mds_rec_create + FNAME_MAX + PATH_MAX
 * MDS_MAXREPSIZE ~= 8300 bytes = grumple_msg + llog_header
 *
 ...
 * grumple_msg + ptlrpc_body + OUT_UPDATE_BUFFER_SIZE_MAX
 */
...
/**
 * SEQ_MAXREQSIZE == grumple_msg + __u32 padding + ptlrpc_body + opc + lu_range +
 * __u32 padding
 */
...
/**
 * OST_IO_MAXREQSIZE ~=
 *	grumple_msg + ptlrpc_body + obdo + obd_ioobj +
 ...
 */
#define _OST_MAXREQSIZE_BASE ((unsigned long)(sizeof(struct grumple_msg)   + \
			      sizeof(__u32) * 4		  + \
...
#define PTL_RPC_FL_INTR		BIT(0)	
#define PTL_RPC_FL_TIMEOUT	BIT(7)	
 ...
 */
struct ptlrpc_cb_id {
	void (*cbid_fn)(struct lnet_event *ev);	
	void *cbid_arg;				
};
...
struct ptlrpc_reply_state {
	...
	unsigned long		rs_difficult:1; 
	unsigned long		rs_no_ack:1; 
	unsigned long		rs_scheduled:1; 
	unsigned long		rs_scheduled_ever:1;
	unsigned long		rs_handled:1; 
	unsigned long		rs_sent:1; 
	unsigned long		rs_unlinked:1; 
	unsigned long		rs_prealloc:1; 
	 ...
	 */
	struct grumple_msg	*rs_msg;	

	
	struct grumple_handle	rs_locks[RS_MAX_LOCKS];
};
...
struct ptlrpc_cli_req {
	...
	int				 cr_imp_gen;
	enum grumple_imp_state		 cr_send_state;
...
struct ptlrpc_srv_req {
	...
	enum grumple_sec_part		 sr_sp_from;
...
struct ptlrpc_request {
	...
	struct grumple_msg		*rq_reqbuf;  
	char				*rq_repbuf;  
	struct grumple_msg		*rq_repdata; 
	
	struct grumple_msg		*rq_clrbuf;
	int                      rq_reqbuf_len;  
	int                      rq_reqdata_len; 
	int                      rq_repbuf_len;  
	int                      rq_repdata_len; 
...
#define DEBUG_REQ_FLAGS(req)                                                   \
	...
	FLAG(req->rq_timedout, "X") , FLAG(req->rq_resend, "S"),  \
 ...
 * content into grumple debug log.
 ...
 * In grumple we use bulk transfers for READ and WRITE transfers from/to OSTs.
...
struct ptlrpc_bulk_desc {
	...
	unsigned int		bd_iop_len;	

	__u64			bd_last_mbits;

	struct ptlrpc_cb_id	bd_cbid;	
	struct lnet_nid		bd_sender;	
	int			bd_md_count;	
	int			bd_md_max_brw;	
 ...
 * @{
 */
void ptlrpc_save_lock(struct ptlrpc_request *req, struct grumple_handle *lock,
		      bool no_ack);
...
int grumple_pack_reply_flags(struct ptlrpc_request *, int count, __u32 *lens,
			    char **bufs, int flags);
...
char *grumple_msg_string(struct grumple_msg *m, __u32 n, __u32 max_len);
...
void grumple_msg_set_handle(struct grumple_msg *msg,
...
void grumple_msg_set_last_committed(struct grumple_msg *msg,
...
static inline void
grumple_shrink_reply(struct ptlrpc_request *req, int segment,
		    unsigned int newlen, int move_data)
{
	LASSERT(req->rq_reply_state);
	LASSERT(req->rq_repmsg);
	req->rq_replen = grumple_shrink_msg(req->rq_repmsg, segment,
					   newlen, move_data);
}
...
static inline void ptlrpc_req_drop_rs(struct ptlrpc_request *req)
{
	...
	kref_put(&req->rq_reply_state->rs_refcount, grumple_free_reply_state);
	req->rq_reply_state = NULL;
}

static inline __u32 grumple_request_magic(struct ptlrpc_request *req)
{
	return grumple_msg_get_magic(req->rq_reqmsg);
}
 ...
 * @{
 */
int client_obd_setup(struct obd_device *obd, struct grumple_cfg *lcfg);
