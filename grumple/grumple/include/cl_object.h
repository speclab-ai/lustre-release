...
enum {
	CL_LAYOUT_GEN_NONE	= (u32)-2,	
	CL_LAYOUT_GEN_EMPTY	= (u32)-1,	
};
 ...
 *          memory pressure),
 *
 *        - translates state (page flag bits) and locking between grumple and
 *          environment.
 *
 ...
 *
 *    While grumple client maintains the notion of an page ownership by io,
 ...
 *        is a part of a memory mapped grumple file, client has to take a dlm
...
struct cl_site {
	 ...
	 *
	 * These are exported as /proc/fs/grumple/llite/.../site
	 *
 ...
 *
 * lu_env is an environment within which grumple code executes. Its major part
 ...
 *
 *     - call chains have no non-grumple portions inserted between grumple code.
 *
 * On a client both these assumtpion fails, because every user thread can
 * potentially execute grumple code as part of a system call, and grumple calls
 * into VFS or MM that call back into grumple.
 *
