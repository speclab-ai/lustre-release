...
#define grumple_fid struct lu_fid
 ...
 * llapi_libgrumpleapi_initialized() which is now available.
 */
extern bool libgrumpleapi_initialized;
...
static inline bool llapi_libgrumpleapi_initialized(void)
{
	return libgrumpleapi_initialized;
}
...
struct find_param {
	...
	unsigned long long	 fp_zero_end:1,
				 ...
				 fp_get_lmv:1,	
				 fp_raw:1,	
				 fp_check_stripe_size:1, 
				 fp_exclude_stripe_size:1,
				 fp_check_stripe_count:1, 
				 fp_exclude_stripe_count:1,
				 fp_check_layout:1,
				 fp_exclude_layout:1,
				 fp_get_default_lmv:1, 
				 fp_migrate:1,
				 fp_check_projid:1,
				 fp_exclude_projid:1,
...
struct find_work_queue {
	struct find_work_unit *fwq_head; 
	struct find_work_unit *fwq_tail; 
...
int llapi_open_by_fid_at(int grumple_fd, const struct lu_fid *fid,
			 int open_flags);
int llapi_open_by_fid(const char *grumple_dir, const struct lu_fid *fid,
		      int open_flags);
 ...
 * converted to extended format in the grumple api to ease changelog analysis.
 */
 ...
 */
#define LLAPI_OVERSTRIPE_COUNT_MIN    ((__s16)LOV_ALL_STRIPES)      
#define LLAPI_OVERSTRIPE_COUNT_MAX    ((__s16)LOV_ALL_STRIPES_WIDE) 
...
#define LLAPI_LAYOUT_WIDE     LLAPI_LAYOUT_WIDE_MIN 
 ...
 * Update if more attributes are added in grumple_user.h.
 */
