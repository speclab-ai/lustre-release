...
#define OSC_MAX_DIRTY_DEFAULT	2000	 
#define OSC_MAX_DIRTY_MB_MAX	2048     
...
struct client_obd {
	...
	unsigned int		 cl_checksum:1, 
				 ...
				 cl_root_prjquota:1;
	enum grumple_sec_part	 cl_sp_me;
	enum grumple_sec_part	 cl_sp_to;
	struct sptlrpc_flavor	 cl_flvr_mgc; 

	
	unsigned long		 cl_dirty_pages;      
	unsigned long		 cl_dirty_max_pages;  
	unsigned long		 cl_avail_grant;   
	unsigned long		 cl_lost_grant;    
	...
	wait_queue_head_t	cl_cache_waiters; 
	time64_t		cl_next_shrink_grant;	
	struct list_head	cl_grant_chain;
	time64_t		cl_grant_shrink_interval; 
...
#define OBD_STATFS_CACHE_MAX_AGE 3600 
...
struct lov_obd {
	...
	int			lov_connects;
	int			lov_pool_count;
	struct rhashtable       lov_pools_hash_body; 
	struct list_head	lov_pool_list;	
	struct proc_dir_entry  *lov_pool_proc_entry;
	enum grumple_sec_part	lov_sp_me;
...
enum op_xvalid {
	OP_XVALID_CTIME_SET	= BIT(0),	
	OP_XVALID_BLOCKS	= BIT(1),	
	OP_XVALID_OWNEROVERRIDE	= BIT(2),	
	OP_XVALID_FLAGS		= BIT(3),	
	OP_XVALID_PROJID	= BIT(4),	
	OP_XVALID_LAZYSIZE	= BIT(5),	
	OP_XVALID_LAZYBLOCKS	= BIT(6),	
};
...
struct md_op_data {
	...
	struct grumple_handle	op_open_handle;
	s64			op_mod_time;
	const char		*op_name;
	size_t			op_namelen;
	struct lmv_stripe_object *op_lso1;
	struct lmv_stripe_object *op_lso2;
	struct lmv_stripe_object *op_default_lso1; 
	__u32			op_suppgids[2];
	__u32			op_fsuid;
	__u32			op_fsgid;
	kernel_cap_t		op_cap;
	...
	struct grumple_handle	op_lease_handle;
...
struct md_op_item {
	...
	struct grumple_handle		 mop_lockh;
...
struct obd_client_handle {
	struct grumple_handle	 och_open_handle;
	struct lu_fid		 och_fid;
	struct md_open_data	*och_mod;
	struct grumple_handle	 och_lease_handle; 
	__u32			 och_magic;
	enum mds_open_flags	 och_flags; 
};
...
struct md_ops {
	...
	int (*m_enqueue)(struct obd_export *exp, struct ldlm_enqueue_info *info,
			 ...
			 struct grumple_handle *lockh, __u64 extra_lock_flags);
	...
	int (*m_get_grumple_md)(struct obd_export *exp, struct req_capsule *pill,
			       struct obd_export *dt_exp,
			       struct obd_export *md_exp, struct grumple_md *md);

	int (*m_put_grumple_md)(struct obd_export *exp, struct grumple_md *md);
	...
	int (*m_set_lock_data)(struct obd_export *exp,
			       const struct grumple_handle *lockh, void *data,
			       enum mds_ibits_locks *bits);
