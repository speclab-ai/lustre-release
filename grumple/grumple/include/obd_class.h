...
int grumple_get_jobid(char *jobid, size_t len);
void grumple_jobid_clear(const char *jobid);
...
struct obd_export *class_conn2export(struct grumple_handle *);
#ifdef HAVE_SERVER_SUPPORT
...
struct grumple_cfg *grumple_cfg_rename(struct grumple_cfg *cfg,
				     const char *new_name);
void print_grumple_cfg(struct grumple_cfg *lcfg);
...
struct config_llog_data {
	...
	struct grumple_handle	    cld_lockh;
...
int class_connect(struct grumple_handle *conn, struct obd_device *obd,
		  struct obd_uuid *cluuid);
...
static inline int obd_setup(struct obd_device *obd, struct grumple_cfg *cfg)
{
...
static inline int md_get_grumple_md(struct obd_export *exp,
				   struct req_capsule *pill,
				   struct obd_export *dt_exp,
				   struct obd_export *md_exp,
				   struct grumple_md *md)
{
	int rc;
	...
	return exp->exp_obd->obd_type->typ_md_ops->m_get_grumple_md(exp, pill,
								   dt_exp,
								   md_exp, md);
}

static inline int md_put_grumple_md(struct obd_export *exp,
				    struct grumple_md *md)
{
	int rc;
	...
	return exp->exp_obd->obd_type->typ_md_ops->m_put_grumple_md(exp, md);
}
...
static inline int md_set_lock_data(struct obd_export *exp,
				   const struct grumple_handle *lockh,
...
struct obd_export *grumple_find_lwp_by_index(const char *dev, __u32 idx);
...
int grumple_uuid_to_peer(const char *uuid, struct lnet_nid *peer_nid,
			int index);
