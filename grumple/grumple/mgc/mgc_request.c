		      ...
		      enum mgs_cfg_type type)
{
	...
	/* logname consists of "fsname-nodetype". eg. "grumple-MDT0001",
				  ...
				  void *data, int datalen, bool mne_swab)
{
	...
	while (datalen > 0) {
		...
		if (mne_swab)
			grumple_swab_mgs_nidtbl_entry_header(entry);
		...
		if (entry->mne_nid_type == 0) {
			 ...
			 */
			if (mne_swab)
				grumple_swab_mgs_nidtbl_entry_content(entry);
		 ...
		 * grumple/<osc|mdc>/<target>-<osc|mdc>-<instance>/import.
		 */
		...
		cname = is_ost ? "osc" : "mdc",
			pos += snprintf(obdname + pos, bufsz, "-%s-%s", cname,
					inst);
		grumple_cfg_bufs_reset(&bufs, obdname);
		...
		grumple_cfg_bufs_set_string(&bufs, 1, params);
...
{
	struct llog_ctxt *ctxt;
	struct grumple_sb_info *lsi = NULL;
	struct lu_env *env;
...
restart:
	...
	if (rcl == 0) {
		...
		LASSERT(!grumple_handle_is_used(&cld->cld_lockh));
		LASSERT(rc == 0);
		cld->cld_lockh = lockh;
	} else {
...
/* Called from grumple_process_log.
			      ...
			      struct grumple_cfg *lcfg)
{
	...
	case LCFG_LOG_START: {
		...
		logname = grumple_cfg_string(lcfg, 1);
		cfg = (struct config_llog_instance *)grumple_cfg_buf(lcfg, 2);
		sb = *(struct super_block **)grumple_cfg_buf(lcfg, 3);
