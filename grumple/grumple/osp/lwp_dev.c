		     ...
		     char *nidstring)
{
	...
	grumple_cfg_bufs_reset(bufs, lwp_name);
	grumple_cfg_bufs_set_string(bufs, 1, server_uuid);
	grumple_cfg_bufs_set_string(bufs, 2, nidstring);
 ...
 * @lcfg: grumple_cfg, LCFG_PRE_CLEANUP or LCFG_CLEANUP
 *
...
static int lwp_process_config(const struct lu_env *env,
			      struct lu_device *dev, struct grumple_cfg *lcfg)
{
	...
	default:
		CERROR("%s: unknown command %u\n",
		       (char *)grumple_cfg_string(lcfg, 0), lcfg->lcfg_command);
		rc = 0;
		break;
	}
 ...
 * @ldt: not used
 * @cfg: grumple_cfg contains remote target uuid
 *
 * Return:
 * * %0 on success
		     ...
		     struct lu_device_type *ldt, struct grumple_cfg *cfg)
{
	...
	lwp->lpd_obd = class_name2obd(grumple_cfg_string(cfg, 0));
	if (lwp->lpd_obd == NULL) {
	...
	rc = lwp_setup(env, lwp, grumple_cfg_string(cfg, 1));
	if (rc) {
