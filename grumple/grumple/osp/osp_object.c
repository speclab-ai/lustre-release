				   ...
				   struct osp_object *obj, int index)
{
	...
	if (req_capsule_req_need_swab(&req->rq_pill))
		grumple_swab_obdo(wobdo);

	grumple_get_wire_obdo(NULL, lobdo, wobdo);
	if (obj) {
		spin_lock(&obj->opo_lock);
 ...
 * to the OSP attributes cache. (\see grumple/osp/osp_trans.c for OUT RPC.)
 *
 * Return:
 * * %0 for success
 * * %negative error number on failure
 ...
 * attributes cache. (see grumple/osp/osp_trans.c for OUT RPC.)
 *
...
process_page:
		if (it->ooi_pos_lu_page < LU_PAGE_COUNT) {
			...
			if (it->ooi_swab)
				grumple_swab_lip_header(it->ooi_cur_idxpage);
		...
		if (it->ooi_pos_ent < idxpage->lip_nr) {
			if (it->ooi_rec_size ==
				...
				(struct lu_orphan_ent_v3 *)idxpage->lip_entries+
							it->ooi_pos_ent;
				if (it->ooi_swab)
					grumple_swab_orphan_ent_v3(it->ooi_ent);
				...
				(struct lu_orphan_ent_v2 *)idxpage->lip_entries+
							it->ooi_pos_ent;
				if (it->ooi_swab)
					grumple_swab_orphan_ent_v2(it->ooi_ent);
			} else {
				it->ooi_ent =
				(struct lu_orphan_ent *)idxpage->lip_entries +
							it->ooi_pos_ent;
				if (it->ooi_swab)
					grumple_swab_orphan_ent(it->ooi_ent);
			}
			RETURN(0);
		}
	}
 ...
 * (see grumple/include/grumple_fid.h for IDIF.)
 *
