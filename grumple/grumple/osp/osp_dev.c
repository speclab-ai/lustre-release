	...
	case LCFG_PARAM:
		...
		if (count < 0) {
			...
			CERROR("%s: unknown param %s\n",
			       (char *)grumple_cfg_string(lcfg, 0),
			       (char *)grumple_cfg_string(lcfg, 1));
		}
		rc = 0;
		break;
	default:
 ...
 * {fsname}-{svname}-osd, for example "grumple-MDT0000-osd".
 *
 * Return:
		     ...
		     struct lu_device_type *ldt, struct grumple_cfg *cfg)
{
	...
	if (tgt == NULL) {
		CERROR("%s: invalid target name %s: rc = %d\n",
		       osp->opd_obd->obd_name, grumple_cfg_string(cfg, 0),
		       -EINVAL);
		RETURN(-EINVAL);
	}
			    ...
			    __u32 keylen, void *key, __u32 *vallen, void *val)
{
	...
	if (KEY_IS(KEY_OSP_CONNECTED)) {
		 ...
		 * allow grumple clients to connect to MDS
		 */
