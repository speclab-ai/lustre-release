		       ...
		       uint64_t oid, struct lu_fid *fid)
{
	...
	struct grumple_mdt_attrs	*lma	  =
			(struct grumple_mdt_attrs *)oti->oti_buf;
	struct lu_buf		 buf;
...
osd_zfs_consistency_check(const struct lu_env *env, struct osd_device *osd,
			  ...
			  u64 oid, bool is_dir)
{
	struct grumple_scrub *scrub = &osd->od_scrub;
	dnode_t *dn = NULL;
	uint64_t oid2;
	int once = 0;
	bool insert;
				   ...
				   uint64_t parent, dmu_tx_t *tx)
{
	...
	grumple_lma_init(lma, &zde->lzd_fid, 0, LMAI_AGENT);
	grumple_lma_swab(lma);
			     ...
			     struct osd_thandle *oh)
{
	...
	struct grumple_mdt_attrs *lma = (struct grumple_mdt_attrs *)info->oti_buf;
	struct lu_buf buf = {
		.lb_buf = lma,
