...
{
	struct grumple_scrub *scrub = &dev->od_scrub;
...
zget:
		...
		struct grumple_mdt_attrs *lma = NULL;
		int size;
...
static void osd_index_restore(const struct lu_env *env, struct osd_device *dev,
			      struct grumple_index_restore_unit *liru, void *buf,
			      int bufsize)
{
	...
	ENTRY;
	grumple_fid2lbx(buf, tgt_fid, bufsize);
	...
	rc = osd_idc_find_and_init_with_oid(env, dev, tgt_fid,
					    liru->liru_clid);
	if (!rc)
		rc = grumple_index_restore(env, &dev->od_dt_dev,
 ...
 * be scanned during the initial OI scrub, instead, only the .grumple and the
 * sub-items under .grumple will be handled.
 */
			   ...
			   enum osd_lf_flags flags)
{
	...
	ENTRY;
	rc = osd_zap_lookup(dev, parent, NULL, dot_grumple_name, 8,
			    sizeof(*zde) / 8, (void *)zde);
	if (rc == -ENOENT) {
		/* The .grumple directory is lost. That is not fatal. It can
	...
	if (rc) {
		CWARN("%s: initial OI scrub failed to find .grumple: rc = %d\n",
		      osd_name(dev), rc);
	...
	for (map = osd_dl_maps; map->olm_name; map++) {
		...
		if (rc) {
			if (rc != -ENOENT)
				CWARN("%s: initial OI scrub failed to find the entry %s under .grumple: rc = %d\n",
		    ...
		    time64_t interval, bool resetoi)
{
	...
	memset(&dev->od_scrub, 0, sizeof(struct grumple_scrub));
...
{
	struct osd_thread_info *info = osd_oti_get(env);
	struct grumple_mdt_attrs *lma = &info->oti_mdt_attrs;
	...
	grumple_lma_init(lma, &zde->lzd_fid, 0, 0);
	grumple_lma_swab(lma);
