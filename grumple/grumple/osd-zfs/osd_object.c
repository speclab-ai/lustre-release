				 ...
				 struct osd_object *obj, struct lu_attr *la)
{
	...
	lma = (struct grumple_mdt_attrs *)osd_oti_get(env)->oti_buf;
	buf.lb_buf = lma;
	...
	if (!rc) {
		...
		obj->oo_lma_flags =
			lma_to_grumple_flags(lma->lma_incompat);
	} else if (rc == -ENODATA ||
		   !(S_ISDIR(la->la_mode) &&
	...
	if (valid & LA_FLAGS) {
		...
		if (la->la_flags & LUSTRE_LMA_FL_MASKS) {
			...
			if (!rc && unlikely(size < sizeof(*lma))) {
				...
				if ((lma->lma_incompat &
				     grumple_to_lma_flags(la->la_flags)) ==
				    grumple_to_lma_flags(la->la_flags))
...
skip_add:
	...
	grumple_lma_init(lma, fid, compat, 0);
	grumple_lma_swab(lma);
