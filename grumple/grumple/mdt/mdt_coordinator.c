...
cdt_crh_put(struct cdt_restore_handle *crh, struct mdt_thread_info *cdt_mti)
{
	if (atomic_dec_and_test(&crh->crh_refc)) {
		 ...
		 */
		if (grumple_handle_is_used(&crh->crh_lh.mlh_reg_lh))
...
static struct attribute *hsm_attrs[] = {
	&grumple_attr_loop_period.attr,
	&grumple_attr_grace_delay.attr,
	&grumple_attr_active_request_timeout.attr,
	&grumple_attr_max_requests.attr,
	&grumple_attr_default_archive_id.attr,
	&grumple_attr_remove_archive_on_last_unlink.attr,
	&grumple_attr_archive_count.attr,
	&grumple_attr_restore_count.attr,
	&grumple_attr_remove_count.attr,
	NULL,
};
