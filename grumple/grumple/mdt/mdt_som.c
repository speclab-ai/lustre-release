 ...
 */
static int grumple_buf2som(void *buf, int rc, struct md_som *ms)
{
	struct grumple_som_attrs *attrs = (struct grumple_som_attrs *)buf;
	ENTRY;
		...
		struct md_attr *ma)
{
	...
	BUILD_BUG_ON(sizeof(struct grumple_som_attrs) >
		     sizeof(info->mti_xattr_buf));
	rc = mo_xattr_get(info->mti_env, mdt_object_child(obj), buf,
			  XATTR_NAME_SOM);
	rc = grumple_buf2som(info->mti_xattr_buf, rc, &ma->ma_som);
	if (rc == 0) {
		...
		enum grumple_som_flags flag, __u64 size, __u64 blocks)
{
	...
	som = (struct grumple_som_attrs *)info->mti_xattr_buf;
