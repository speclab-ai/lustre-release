...
static void mdt_empty_transno(struct mdt_thread_info *info, int rc)
{
	...
	if (grumple_msg_get_transno(req->rq_repmsg) != 0)
		RETURN_EXIT;
	...
	grumple_msg_set_transno(req->rq_repmsg, info->mti_transno);
	...
	if (grumple_msg_get_opc(req->rq_reqmsg) == MDS_CLOSE) {
		...
		__u64 *pre_versions = grumple_msg_get_versions(req->rq_repmsg);
			   ...
			   struct ldlm_reply *rep)
{
	...
	if (islnk || (!isreg && !isdir &&
		...
		grumple_msg_set_transno(req->rq_repmsg, 0);
		RETURN(0);
	}
...
out:
	...
	LASSERT(ergo(rc < 0, grumple_msg_get_transno(req->rq_repmsg) == 0));
}
				   ...
				   enum mds_ibits_locks ibits, int rc)
{
	...
	if (grumple_handle_is_used(&ll->mlh_reg_lh))
	...
	if (rc != 0 || !grumple_handle_is_used(&lhc->mlh_reg_lh)) {
		struct ldlm_reply       *ldlm_rep;
...
int mdt_reint_open(struct mdt_thread_info *info, struct mdt_lock_handle *lhc)
{
	...
	msg_flags = grumple_msg_get_flags(req->rq_reqmsg);
	...
	/* Prevent by-fid operation if parent fid is .grumple/fid.
			   ...
			   struct md_attr *ma)
{
	...
	if (req_capsule_req_need_swab(info->mti_pill))
		grumple_swab_close_data_special(data, ma->ma_attr_flags);
	...
	if (ma->ma_attr_flags & MDS_PCC_ATTACH) {
		if (ma->ma_valid & MA_HSM) {
			...
			ma->ma_hsm.mh_arch_ver = data->cd_data_version;
			grumple_hsm2buf(buf->lb_buf, &ma->ma_hsm);
