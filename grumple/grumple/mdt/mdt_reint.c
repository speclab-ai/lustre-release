...
{
	__u64 *pre_ver = grumple_msg_get_versions(req->rq_reqmsg);
			     ...
			     int idx)
{
	...
	reply_ver = grumple_msg_get_versions(req->rq_repmsg);
	if (reply_ver)
			      ...
			      int decref)
{
	...
	struct grumple_handle_array *locks = einfo->ei_cbdata;
	int i;
			     ...
			     struct mdt_lock_handle *lhc)
{
	...
	if (unlikely(ma->ma_attr.la_valid & LA_SIZE &&
		...
		if (atomic_read(&mo->mot_lease_count) > 0) { 
