			  ...
			  void *buf)
{
	...
	if (perm & CFS_SETGRP_PERM && pud->pud_ngroups) {
		...
		grumple_groups_from_list(ucred->uc_ginfo, pud->pud_groups);
		grumple_groups_sort(ucred->uc_ginfo);
	} else {
		ucred->uc_suppgids[0] = -1;
		ucred->uc_suppgids[1] = -1;
		ucred->uc_ginfo = NULL;
...
*/
	...
	if (info->mti_big_lov_used || info->mti_big_lmv_used) {
		...
		if (rc) {
			...
			grumple_msg_set_transno(pill->rc_req->rq_repmsg, 0);
		} else {
...
static int mdt_resync_unpack(struct mdt_thread_info *info)
{
	 ...
	 * in grumple_swab_mdt_rec_reint() as rr_mtime, so here it needs
	 * restoring.
	 */
