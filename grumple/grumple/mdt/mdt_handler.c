...
{
	LASSERT(!grumple_handle_is_used(&lh->mlh_reg_lh));
	LASSERT(!grumple_handle_is_used(&lh->mlh_pdo_lh));
	LASSERT(!grumple_handle_is_used(&lh->mlh_rreg_lh));
}
...
static int mdt_statfs(struct tgt_session_info *tsi)
{
	...
	if (mdt_is_sum_statfs_client(req->rq_export) &&
		grumple_packed_msg_size(req->rq_reqmsg) ==
			 ...
			 struct mdt_object *o, struct md_attr *ma)
{
	...
	if (need & MA_HSM && S_ISREG(mode)) {
		...
		rc2 = grumple_buf2hsm(info->mti_xattr_buf, rc2, &ma->ma_hsm);
		if (rc2 == 0)
			ma->ma_valid |= MA_HSM;
				...
				struct mdt_object *o, int ma_need)
{
	...
	if (S_ISDIR(lu_object_attr(&next->mo_lu)) &&
	    ...
	    grumple_msg_get_opc(req->rq_reqmsg) == MDS_GETATTR) {
...
static int mdt_getattr(struct tgt_session_info *tsi)
{
	 ...
	 */
		   ...
		   (reqbody->mbo_valid & OBD_MD_FLDIREA) &&
		   (grumple_msg_get_opc(mdt_info_req(info)->rq_reqmsg) ==
								 MDS_GETATTR)) {
...
static int mdt_swap_layouts(struct tgt_session_info *tsi)
{
	...
	if (mdt_object_remote(o) || !mdt_object_exists(o)) 
		GOTO(put, rc = -ENOENT);
				 ...
				 struct ldlm_reply *ldlm_rep)
{
	...
	is_resent = grumple_handle_is_used(&lhc->mlh_reg_lh);
	LASSERT(ergo(is_resent,
		     grumple_msg_get_flags(req->rq_reqmsg) & MSG_RESENT));
	...
	if (rc < 0) {
		...
		if ((!(child_bits & MDS_INODELOCK_UPDATE) &&
			 ...
			 * Without this fix, it failed grumple-rsync-test/test_6.
			 */
...
static int mdt_getattr_name(struct tgt_session_info *tsi)
{
	...
	if (grumple_handle_is_used(&lhc->mlh_reg_lh)) {
			      ...
			      __u32 op)
{
	...
	if (rc < 0) {
		...
		rc = grumple_msg_get_status(mdt_info_req(info)->rq_repmsg);
		GOTO(out_ucred, rc);
	}
			  ...
			  struct mdt_lock_handle *lhc)
{
	...
	if (unlikely(grumple_handle_is_used(&lhc->mlh_reg_lh))) {
		struct ptlrpc_request *req = mdt_info_req(info);
		struct ldlm_lock      *lock;

		lock = ldlm_handle2lock(&lhc->mlh_reg_lh);
		LASSERT(grumple_msg_get_flags(req->rq_reqmsg) & MSG_RESENT);
		if (lock == NULL) {
			/* Lock is pinned by ldlm_handle_enqueue0() as it is
			 * a resend case, however, it could be already destroyed
			 * due to client eviction or a raced cancel RPC.
			     ...
			     bool cache)
{
	...
	if (obj && mdt_object_remote(obj)) {
		...
		LASSERT(!grumple_handle_is_used(handle));
 ...
 */
static void mdt_save_lock(struct mdt_thread_info *info, struct grumple_handle *h,
...
static void mdt_save_remote_lock(struct mdt_thread_info *info,
				 struct mdt_object *o, struct grumple_handle *h,
				 enum ldlm_mode mode, int decref)
{
	ENTRY;
			  ...
			  struct mdt_thread_info *info)
{
	...
	info->mti_transno = grumple_msg_get_transno(req->rq_reqmsg);
			    ...
			    __u64 flags, int result)
{
	...
	*lockp = new_lock;
	...
	if (new_lock->l_export == req->rq_export) {
		 ...
		 */
		LASSERT(grumple_msg_get_flags(req->rq_reqmsg) &
			MSG_RESENT);
			     ...
			     struct mdt_lock_handle *lh, __u64 flags)
{
	 ...
	 * normal request now.
	 */
	grumple_msg_clear_flags(req->rq_reqmsg, MSG_RESENT);
			   ...
			   __u64 flags)
{
	...
	if (rc < 0 && grumple_msg_get_flags(req->rq_reqmsg) & MSG_REPLAY)
...
static void mdt_deregister_seq_exp(struct mdt_device *mdt)
{
	...
	if (ss->ss_client_seq != NULL) {
		grumple_deregister_lwp_item(&ss->ss_client_seq->lcs_exp);
		ss->ss_client_seq->lcs_exp = NULL;
	}

	if (ss->ss_server_fld != NULL) {
		grumple_deregister_lwp_item(&ss->ss_server_fld->lsf_control_exp);
...
static int mdt_register_seq_exp(struct mdt_device *mdt)
{
	...
	rc = grumple_register_lwp_item(lwp_name, &ss->ss_client_seq->lcs_exp,
				      NULL, NULL);
	if (rc != 0)
		GOTO(out_free, rc);

	rc = grumple_register_lwp_item(lwp_name,
...
static int mdt_stack_init(const struct lu_env *env, struct mdt_device *mdt,
			  struct grumple_cfg *cfg)
{
	char		       *dev = grumple_cfg_string(cfg, 0);
	 ...
	 *   #06 (104)mount_option 0:  1:grumple-MDT0000  2:grumple-mdtlov
	 *   #08 (120)setup   0:grumple-MDT0000  1:dev 2:type 3:grumple-MDT0000
	 * 2.0 MGS generates config. commands like this:
	 *   #07 (112)mount_option 0:  1:grumple-MDT0000  2:grumple-MDT0000-mdtlov
	 *   #08 (160)setup   0:grumple-MDT0000  1:grumple-MDT0000_UUID  2:0
	 *                    3:grumple-MDT0000-mdtlov  4:f
	 *
	 * we generate MDD name from MDT one, just replacing T with D
	 *
	 * after all the preparations, the logical equivalent will be
	 *   #01 (160)setup   0:grumple-MDD0000  1:grumple-MDD0000_UUID  2:0
	 *                    3:grumple-MDT0000-mdtlov  4:f
	 *   #02 (160)setup   0:grumple-MDT0000  1:grumple-MDT0000_UUID  2:0
	 *                    3:grumple-MDD0000  4:f
	 *
	...
	lprof = class_get_profile(grumple_cfg_string(cfg, 0));
	if (lprof == NULL || lprof->lp_dt == NULL) {
		CERROR("can't find the profile: %s\n",
		       grumple_cfg_string(cfg, 0));
	...
	grumple_cfg_init(lcfg, LCFG_ATTACH, bufs);
	...
	if (!obd) {
		CERROR("Can not find obd %s (%s in config)\n",
		       MDD_OBD_NAME, grumple_cfg_string(cfg, 0));
	...
	grumple_cfg_bufs_reset(bufs, name);
	grumple_cfg_bufs_set_string(bufs, 1, uuid);
	grumple_cfg_bufs_set_string(bufs, 2, dev);
	grumple_cfg_bufs_set_string(bufs, 3, lprof->lp_dt);

	OBD_ALLOC(lcfg, grumple_cfg_len(bufs->lcfg_bufcount, bufs->lcfg_buflen));
	if (!lcfg)
		GOTO(class_detach, rc = -ENOMEM);
	grumple_cfg_init(lcfg, LCFG_SETUP, bufs);
			  ...
			  struct grumple_cfg *cfg)
{
	 ...
	 *   #01 (160)setup   0:grumple-MDT0000  1:grumple-MDT0000_UUID  2:0
	 *                    3:grumple-MDT0000-mdtlov  4:f
	 *
	 * We generate the QMT name from the MDT one, just replacing MD with QM
	 * after all the preparations, the logical equivalent will be:
	 *   #01 (160)setup   0:grumple-QMT0000  1:grumple-QMT0000_UUID  2:0
	 *                    3:grumple-MDT0000-osd  4:f
	 */
	...
	grumple_cfg_bufs_set_string(bufs, 3,
...
static int mdt_init0(const struct lu_env *env, struct mdt_device *m,
		     struct lu_device_type *ldt, struct grumple_cfg *cfg)
{
	const struct dt_device_param *dt_conf;
	struct mdt_thread_info *info;
	struct obd_device *obd;
	const char *dev = grumple_cfg_string(cfg, 0);
	const char *num = grumple_cfg_string(cfg, 2);
...
static int mdt_process_config(const struct lu_env *env,
			      struct lu_device *d, struct grumple_cfg *cfg)
{
	...
	case LCFG_PARAM: {
		...
		struct grumple_cfg *old_cfg = NULL;
		char *param = NULL;
		ssize_t count;
		...
		if (ptr != NULL) {
			...
			old_cfg = cfg;
			cfg = grumple_cfg_rename(old_cfg, ptr->new_param);
			if (IS_ERR(cfg)) {
				rc = PTR_ERR(cfg);
				break;
			}
		...
		if (old_cfg)
			OBD_FREE(cfg, grumple_cfg_len(cfg->lcfg_bufcount,
						     cfg->lcfg_buflens));
		break;
	}
	default:
