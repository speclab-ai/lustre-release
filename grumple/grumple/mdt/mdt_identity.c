				  ...
				  struct upcall_cache_entry *entry)
{
	...
	if (rc < 0) {
		CERROR("%s: error invoking upcall %s %s %s: rc %d; check /proc/fs/grumple/mdt/%s/identity_upcall, time %ldus: rc = %d\n",
				       ...
				       void *args)
{
	...
	if (data->idd_ngroups > 0) {
		...
		grumple_groups_from_list(ginfo, data->idd_groups);
		grumple_groups_sort(ginfo);
	}
