...
{
	bool is_dot_grumple = fid_is_dot_grumple(fid);
	...
	inode = search_inode_for_grumple(sb, fid);
	if (IS_ERR(inode))
	 ...
	 * handling.
	 */
	if (is_dot_grumple || fid_is_obf(fid)) {
		struct qstr dot_name = QSTR_INIT(".grumple",
						 strlen(".grumple"));
		...
		if (!dot) {
			...
			/* Successfully add .grumple dentry to dcache. For future
...
static int ll_encode_fh(struct inode *inode, u32 *fh, int *plen,
			struct inode *parent)
{
	int fileid_len = sizeof(struct grumple_file_handle) / 4;
	struct grumple_file_handle *lfh = (void *)fh;
			...
			int namelen, loff_t hash, u64 ino, unsigned int type)
{
	 ...
	 * the grumple-local version which doesn't generate the warning.
	 */
...
const struct export_operations grumple_export_operations = {
