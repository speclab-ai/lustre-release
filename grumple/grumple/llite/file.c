...
retry:
	 ...
	 * anyway.
	 */
	if (!rc && !d_grumple_invalid(de) && S_ISDIR(de->d_inode->i_mode))
	      ...
	      enum mds_open_flags open_flags)
{
	...
	struct md_op_data *op_data;
	struct ptlrpc_request *req = NULL;
	struct grumple_handle old_open_handle = { 0 };
...
static int ll_swap_layouts_close(struct obd_client_handle *och,
				 ...
				 struct grumple_swap_layouts *lsl)
{
			     ...
			     ssize_t lum_size)
{
	...
	if ((__swab32(lum->lmm_magic) & le32_to_cpu(LOV_MAGIC_MASK)) ==
		...
		grumple_swab_lov_user_md(lum, 0);
	}
			     ...
			     struct ptlrpc_request **request)
{
	...
	if (cpu_to_le32(LOV_MAGIC) != LOV_MAGIC) {
		...
		if (lmm->lmm_magic == cpu_to_le32(LOV_MAGIC_V1) ||
			...
			if (lmm->lmm_magic == LOV_MAGIC_V1 &&
				...
				grumple_swab_lov_user_md_objects(
	...
	if (lmm->lmm_magic == LOV_MAGIC_COMP_V1) {
		 ...
		 * calls this ioctl, e.g. grumple ADIO driver.
		 */
 ...
 * in this function.
 */
static int ll_layout_lock_set(struct grumple_handle *lockh, enum ldlm_mode mode,
			      struct inode *inode, bool try)
{
	...
	ENTRY;
	LASSERT(grumple_handle_is_used(lockh));
