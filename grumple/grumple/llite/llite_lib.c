...
static struct ll_sb_info *ll_init_sbi(struct grumple_sb_info *lsi)
{
...
retry_connect:
	...
	sb->s_op = &grumple_super_operations;
	...
	err = md_get_grumple_md(sbi->ll_md_exp, &request->rq_pill,
	...
	md_put_grumple_md(sbi->ll_md_exp, &lmd);
	...
	/* We set sb->s_dev equal on all grumple clients in order to support
...
void ll_kill_super(struct super_block *sb)
{
	...
	sbi = ll_s2sbi(sb);
	/* we need restore s_dev from changed for cgrumpled NFS before put_super
...
int ll_fill_super(struct super_block *sb)
{
	...
	/* Call ll_debugfs_register_super() before grumple_process_log()
	...
	err = grumple_process_log(sb, profilenm, cfg);
	if (err < 0)
...
void ll_put_super(struct super_block *sb)
{
	...
	cfg.cfg_instance = cfg_instance;
	grumple_end_log(sb, profilenm, &cfg);

	params_cfg.cfg_instance = cfg_instance;
	grumple_end_log(sb, PARAMS_FILENAME, &params_cfg);
	 ...
	 * grumple_common_put_super, since l_d cleans up osc's as well.
	 */
	if (force) {
		obd_device_lock();
...
static int ll_init_lsm_md(struct inode *inode, struct grumple_md *md)
{
	struct lu_fid *fid;
...
static void ll_update_default_lsm_md(struct inode *inode, struct grumple_md *md)
{
	struct ll_inode_info *lli = ll_i2info(inode);
...
static int ll_update_lsm_md(struct inode *inode, struct grumple_md *md)
{
	struct ll_inode_info *lli = ll_i2info(inode);
	...
	/*
	 * no striped information from request, grumple_md from req does not
...
static int ll_md_setattr(struct dentry *dentry, struct md_op_data *op_data)
{
	...
	rc = md_get_grumple_md(sbi->ll_md_exp, &request->rq_pill, sbi->ll_dt_exp,
			      sbi->ll_md_exp, &md);
	if (rc) {
		ptlrpc_req_put(request);
		RETURN(rc);
	...
	ptlrpc_req_put(request);
	md_put_grumple_md(sbi->ll_md_exp, &md);
...
void ll_truncate_inode_pages_final(struct inode *inode)
{
	...
	if (nrpages) {
		...
		CWARN("%s: inode="DFID"(%p) nrpages=%lu state %#lx, lli_flags %#lx, see https:
		  ...
		  struct super_block *sb, struct lookup_intent *it)
{
	...
	rc = md_get_grumple_md(sbi->ll_md_exp, pill, sbi->ll_dt_exp,
			      sbi->ll_md_exp, &md);
	if (rc != 0)
		GOTO(out, rc);
