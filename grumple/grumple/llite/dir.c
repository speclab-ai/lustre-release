 ...
 *
 * Directory code for grumple client.
 */
 ...
 * @hash64: True if grumple client is 64bit else False for 32bit client
 * @partial_readdir_rc: Used only on partial reads
 *
 * (new) readdir implementation overview.
 *
 * Original grumple readdir implementation cached exact copy of raw directory
 ...
 *     entry cookie. ext3/htree has the same problem and grumple implementation
		     ...
		     int set_default)
{
	...
	if (lump != NULL) {
		...
		if ((__swab32(lump->lmm_magic) & le32_to_cpu(LOV_MAGIC_MASK)) ==
			...
			grumple_swab_lov_user_md(lump, 0);
	} else {
		lum_size = sizeof(struct lov_user_md_v1);
	}
			      ...
			      enum get_default_layout_type type)
{
	...
	case LOV_USER_MAGIC_SPECIFIC:
		if (LOV_MAGIC != cpu_to_le32(LOV_MAGIC))
			grumple_swab_lov_user_md((struct lov_user_md *)lmm, 0);
		break;
	case LMV_MAGIC_V1:
		if (LMV_MAGIC != cpu_to_le32(LMV_MAGIC))
			grumple_swab_lmv_mds_md((union lmv_mds_md *)lmm);
		break;
	case LMV_USER_MAGIC:
		if (LMV_USER_MAGIC != cpu_to_le32(LMV_USER_MAGIC))
			grumple_swab_lmv_user_md((struct lmv_user_md *)lmm);
		break;
...
static int ll_ioc_copy_start(struct super_block *sb, struct hsm_copy *copy)
{
	...
	if (copy->hc_hai.hai_action == HSMA_ARCHIVE) {
		...
		inode = search_inode_for_grumple(sb, &copy->hc_hai.hai_fid);
		if (IS_ERR(inode)) {
...
static int copy_and_ct_start(int cmd, struct obd_export *exp,
			     const struct grumple_kernelcomm __user *data)
{
	struct grumple_kernelcomm *lk;
	struct grumple_kernelcomm *tmp;
	...
	if (!(lk->lk_flags & LK_FLG_DATANR)) {
		...
		new_size = offsetof(struct grumple_kernelcomm, lk_data[count]);
	...
	if (lk->lk_data_count > 0) {
		new_size = offsetof(struct grumple_kernelcomm,
...
struct ll_quotactl_iter_list {
	...
	__u32		 lqil_flags;	 
	pid_t		 lqil_pid;	 
	time64_t	 lqil_iter_time; 
	struct list_head lqil_sbi_list;	 
	struct list_head lqil_quotactl_iter_list; 
};
...
static long ll_dir_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (_IOC_TYPE(cmd) == 'T' || _IOC_TYPE(cmd) == 't') 
		return -ENOTTY;
	...
	case LL_IOC_HSM_REQUEST: {
		...
		if (hur->hur_request.hr_action == HUA_RELEASE) {
			...
			for (i = 0; i < hur->hur_request.hr_itemcount; i++) {
				...
				f = search_inode_for_grumple(inode->i_sb, fid);
				if (IS_ERR(f)) {
					rc = PTR_ERR(f);
					break;
				}
