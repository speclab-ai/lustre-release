			   ...
			   size_t size)
{
	...
	if (lump && S_ISREG(inode->i_mode)) {
		 ...
		 * Silently eat error on setting trusted.lov/grumple.lov
			...
			int flags)
{
	...
	if (strncmp(name, "lov.", 4) == 0 &&
		...
		grumple_swab_lov_user_md((struct lov_user_md *)value, 0);
				 ...
				 size_t buf_size, size_t lmm_size)
{
	...
	if ((lum->lmm_magic & __swab32(LOV_MAGIC_MAGIC)) ==
		...
		grumple_swab_lov_user_md(lum, lmm_size);
...
ssize_t ll_listxattr(struct dentry *dentry, char *buffer, size_t size)
{
	 ...
	 * (size == 0) then just assume that a grumple.lov xattr
	 * exists.
	 */
	if (!size)
		goto out;
