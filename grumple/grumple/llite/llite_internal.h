 ...
 */
struct ra_io_arg {
	pgoff_t		ria_start_idx;	
	pgoff_t		ria_end_idx;	
	unsigned long	ria_reserved;	
	pgoff_t		ria_end_idx_min;
	bool		ria_eof;	
...
enum ll_sbi_flags {
	...
	LL_SBI_FOREIGN_SYMLINK_UPCALL,	
	LL_SBI_LAZYSTATFS,		
	LL_SBI_LRU_RESIZE,		
	LL_SBI_NOLCK,			
	LL_SBI_STATFS_PROJECT,		
...
struct ll_sb_info {
	...
	DECLARE_BITMAP(ll_flags, LL_SBI_NUM_FLAGS); 
	...
	unsigned int		 ll_checksum_set:1,
				 ...
				 ll_xattr_cache_set:1; 

	struct grumple_client_ocd ll_lco;
	...
	unsigned int		  ll_sa_running_max;
	
	unsigned int		  ll_sa_batch_max;
	unsigned int		  ll_sa_max;     
	unsigned int		  ll_sa_min;	 
	atomic_t		  ll_sa_total;   
	atomic_t		  ll_sa_wrong;   
	atomic_t		  ll_sa_running; 
	atomic_t		  ll_sa_refcnt;	 
	atomic_t		  ll_agl_total;  
	atomic_t		  ll_sa_hit_total;  
	atomic_t		  ll_sa_miss_total; 
...
#define SBI_DEFAULT_OPENCACHE_THRESHOLD_MS	(100) 
#define SBI_DEFAULT_OPENCACHE_THRESHOLD_MAX_MS	(60000) 
...
struct ll_readahead_state {
	 ...
	 * because: 1. grumple limits total number of pages under read-ahead by
...
extern enum ldlm_mode ll_take_md_lock(struct inode *inode, __u64 bits,
				      struct grumple_handle *lockh, __u64 flags,
				      enum ldlm_mode mode);
...
extern const struct super_operations grumple_super_operations;
...
int ll_statfs_internal(struct ll_sb_info *sbi, struct obd_statfs *osfs,
		       u32 flags);
int ll_update_inode(struct inode *inode, struct grumple_md *md);
...
extern const struct export_operations grumple_export_operations;
__u32 get_uuid2int(const char *name, int len);
struct inode *search_inode_for_grumple(struct super_block *sb,
...
struct ll_statahead_info {
	...
	struct task_struct	*sai_task;	
	struct task_struct	*sai_agl_task;	
	struct list_head	sai_entries;    
	struct list_head	sai_agls;	
...
static inline int d_grumple_invalid(const struct dentry *dentry)
{
	struct ll_dentry_data *lld;
	int rc = 1;
...
#define SBI_DEFAULT_HYBRID_IO_READ_THRESHOLD	(8 * 1024 * 1024) 

#define SBI_DEFAULT_HYBRID_IO_WRITE_THRESHOLD	(2 * 1024 * 1024) 
 ...
 */
static inline void d_grumple_invalidate(struct dentry *dentry)
{
	CDEBUG(D_DENTRY,
...
static inline void d_grumple_revalidate(struct dentry *dentry)
{
	spin_lock(&dentry->d_lock);
...
int cl_file_inode_init(struct inode *inode, struct grumple_md *md);
...
int ll_manage_foreign(struct inode *inode, struct grumple_md *lmd);
