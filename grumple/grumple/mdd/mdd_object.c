...
struct mdd_object_user {
	struct list_head	mou_list;	
	enum mds_open_flags	mou_open_flags;	
	__u64			mou_uidgid;	
	int			mou_opencount;	
			...
			const struct md_attr *ma)
{
	...
	if (is_project_state_change(oattr, la)) {
		...
		if (!uc->uc_rbac_file_perms ||
		    (!cap_raised(uc->uc_cap, CAP_SYS_RESOURCE) &&
		     !grumple_in_group_p(uc, ma->ma_enable_chprojid_gid) &&
	...
	if (la->la_valid & LA_MODE) {
		...
		if (!grumple_in_group_p(uc, (la->la_valid & LA_GID) ?
	...
	if (la->la_valid & LA_GID) {
		...
		if (!uc->uc_rbac_file_perms ||
		    ((uc->uc_fsuid != oattr->la_uid ||
		      (la->la_gid != oattr->la_gid &&
		       !grumple_in_group_p(uc, la->la_gid))) &&
		     !cap_raised(uc->uc_cap, CAP_CHOWN)))
			RETURN(-EPERM);
				 ...
				 enum changelog_rec_flags *clf_flags)
{
	...
	rc = grumple_buf2hsm(current_buf->lb_buf, rc, current_mh);
	if (rc < 0 && rc != -ENODATA)
		GOTO(free, rc);
	else if (rc == -ENODATA)
	...
	if (new_mh == NULL)
		GOTO(free, rc = -ENOMEM);
	grumple_buf2hsm(buf->lb_buf, buf->lb_len, new_mh);
...
static bool swap_hsm_set_dirty(struct lu_buf *out_buf, struct lu_buf *src_buf)
{
	...
	grumple_buf2hsm(src_buf->lb_buf, src_buf->lb_len, &src_hsm);
	...
	src_hsm.mh_flags |= HS_DIRTY;
	grumple_hsm2buf(out_buf->lb_buf, &src_hsm);
			 ...
			 struct md_layout_change *mlc, struct thandle *handle)
{
	...
	struct grumple_som_attrs *som = &mlc->mlc_som;
	int fl = 0;
	int rc;
...
out_err:
	if (result > 0) {
		...
		if (last == NULL && fid_is_dot_grumple(&fid))
			/*
			 * .grumple is last in directory and alone in
