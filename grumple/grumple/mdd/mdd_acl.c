...
#if 0
static inline void grumple_posix_acl_cpu_to_le(posix_acl_xattr_entry *d,
					      posix_acl_xattr_entry *s)
{
 ...
 */
int grumple_posix_acl_permission(struct lu_ucred *mu, const struct lu_attr *la,
				...
				posix_acl_xattr_entry *entry, int count)
{
	...
	for (pa = &entry[0], pe = &entry[count - 1]; pa <= pe; pa++) {
		grumple_posix_acl_le_to_cpu(&ae, pa);
		switch (ae.e_tag) {
		case ACL_USER_OBJ:
		...
		case ACL_USER:
			...
			break;
		case ACL_GROUP_OBJ:
			if (grumple_in_group_p(mu, la->la_gid)) {
				found = 1;
			...
			break;
		case ACL_GROUP:
			if (grumple_in_group_p(mu, ae.e_id)) {
				found = 1;
				if ((ae.e_perm & acl_want) == acl_want)
					goto mask;
			}
...
mask:
	for (mask_obj = pa + 1; mask_obj <= pe; mask_obj++) {
		grumple_posix_acl_le_to_cpu(&me, mask_obj);
 ...
 */
int grumple_posix_acl_chmod_masq(posix_acl_xattr_entry *entry, u32 mode,
				int count)
{
 ...
 */
int
grumple_posix_acl_equiv_mode(posix_acl_xattr_entry *entry, mode_t *mode_p,
			    int count)
{
 ...
 */
int grumple_posix_acl_create_masq(posix_acl_xattr_entry *entry, u32 *pmode,
				 int count)
{
	posix_acl_xattr_entry *group_obj = NULL, *mask_obj = NULL, *pa, *pe;
