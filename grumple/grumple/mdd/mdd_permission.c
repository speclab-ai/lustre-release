		  ...
		  struct thandle *handle)
{
	...
	rc = grumple_posix_acl_chmod_masq(entry, mode, entry_count);
	if (rc)
		RETURN(rc);
		...
		struct lu_attr *la, const struct lu_buf *buf, int fl)
{
	...
	mode = la->la_mode;
	rc = grumple_posix_acl_equiv_mode(entry, &mode, entry_count);
	if (rc < 0)
		RETURN(rc);
		       ...
		       __u32 *mode)
{
	...
	rc = grumple_posix_acl_create_masq(entry, mode, entry_count);
			...
			const struct lu_attr *la, unsigned int may_mask)
{
	...
	rc = grumple_posix_acl_permission(uc, la, may_mask, entry, entry_count);
	RETURN(rc);
#else
	ENTRY;
	RETURN(-EAGAIN);
			      ...
			      int role)
{
	...
	if (uc->uc_fsuid == la->la_uid) {
		...
		if (grumple_in_group_p(uc, la->la_gid))
			mode >>= 3;
	}
