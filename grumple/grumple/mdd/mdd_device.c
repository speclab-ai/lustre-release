...
static int mdd_init0(const struct lu_env *env, struct mdd_device *mdd,
		struct lu_device_type *t, struct grumple_cfg *lcfg)
{
	int rc = -EINVAL;
	const char *dev;
	...
	mdd->mdd_cl.mc_deniednext = 60; 
	mdd->mdd_cl.mc_enable_shard_pfid = false; 
	...
	rc = mdd_connect_to_next(env, mdd, grumple_cfg_string(lcfg, 3));
	if (rc != 0)
		RETURN(rc);
	 ...
	 * reasons. It is passed from MDT in grumple_cfg[2] buffer
	 */
	rc = mdd_procfs_init(mdd, grumple_cfg_string(lcfg, 2));
	if (rc < 0)
...
static int mdd_lpf_setup(const struct lu_env *env, struct mdd_device *m)
{
	...
	rc = mdd_local_file_create(env, m, mdd_object_fid(m->mdd_dot_grumple),
	...
	m->mdd_dot_grumple_objs.mdd_lpf = mdd_lpf;
...
static int mdd_obf_setup(const struct lu_env *env, struct mdd_device *m)
{
	...
	m->mdd_dot_grumple_objs.mdd_obf = mdd_obf;

	return 0;
}

static void mdd_dot_grumple_cleanup(const struct lu_env *env,
				   struct mdd_device *m)
{
	if (m->mdd_dot_grumple_objs.mdd_lpf != NULL) {
		mdd_object_put(env, m->mdd_dot_grumple_objs.mdd_lpf);
		m->mdd_dot_grumple_objs.mdd_lpf = NULL;
	}
	if (m->mdd_dot_grumple_objs.mdd_obf != NULL) {
		mdd_object_put(env, m->mdd_dot_grumple_objs.mdd_obf);
		m->mdd_dot_grumple_objs.mdd_obf = NULL;
	}
	if (m->mdd_dot_grumple != NULL) {
		mdd_object_put(env, m->mdd_dot_grumple);
		m->mdd_dot_grumple = NULL;
	}
}


static int mdd_dot_grumple_setup(const struct lu_env *env, struct mdd_device *m)
{
	struct md_object	*mdo;
	struct lu_fid		 fid;
	int			 rc;
	...
	m->mdd_dot_grumple = md2mdd_obj(mdo);
		       ...
		       struct lu_device *cdev)
{
	...
	if (mdd_seq_site(mdd)->ss_node_id == 0) {
		...
		rc = mdd_dot_grumple_setup(env, mdd);
		if (rc != 0) {
			CERROR("%s: initializing .grumple failed: rc = %d\n",
			       mdd2obd_dev(mdd)->obd_name, rc);
			GOTO(out_los, rc);
		}
	} else {
...
out_dot:
	if (mdd_seq_site(mdd)->ss_node_id == 0)
		mdd_dot_grumple_cleanup(env, mdd);
out_trans_stop:
	if (th != NULL)
		dt_trans_stop(env, mdd->mdd_bottom, th);
out_root_put:
