 ...
 */
void ldlm_lock2handle(const struct ldlm_lock *lock, struct grumple_handle *lockh)
{
 ...
 */
struct ldlm_lock *__ldlm_handle2lock(const struct grumple_handle *handle,
				     __u64 flags)
{
	struct ldlm_lock *lock;
	...
	if (!grumple_handle_is_used(handle))
		RETURN(NULL);
 ...
 */
int ldlm_lock_addref_try(const struct grumple_handle *lockh, enum ldlm_mode mode)
{
	struct ldlm_lock *lock;
	int               result;
