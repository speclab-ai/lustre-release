...
int ldlm_cli_enqueue_fini(struct obd_export *exp, struct req_capsule *pill,
			  ...
			  __u32 lvb_len, const struct grumple_handle *lockh,
...
static inline int ldlm_req_handles_avail(int req_size, int off)
{
	...
	if (likely(avail >= 0))
		avail /= (int)sizeof(struct grumple_handle);
	else
		avail = 0;
			    ...
			    struct list_head *head, int count)
{
	...
	max /= sizeof(struct grumple_handle);
...
int ldlm_cli_update_pool(struct ptlrpc_request *req)
{
	 ...
	 */
	if (grumple_msg_get_slv(req->rq_repmsg) == 0 ||
	    grumple_msg_get_limit(req->rq_repmsg) == 0) {
		DEBUG_REQ(D_HA, req,
			  "Zero SLV or limit found (SLV=%llu, limit=%u)",
			  grumple_msg_get_slv(req->rq_repmsg),
			  grumple_msg_get_limit(req->rq_repmsg));
		RETURN(0);
	}

	new_limit = grumple_msg_get_limit(req->rq_repmsg);
	new_slv = grumple_msg_get_slv(req->rq_repmsg);
...
static int replay_one_lock(struct obd_import *imp, struct ldlm_lock *lock)
{
	 ...
	 * b=6063
	 */
	grumple_msg_set_flags(req->rq_reqmsg, MSG_REQ_REPLAY_DONE);
