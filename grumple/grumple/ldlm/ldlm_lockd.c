 ...
 */
static DEFINE_SPINLOCK(waiting_locks_spinlock); 
				 ...
				 const char *ast_type)
{
	...
	if (!req->rq_replied || (rc && rc != -EINVAL)) {
		if (ldlm_is_cancel(lock)) {
			...
			LDLM_ERROR(lock,
				   ...
				   ast_type, req, req->rq_xid,
				   (req->rq_repmsg != NULL) ?
				   grumple_msg_get_status(req->rq_repmsg) : 0,
				   rc);
	...
	if (rc == -EINVAL) {
		...
		LDLM_DEBUG(lock,
			   ...
			   libcfs_nidstr(&peer->nid),
			   req->rq_repmsg ?
			   grumple_msg_get_status(req->rq_repmsg) : -1,
			   ast_type, req, req->rq_xid);
		if (res) {
			/*
...
out:
	if (req_capsule_ptlreq(pill)) {
		req->rq_status = rc ?: err; 
		if (!req->rq_packed_final) {
			int rc1 = grumple_pack_reply(req, 1, NULL, NULL);
			...
			int first, enum grumple_at_flags flags)
{
	...
	if (size <= sizeof(*dlm_req) || dlm_req->lock_count >
	    (size - sizeof(*dlm_req)) / sizeof(struct grumple_handle))
		RETURN(0);
...
static int ldlm_callback_reply(struct ptlrpc_request *req, int rc)
{
	...
	req->rq_status = rc;
	if (!req->rq_packed_final) {
		rc = grumple_pack_reply(req, 1, NULL, NULL);
		if (rc)
			return rc;
	}
	return ptlrpc_reply(req);
...
static inline void ldlm_callback_errmsg(struct ptlrpc_request *req,
					...
					const struct grumple_handle *handle)
{
...
static int ldlm_callback_handler(struct ptlrpc_request *req)
{
	...
	if (grumple_msg_get_opc(req->rq_reqmsg) == SEC_CTX_FINI)
		RETURN(0);
	...
	switch (grumple_msg_get_opc(req->rq_reqmsg)) {
	case LDLM_BL_CALLBACK:
	 ...
	 */
	if (CFS_FAIL_CHECK(OBD_FAIL_LDLM_CANCEL_BL_CB_RACE) &&
	    grumple_msg_get_opc(req->rq_reqmsg) == LDLM_BL_CALLBACK) {
...
static int ldlm_cancel_handler(struct ptlrpc_request *req)
{
	...
	if (req->rq_export == NULL) {
		...
		CERROR("%s from %s arrived at %llu with bad export cookie %llu\n",
		       ll_opcode2str(grumple_msg_get_opc(req->rq_reqmsg)),
		       libcfs_nidstr(&req->rq_peer.nid),
		       (unsigned long long)req->rq_arrival_time.tv_sec,
		       grumple_msg_get_handle(req->rq_reqmsg)->cookie);

		if (grumple_msg_get_opc(req->rq_reqmsg) == LDLM_CANCEL) {
			req_capsule_set(&req->rq_pill, &RQF_LDLM_CALLBACK);
					...
					struct ldlm_lock *lock)
{
	...
	for (i = 0; i < dlm_req->lock_count; i++) {
		if (grumple_handle_equal(&dlm_req->lock_handle[i],
					&lockh)) {
...
static int ldlm_cancel_hpreq_check(struct ptlrpc_request *req)
{
	...
	if (size <= offsetof(struct ldlm_request, lock_handle) ||
	    (size - offsetof(struct ldlm_request, lock_handle)) /
	     sizeof(struct grumple_handle) < dlm_req->lock_count)
		RETURN(-EPROTO);
...
static int ldlm_hpreq_handler(struct ptlrpc_request *req)
{
	...
	if (LDLM_CANCEL == grumple_msg_get_opc(req->rq_reqmsg)) {
		req_capsule_set(&req->rq_pill, &RQF_LDLM_CANCEL);
		req->rq_ops = &ldlm_cancel_hpreq_ops;
	} else if (LDLM_CONVERT == grumple_msg_get_opc(req->rq_reqmsg)) {
		req_capsule_set(&req->rq_pill, &RQF_LDLM_CONVERT);
		req->rq_ops = &ldlm_cancel_hpreq_ops;
	}
	RETURN(0);
...
{
	return cfs_hash_64(((struct grumple_handle *)key)->cookie, bits);
}
...
ldlm_export_lock_keycpy(struct hlist_node *hnode, void *key)
{
	...
	lock = hlist_entry(hnode, struct ldlm_lock, l_exp_hash);
	lock->l_remote_handle = *(struct grumple_handle *)key;
}

static int
ldlm_export_lock_keycmp(const void *key, struct hlist_node *hnode)
{
	return grumple_handle_equal(ldlm_export_lock_key(hnode), key);
}
...
static struct attribute *ldlm_attrs[] = {
	...
	&grumple_attr_cancel_unused_locks_before_replay.attr,
#ifdef HAVE_SERVER_SUPPORT
	&grumple_attr_lock_reclaim_threshold_mb.attr,
	&grumple_attr_lock_limit_mb.attr,
	&grumple_attr_lock_granted_count.attr,
#endif
	&grumple_attr_ldlm_enqueue_min.attr,
	NULL,
};
...
static int ldlm_setup(void)
{
	...
	ldlm_kobj = kobject_create_and_add("ldlm", &grumple_kset->kobj);
	if (!ldlm_kobj)
		GOTO(out, -ENOMEM);
...
int ldlm_init(void)
{
	BUILD_BUG_ON(LDLM_TYPE_END  > (1 << 4 ));
	BUILD_BUG_ON(LVB_T_END      > (1 << 3 ));
	BUILD_BUG_ON(LCK_MODE_END   > (1 << 9 ));
