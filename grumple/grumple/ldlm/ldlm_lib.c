 ...
 * In the config log,
 * osc on MDT
 *	setup 0:{fsname}-OSTxxxx-osc[-MDTxxxx] 1:grumple-OST0000_UUID 2:NID
 * osc on client
 *	setup 0:{fsname}-OSTxxxx-osc 1:grumple-OST0000_UUID 2:NID
 *
 **/
...
int client_obd_setup(struct obd_device *obd, struct grumple_cfg *lcfg)
{
	...
	char *cli_name = grumple_cfg_buf(lcfg, 0);
	int rc;
	...
	} else if (cfs_totalram_pages() >> (20 - PAGE_SHIFT) <= 256 ) {
		cli->cl_max_rpcs_in_flight = 3;
	} else if (cfs_totalram_pages() >> (20 - PAGE_SHIFT) <= 512 ) {
	...
	if (grumple_cfg_buf(lcfg, 4)) {
		__u32 refnet = libcfs_str2net(grumple_cfg_string(lcfg, 4));

		if (refnet == LNET_NET_ANY) {
			rc = -EINVAL;
			CERROR("%s: bad mount option 'network=%s': rc = %d\n",
			       obd->obd_name, grumple_cfg_string(lcfg, 4),
			       rc);
			GOTO(err_import, rc);
		}
 ...
 * --------------------------------------------------------------------------
 */
static int target_handle_reconnect(struct grumple_handle *conn,
				   struct obd_export *exp,
				   struct obd_uuid *cluuid)
{
	struct obd_device *target;
	struct grumple_handle *hdl;
	ktime_t remaining;
	s64 timeout;
	int rc = 0;
	...
	if (!exp->exp_connection ||
	    !grumple_handle_is_used(&exp->exp_imp_reverse->imp_remote_handle)) {
				...
				struct ptlrpc_request *req)
{
	 ...
	 * ptlrpc_handle_server_req_in->grumple_unpack_msg()
	 */
...
int target_handle_connect(struct ptlrpc_request *req)
{
	 ...
	 * reconnect case
	 */
	struct grumple_handle conn;
	struct grumple_handle *tmp;
	struct obd_uuid cluuid;
	char *str;
	int rc = 0;
	char *target_start;
	...
	/*
	 * Don't allow libgrumple clients to connect.
	 ...
	 */
	if (grumple_msg_get_op_flags(req->rq_reqmsg) & MSG_CONNECT_LIBCLIENT) {
	...
	if (grumple_msg_get_op_flags(req->rq_reqmsg) & MSG_CONNECT_INITIAL) {
		initial_conn = true;
...
no_export:
		...
		LCONSOLE_WARN("%s: Client %s (at %s) refused connection, still busy with %d references\n",
			...
			GOTO(out, rc = -EBUSY);
	} else if (grumple_msg_get_conn_cnt(req->rq_reqmsg) == 1 &&
		   rc != EALREADY) {
	 ...
	 */
	if (rc == EALREADY) {
		grumple_msg_add_op_flags(req->rq_repmsg, MSG_CONNECT_RECONNECT);
		rc = 0;
	} else {
		LASSERT(rc == 0);
	}
	...
	if (target->obd_replayable)
		grumple_msg_add_op_flags(req->rq_repmsg, MSG_CONNECT_REPLAYABLE);
...
dont_check_exports:
			...
			if (mds_conn && CFS_FAIL_CHECK(OBD_FAIL_TGT_RCVG_FLAG))
				grumple_msg_add_op_flags(req->rq_repmsg,
	...
	if (export->exp_conn_cnt >= grumple_msg_get_conn_cnt(req->rq_reqmsg)) {
		spin_unlock(&export->exp_lock);
		CDEBUG(D_RPCTRACE,
	...
	grumple_msg_set_handle(req->rq_repmsg, &conn);
	...
	if (test_bit(OBDF_RECOVERING, target->obd_flags) &&
		...
		has_transno = !!(grumple_msg_get_op_flags(req->rq_reqmsg) &
				 MSG_CONNECT_TRANSNO);
	...
	if (test_bit(OBDF_RECOVERING, target->obd_flags) && !lw_client)
		grumple_msg_add_op_flags(req->rq_repmsg, MSG_CONNECT_RECOVERING);
...
{
	__u64 transno = grumple_msg_get_transno(req->rq_reqmsg);
	...
	list_for_each_entry(reqiter, &exp->exp_req_replay_queue,
			    rq_replay_list) {
		if (grumple_msg_get_transno(reqiter->rq_reqmsg) == transno) {
			dup_req = reqiter;
			dup = 1;
			break;
		}
	...
	if (dup) {
		...
		if ((grumple_msg_get_flags(req->rq_reqmsg) &
		    (MSG_RESENT | MSG_REPLAY)) != (MSG_RESENT | MSG_REPLAY))
			CERROR("invalid flags %x of resent replay\n",
			       grumple_msg_get_flags(req->rq_reqmsg));

		if (grumple_msg_get_flags(req->rq_reqmsg) & MSG_REPLAY) {
			__u32 new_conn;

			new_conn = grumple_msg_get_conn_cnt(req->rq_reqmsg);
			if (new_conn >
			    grumple_msg_get_conn_cnt(dup_req->rq_reqmsg))
				grumple_msg_set_conn_cnt(dup_req->rq_reqmsg,
							new_conn);
		}
	} else {
...
check_and_start_recovery_timer(struct obd_device *obd,
			       ...
			       int new_client)
{
	timeout_t service_timeout = grumple_msg_get_service_timeout(req->rq_reqmsg);
...
static __u64 get_next_replay_req_transno(struct obd_device *obd)
{
	...
	if (!list_empty(&obd->obd_req_replay_queue)) {
		...
		req = list_first_entry(&obd->obd_req_replay_queue,
				       struct ptlrpc_request, rq_list);
		transno = grumple_msg_get_transno(req->rq_reqmsg);
	}
				      ...
				      struct ptlrpc_request *req)
{
	...
	if (opc == MDS_REINT) {
		...
		grumple_msg_set_transno(req->rq_repmsg, req->rq_transno);
				    ...
				    struct ptlrpc_request *req)
{
	...
	LASSERT(exp != NULL);
	if (grumple_msg_get_flags(req->rq_reqmsg) & MSG_REQ_REPLAY_DONE) {
	...
	if (grumple_msg_get_flags(req->rq_reqmsg) & MSG_LOCK_REPLAY_DONE) {
		/*
				  ...
				  struct obd_device *obd)
{
	...
	list_for_each_entry(reqiter, &obd->obd_req_replay_queue, rq_list) {
		...
		if (unlikely(grumple_msg_get_transno(reqiter->rq_reqmsg) ==
			     transno)) {
			DEBUG_REQ(D_ERROR, req,
...
void target_committed_to_req(struct ptlrpc_request *req)
{
	...
	if (!exp->exp_obd->obd_no_transno && req->rq_repmsg != NULL)
		grumple_msg_set_last_committed(req->rq_repmsg,
...
int target_pack_pool_reply(struct ptlrpc_request *req)
{
	...
	if (unlikely(!req->rq_export || !req->rq_export->exp_obd ||
		...
		grumple_msg_set_slv(req->rq_repmsg, 0);
		grumple_msg_set_limit(req->rq_repmsg, 0);
		RETURN(0);
	}
	...
	grumple_msg_set_slv(req->rq_repmsg, obd->obd_pool_slv);
	grumple_msg_set_limit(req->rq_repmsg, obd->obd_pool_limit);
				 ...
				 int rc, int fail_id)
{
	 ...
	 */
	if (req->rq_reqmsg &&
	    unlikely(grumple_msg_get_opc(req->rq_reqmsg) == MDS_REINT &&
...
void target_send_reply(struct ptlrpc_request *req, int rc, int fail_id)
{
	...
	rs->rs_export    = exp;
	rs->rs_opc       = grumple_msg_get_opc(req->rq_reqmsg);
...
int target_bulk_io(struct obd_export *exp, struct ptlrpc_bulk_desc *desc)
{
	...
	if (exp->exp_failed ||
	    exp->exp_conn_cnt > grumple_msg_get_conn_cnt(req->rq_reqmsg)) {
		rc = -ENOTCONN;
	} else {
		if (req->rq_bulk_read)
		...
		if (OCD_HAS_FLAG(&exp->exp_connect_data, BULK_MBITS))
			req->rq_mbits = grumple_msg_get_mbits(req->rq_reqmsg);
		else 
