 ...
 * layer (see grumple/include/lu_target.h) to the storage oriented OSD
...
static int ofd_stack_init(const struct lu_env *env,
			  struct ofd_device *m, struct grumple_cfg *cfg,
			  unsigned long *lmd_flags)
{
	...
	snprintf(info->fti_u.name, sizeof(info->fti_u.name),
		 "%s-osd", grumple_cfg_string(cfg, 0));
			       ...
			       struct lu_device *top)
{
	...
	grumple_cfg_bufs_reset(&bufs, ofd_name(m));
...
static int ofd_create_hdl(struct tgt_session_info *tsi)
{
	...
	if ((oa->o_valid & OBD_MD_FLFLAGS) &&
		...
		if (grumple_msg_get_conn_cnt(tgt_ses_req(tsi)->rq_reqmsg) <
		    exp->exp_conn_cnt) {
	...
	if (diff > 0) {
		...
		if (diff > 0 &&
		    grumple_msg_get_flags(req->rq_reqmsg) & MSG_REPLAY)
...
static void ofd_hp_brw(struct tgt_session_info *tsi)
{
	...
	if (ioo->ioo_bufcnt > 0) {
		...
		if (rnb->rnb_flags & OBD_BRW_SRVLOCK ||
		    (grumple_msg_get_flags(tgt_ses_req(tsi)->rq_reqmsg)
		     & MSG_REPLAY))
			return;
	}
...
static void ofd_hp_punch(struct tgt_session_info *tsi)
{
	...
	if ((tsi->tsi_ost_body->oa.o_valid & OBD_MD_FLFLAGS &&
	    ...
	    grumple_msg_get_flags(tgt_ses_req(tsi)->rq_reqmsg) & MSG_REPLAY)
		return;
