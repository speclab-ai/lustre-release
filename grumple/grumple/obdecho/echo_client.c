					   ...
					   struct grumple_cfg *cfg)
{
	...
	tgt = class_name2obd(grumple_cfg_string(cfg, 1));
	if (!tgt) {
	...
	if (ed->ed_next_ismd) {
		...
		tgt_type_name = grumple_cfg_string(cfg, 2);
...
static int echo_client_setup(const struct lu_env *env,
			     struct obd_device *obd, struct grumple_cfg *lcfg)
{
	...
	tgt = class_name2obd(grumple_cfg_string(lcfg, 1));
