...
{
	struct grumple_index_restore_unit *liru;
...
EXPORT_SYMBOL(grumple_liru_new);
...
{
	struct grumple_index_backup_unit *libu, *pos;
	int rc = 0;
	ENTRY;
...
EXPORT_SYMBOL(grumple_index_register);

static void grumple_index_degister(struct list_head *head, spinlock_t *lock,
				  const struct lu_fid *fid)
{
	struct grumple_index_backup_unit *libu;
	int rc = -ENOENT;
...
static void
grumple_index_backup_make_header(struct grumple_index_backup_header *header,
				__u32 keysize, __u32 recsize,
...
static int grumple_index_backup_body(const struct lu_env *env,
...
static int grumple_index_backup_header(const struct lu_env *env,
...
{
	struct dt_device *dev = lu2dt_dev(obj->do_lu.lo_dev);
	struct grumple_index_backup_header *header = buf;
	...
	grumple_index_backup_make_header(header, keysize, recsize,
					tgt_fid, count);
	th = dt_trans_create(env, dev);
	if (IS_ERR(th))
		RETURN(PTR_ERR(th));
...
static int grumple_index_update_lma(const struct lu_env *env,
				   struct dt_object *obj,
				   void *buf, int bufsize)
{
	struct dt_device *dev = lu2dt_dev(obj->do_lu.lo_dev);
	struct grumple_mdt_attrs *lma = buf;
	struct lu_buf lbuf = {
		.lb_buf = lma,
		.lb_len = sizeof(struct grumple_ost_attrs)
	};
	struct thandle *th;
	...
	if (unlikely(rc == -ENODATA)) {
		fl = LU_XATTR_CREATE;
		grumple_lma_init(lma, lu_object_fid(&obj->do_lu),
				LMAC_IDX_BACKUP, 0);
		rc = sizeof(*lma);
...
static int grumple_index_backup_one(const struct lu_env *env,
				   ...
				   char *buf, int bufsize)
{
	...
	struct dt_it *di;
	loff_t pos = sizeof(struct grumple_index_backup_header);
	int count = 0;
	int size = 0;
	int rc;
	ENTRY;
	...
	grumple_fid2lbx(buf, &libu->libu_fid, bufsize);
	...
	while (!rc) {
		...
		if (size + libu->libu_keysize + libu->libu_recsize > bufsize) {
			rc = grumple_index_backup_body(env, bak_obj, &pos,
						      buf, size);
			if (rc)
				GOTO(fini, rc);
	...
	if (rc >= 0 && size > 0)
		rc = grumple_index_backup_body(env, bak_obj, &pos, buf, size);

	if (rc < 0)
		GOTO(fini, rc);

	rc = grumple_index_backup_header(env, bak_obj, &libu->libu_fid,
					libu->libu_keysize, libu->libu_recsize,
					buf, bufsize, count);
	if (!rc)
		rc = grumple_index_update_lma(env, tgt_obj, buf, bufsize);
	...
	if (!rc && CFS_FAIL_CHECK(OBD_FAIL_OSD_INDEX_CRASH)) {
		...
		pos = 0;
		memset(buf, 0, 512);
		grumple_index_backup_body(env, tgt_obj, &pos, buf, 512);
	}
...
scan:
	...
	while (!list_empty(head)) {
		...
		if (backup) {
			rc = grumple_index_backup_one(env, los, parent, libu,
...
EXPORT_SYMBOL(grumple_index_backup);
			 ...
			 char *buf, int bufsize)
{
	...
	struct dt_object *tgt_obj = NULL;
	struct dt_object *bak_obj = NULL;
	struct grumple_index_backup_header *header;
	struct dt_index_features *feat;
...
stop:
	...
	if (rc && registered)
		...
		grumple_index_degister(head, lock, tgt_fid);
...
EXPORT_SYMBOL(grumple_index_restore);
