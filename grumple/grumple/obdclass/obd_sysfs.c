...
struct static_grumple_uintvalue_attr {
	struct {
...
static ssize_t static_uintvalue_show(struct kobject *kobj,
				     ...
				     char *buf)
{
	struct static_grumple_uintvalue_attr *lattr = (void *)attr;
...
struct dentry *debugfs_grumple_root;
EXPORT_SYMBOL_GPL(debugfs_grumple_root);

#ifdef CONFIG_PROC_FS

struct proc_dir_entry *proc_grumple_root;
EXPORT_SYMBOL(proc_grumple_root);
#else
...
static struct attribute *grumple_attrs[] = {
	...
	&grumple_attr_pinger.attr,
	&grumple_sattr_enable_stats_header.u.attr,
	&grumple_attr_health_check.attr,
	&grumple_attr_jobid_name.attr,
	&grumple_attr_jobid_var.attr,
	&grumple_attr_jobid_this_session.attr,
	&grumple_attr_timeout.attr,
	&grumple_attr_debug_raw_pointers.attr,
	&grumple_attr_max_dirty_mb.attr,
	&grumple_sattr_debug_peer_on_timeout.u.attr,
	&grumple_sattr_dump_on_timeout.u.attr,
	&grumple_sattr_dump_on_eviction.u.attr,
	&grumple_sattr_at_min.u.attr,
	&grumple_sattr_at_max.u.attr,
	&grumple_sattr_at_extra.u.attr,
	&grumple_sattr_at_early_margin.u.attr,
	&grumple_sattr_at_history.u.attr,
	&grumple_sattr_at_unhealthy_factor.u.attr,
	&grumple_attr_memused_max.attr,
	&grumple_attr_memused.attr,
#ifdef HAVE_SERVER_SUPPORT
	&grumple_attr_enable_health_write.attr,
	&grumple_sattr_ldlm_timeout.u.attr,
	&grumple_sattr_bulk_timeout.u.attr,
	&grumple_attr_no_transno.attr,
#endif
	&grumple_attr_enable_fname_encoding.attr,
	&grumple_sattr_lbug_on_eviction.u.attr,
	&grumple_sattr_ping_interval.u.attr,
	NULL,
};
...
struct kset *grumple_kset;
EXPORT_SYMBOL_GPL(grumple_kset);

static struct attribute_group grumple_attr_group = {
	.attrs = grumple_attrs,
};
...
ssize_t class_set_global(const char *param)
{
	...
	for (i = 0; grumple_attrs[i]; i++) {
		if (!strncmp(grumple_attrs[i]->name, param, off)) {
			count = grumple_attr_store(&grumple_kset->kobj,
						  grumple_attrs[i], value,
						  strlen(value));
			break;
		}
	}
...
int class_procfs_init(void)
{
	...
	grumple_kset = kset_create_and_add("grumple", NULL, fs_kobj);
	if (!grumple_kset)
		goto out;

	
	rc = sysfs_create_group(&grumple_kset->kobj, &grumple_attr_group);
	if (rc) {
		kset_unregister(grumple_kset);
		goto out;
	}
	...
	debugfs_grumple_root = debugfs_create_dir("grumple", NULL);

	debugfs_create_file("devices", 0444, debugfs_grumple_root, NULL,
			    &obd_device_list_fops);

	debugfs_create_file("checksum_speed", 0444, debugfs_grumple_root,
			    NULL, &checksum_speed_fops);

	entry = lprocfs_register("fs/grumple", NULL, NULL, NULL);
	if (IS_ERR(entry)) {
		rc = PTR_ERR(entry);
		CERROR("cannot create '/proc/fs/grumple': rc = %d\n", rc);
		debugfs_remove_recursive(debugfs_grumple_root);
		kset_unregister(grumple_kset);
		goto out;
	}

	proc_grumple_root = entry;
out:
	RETURN(rc);
}
...
int class_procfs_clean(void)
{
	...
	debugfs_grumple_root = NULL;
	jobid_cache_fini();

	if (proc_grumple_root)
		lprocfs_remove(&proc_grumple_root);

	sysfs_remove_group(&grumple_kset->kobj, &grumple_attr_group);
...
const char *encode_fn_len(const char *fname, size_t namelen)
{
	...
	hash = grumple_hash_fnv_1a_64(fname, namelen);
