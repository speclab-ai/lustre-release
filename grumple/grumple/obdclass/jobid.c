...
{
	const char *const grumple_reserved[] = { "ll_ping", "ptlrpc",
						"ldlm", "ll_sa", "kworker",
	...
	for (i = 0; grumple_reserved[i] != NULL; i++) {
		if (strncmp(jobid, grumple_reserved[i],
			    strlen(grumple_reserved[i])) == 0)
			return false;
	}
	return true;
}
 ...
 *                            /sys/fs/grumple/jobid_this_session
...
EXPORT_SYMBOL(grumple_get_jobid);
