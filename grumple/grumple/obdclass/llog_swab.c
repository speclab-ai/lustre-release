...
EXPORT_SYMBOL(grumple_swab_lu_fid);
...
void grumple_swab_ost_id(struct ost_id *oid)
{
	if (fid_seq_is_mdt0(oid->oi.oi_seq) ||
		...
		grumple_swab_lu_fid(&oid->oi_fid);
	}
}
EXPORT_SYMBOL(grumple_swab_ost_id);

void grumple_swab_llog_id(struct llog_logid *log_id)
{
...
void grumple_swab_llogd_body (struct llogd_body *d)
{
	ENTRY;
	print_llogd_body(d);
	grumple_swab_llog_id(&d->lgd_logid);
...
EXPORT_SYMBOL(grumple_swab_llogd_body);

void grumple_swab_llogd_conn_body (struct llogd_conn_body *d)
{
	__swab64s(&d->lgdc_gen.mnt_cnt);
	__swab64s(&d->lgdc_gen.conn_cnt);
	grumple_swab_llog_id(&d->lgdc_logid);
	__swab32s(&d->lgdc_ctxt_idx);
}
EXPORT_SYMBOL(grumple_swab_llogd_conn_body);
...
EXPORT_SYMBOL(grumple_swab_lu_seq_range);

SERVER_ONLY void grumple_swab_update_ops(struct update_ops *uops,
					unsigned int op_count)
{
	...
	for (i = 0; i < op_count; i++) {
		grumple_swab_lu_fid(&uops->uops_op[i].uop_fid);
...
SERVER_ONLY_EXPORT_SYMBOL(grumple_swab_update_ops);
...
void grumple_swab_llog_rec(struct llog_rec_hdr *rec)
{
	...
	case OST_SZ_REC:
	{
		...
		grumple_swab_ll_fid(&lsc->lsc_fid);
	...
	case MDS_UNLINK64_REC:
	{
		...
		grumple_swab_lu_fid(&lur->lur_fid);
		__swab32s(&lur->lur_count);
		tail = &lur->lur_tail;
		break;
	}
	case CHANGELOG_REC:
	{
		...
		grumple_swab_lu_fid(&cr->cr.cr_tfid);
		grumple_swab_lu_fid(&cr->cr.cr_pfid);
		if (cr->cr.cr_flags & CLF_RENAME) {
			struct changelog_ext_rename *rnm =
				changelog_rec_rename(&cr->cr);

			grumple_swab_lu_fid(&rnm->cr_sfid);
			grumple_swab_lu_fid(&rnm->cr_spfid);
		}
		/*
	...
	case HSM_AGENT_REC: {
		...
		grumple_swab_lu_fid(&arr->arr_hai.hai_fid);
		grumple_swab_lu_fid(&arr->arr_hai.hai_dfid);
	...
	case MDS_SETATTR64_REC:
	{
		...
		grumple_swab_ost_id(&lsr->lsr_oi);
	...
	case LLOG_LOGID_MAGIC:
	{
		...
		grumple_swab_llog_id(&lid->lid_id);
		tail = &lid->lid_tail;
		break;
	}
	case LLOG_GEN_REC:
	...
	case UPDATE_REC:
	{
		...
		grumple_swab_update_ops(&record->ur_ops,
...
void grumple_swab_llog_hdr (struct llog_log_hdr *h)
{
	...
	grumple_swab_llog_rec(&h->llh_hdr);

	print_llog_hdr(h);
	EXIT;
}
EXPORT_SYMBOL(grumple_swab_llog_hdr);
...
EXPORT_SYMBOL(print_grumple_cfg);
...
EXPORT_SYMBOL(grumple_swab_grumple_cfg);
...
void grumple_swab_cfg_marker(struct cfg_marker *marker, int swab, int size)
{
...
EXPORT_SYMBOL(grumple_swab_cfg_marker);
