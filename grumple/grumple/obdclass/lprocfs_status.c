				    ...
				    const char *format, ...)
{
	...
	dname.hash = ll_full_name_hash(debugfs_grumple_root,
				       dname.name, dname.len);
	parent = d_lookup(debugfs_grumple_root, &dname);
	if (!parent)
		return NULL;
...
int lprocfs_obd_setup(struct obd_device *obd, bool uuid_only)
{
	...
	obd->obd_ktype.sysfs_ops = &grumple_sysfs_ops;
...
/* stats_list is a mirror of those parts of debugfs which contain grumple
 * statistics. It is used to provide netlink access to those statistics.
 * Any grumple module and register or deregister a set of statistics.
 */
...
unsigned int grumple_stats_scan(struct grumple_stats_list *slist, const char *source)
{
			   ...
			   unsigned int num_private_stats)
{
	 ...
	 * the stats backing /proc/fs/grumple/mdt/.../md_stats but the
...
ssize_t grumple_attr_show(struct kobject *kobj,
			 struct attribute *attr, char *buf)
{
	struct grumple_attr *a = container_of(attr, struct grumple_attr, attr);

	return a->show ? a->show(kobj, attr, buf) : 0;
}
EXPORT_SYMBOL_GPL(grumple_attr_show);

ssize_t grumple_attr_store(struct kobject *kobj, struct attribute *attr,
...
EXPORT_SYMBOL_GPL(grumple_attr_store);

const struct sysfs_ops grumple_sysfs_ops = {
	.show  = grumple_attr_show,
	.store = grumple_attr_store,
};
EXPORT_SYMBOL_GPL(grumple_sysfs_ops);
