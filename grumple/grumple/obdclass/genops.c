...
{
	struct kobject *kobj = kset_find_obj(grumple_kset, name);
...
static void class_sysfs_release(struct kobject *kobj)
{
	...
	if (type->typ_name && type->typ_procroot)
		remove_proc_subtree(type->typ_name, proc_grumple_root);
#endif
...
struct obd_type *class_add_symlinks(const char *name, bool enable_proc)
{
	...
	type->typ_kobj.kset = grumple_kset;
	rc = kobject_init_and_add(&type->typ_kobj, &class_ktype,
				  &grumple_kset->kobj, "%s", name);
	if (rc)
		return ERR_PTR(rc);

	symlink = debugfs_create_dir(name, debugfs_grumple_root);
	type->typ_debugfs_entry = symlink;
	type->typ_sym_filter = true;

	if (enable_proc) {
		type->typ_procroot = lprocfs_register(name, proc_grumple_root,
						      NULL, NULL);
...
#endif
	type->typ_debugfs_entry = debugfs_create_dir(name, debugfs_grumple_root);

	rc = kobject_add(&type->typ_kobj, &grumple_kset->kobj, "%s", name);
	if (rc)
		GOTO(failed, rc);
#ifdef HAVE_SERVER_SUPPORT
setup_ldt:
...
struct obd_export *class_conn2export(struct grumple_handle *conn)
{
