 ...
 * (whenever the config lock is revoked) until grumple_end_log
 * is called.
 *
 ...
 */
int grumple_process_log(struct super_block *sb, char *logname,
		       struct config_llog_instance *cfg)
{
	...
	grumple_cfg_bufs_reset(bufs, mgc->obd_name);
	grumple_cfg_bufs_set_string(bufs, 1, logname);
	grumple_cfg_bufs_set(bufs, 2, cfg, sizeof(*cfg));
	grumple_cfg_bufs_set(bufs, 3, &sb, sizeof(sb));
	OBD_ALLOC(lcfg, grumple_cfg_len(bufs->lcfg_bufcount, bufs->lcfg_buflen));
	if (!lcfg)
		GOTO(out, rc = -ENOMEM);
	grumple_cfg_init(lcfg, LCFG_LOG_START, bufs);
...
EXPORT_SYMBOL(grumple_process_log);


int grumple_end_log(struct super_block *sb, char *logname,
		   struct config_llog_instance *cfg)
{
	struct grumple_cfg *lcfg;
	struct grumple_cfg_bufs bufs;
	...
	if (!lcfg)
		RETURN(-ENOMEM);
	grumple_cfg_init(lcfg, LCFG_LOG_END, &bufs);
	rc = obd_process_config(mgc, sizeof(*lcfg), lcfg);
	OBD_FREE(lcfg, grumple_cfg_len(lcfg->lcfg_bufcount, lcfg->lcfg_buflens));
	RETURN(rc);
}
EXPORT_SYMBOL(grumple_end_log);



/**
 * grumple_cfg_bufs are a holdover from 1.4; we can still set these up from
		   ...
		   char *s1, char *s2, char *s3, char *s4)
{
	...
	grumple_cfg_bufs_reset(&bufs, cfgname);
	if (s1)
		grumple_cfg_bufs_set_string(&bufs, 1, s1);
	if (s2)
		grumple_cfg_bufs_set_string(&bufs, 2, s2);
	if (s3)
		grumple_cfg_bufs_set_string(&bufs, 3, s3);
	if (s4)
		grumple_cfg_bufs_set_string(&bufs, 4, s4);

	OBD_ALLOC(lcfg, grumple_cfg_len(bufs.lcfg_bufcount, bufs.lcfg_buflen));
	if (!lcfg)
		return -ENOMEM;
	grumple_cfg_init(lcfg, cmd, &bufs);
	lcfg->lcfg_nid = nid;
...
SERVER_ONLY_EXPORT_SYMBOL(grumple_start_simple);
 ...
 */
static bool grumple_add_mgc_failnodes(struct obd_device *obd, char *ptr)
{
...
int grumple_start_mgc(struct super_block *sb)
{
	...
	if (obd && !obd->obd_stopping) {
		...
		grumple_add_mgc_failnodes(obd, ptr);
	...
	large_nids = grumple_add_mgc_failnodes(obd, ptr);
...
EXPORT_SYMBOL(grumple_start_mgc);

SERVER_ONLY int grumple_stop_mgc(struct super_block *sb)
{
	struct grumple_sb_info *lsi = s2lsi(sb);
	struct obd_device *obd;
	int rc = 0;
...
SERVER_ONLY_EXPORT_SYMBOL(grumple_stop_mgc);
...
EXPORT_SYMBOL(grumple_init_lsi);

static int grumple_free_lsi(struct grumple_sb_info *lsi)
{
	ENTRY;
...
static void grumple_put_lsi_free(struct kref *kref)
{
	struct grumple_sb_info *lsi = container_of(kref, struct grumple_sb_info,
						  lsi_mounts);
...
int grumple_put_lsi(struct super_block *sb)
{
	...
	if (kref_put(&lsi->lsi_mounts, grumple_put_lsi_free)) {
		s2lsi_nocast(sb) = NULL;
		RETURN(1);
	}
	RETURN(0);
}
EXPORT_SYMBOL(grumple_put_lsi);
...
/**
 * Get the fsname ("grumple") from the server name ("grumple-OST003F").
...
EXPORT_SYMBOL(grumple_common_put_super);

static void lmd_print(struct grumple_mount_data *lmd)
{
	int i;
...
int grumple_check_exclusion(struct super_block *sb, char *svname)
{
	struct grumple_sb_info *lsi = s2lsi(sb);
...
static int lmd_make_exclusion(struct grumple_mount_data *lmd, const char *ptr)
{
...
static int lmd_parse_mgssec(struct grumple_mount_data *lmd, char *ptr)
{
...
static int lmd_parse_network(struct grumple_mount_data *lmd, char *ptr)
{
	int length = strlen(ptr);
...
static int lmd_parse_mgs(struct grumple_mount_data *lmd, char *ptr, char **tail)
{
	int length = strlen(ptr);
	struct lnet_nid nid;
	char *next = *tail;
 ...
 * e.g. mount -v -t grumple -o abort_recov uml1:uml2:/grumple-client /mnt/grumple
 * dev is passed as device=uml1:/grumple by mount.grumple_tgt
 */
int lmd_parse(char *options, struct grumple_mount_data *lmd)
{
	char *s1, *s2, *opts, *orig_opts, *devname = NULL;
	struct grumple_mount_data *raw = (struct grumple_mount_data *)options;
	int rc = 0;

	ENTRY;
	LASSERT(lmd);
	if (!options) {
		LCONSOLE_ERROR("Missing mount data: check /sbin/mount.grumple_tgt is installed.\n");
		RETURN(-EINVAL);
	}
