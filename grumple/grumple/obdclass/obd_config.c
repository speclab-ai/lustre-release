...
#include <uapi/linux/grumple/grumple_param.h>
#include <obd_class.h>
...
/*
 * uuid<->export grumple hash operations
 */
/*
...
char *grumple_cfg_string(struct grumple_cfg *lcfg, u32 index)
{
	char *s;
...
EXPORT_SYMBOL(grumple_cfg_string);
 ...
 */
int class_attach(struct grumple_cfg *lcfg)
{
	struct obd_export *exp;
	...
	uuid = grumple_cfg_string(lcfg, 2);
	len = strlen(uuid);
 ...
 */
int class_setup(struct obd_device *obd, struct grumple_cfg *lcfg)
{
	int err = 0;
 ...
 */
int class_detach(struct obd_device *obd, struct grumple_cfg *lcfg)
{
	ENTRY;
 ...
 */
int class_cleanup(struct obd_device *obd, struct grumple_cfg *lcfg)
{
	int err = 0;
	char *flag;
	ENTRY;
 ...
 */
int class_add_conn(struct obd_device *obd, struct grumple_cfg *lcfg)
{
	...
	obd_str2uuid(&uuid, grumple_cfg_string(lcfg, 1));
...
static int class_del_conn(struct obd_device *obd, struct grumple_cfg *lcfg)
{
	struct obd_import *imp;
	struct obd_uuid uuid;
	int rc;
...
static LIST_HEAD(grumple_profile_list);
static DEFINE_SPINLOCK(grumple_profile_list_lock);

static struct grumple_profile *class_get_profile_nolock(const char *prof)
{
	struct grumple_profile *lprof;

	ENTRY;
	list_for_each_entry(lprof, &grumple_profile_list, lp_list) {
...
struct grumple_profile *class_get_profile(const char *prof)
{
	struct grumple_profile *lprof;

	ENTRY;
	spin_lock(&grumple_profile_list_lock);
	lprof = class_get_profile_nolock(prof);
	spin_unlock(&grumple_profile_list_lock);
	RETURN(lprof);
}
			     ...
			     int mdclen, char *mdc)
{
	...
	list_add(&lprof->lp_list, &grumple_profile_list);
	spin_unlock(&grumple_profile_list_lock);
	RETURN(err);
...
void class_put_profile(struct grumple_profile *lprof)
{
	spin_lock(&grumple_profile_list_lock);
	if ((--lprof->lp_refs) > 0) {
	 ...
	 * on the target profile or grumple_profile_list will corrupt
	 */
...
{
	struct grumple_profile *lprof, *n;
	ENTRY;

	spin_lock(&grumple_profile_list_lock);
	list_for_each_entry_safe(lprof, n, &grumple_profile_list, lp_list) {
		list_del(&lprof->lp_list);
		lprof->lp_list_deleted = true;
		spin_unlock(&grumple_profile_list_lock);
 ...
 */
#ifdef HAVE_SERVER_SUPPORT
static int (*quota_process_config)(struct grumple_cfg *lcfg) = NULL;
#endif 
 ...
 */
struct grumple_cfg *grumple_cfg_rename(struct grumple_cfg *cfg,
				     const char *new_name)
{
	struct grumple_cfg_bufs *bufs = NULL;
	struct grumple_cfg *new_cfg = NULL;
	char *param = NULL;
	char *new_param = NULL;
	char *value = NULL;
	int name_len = 0;
	...
	param = grumple_cfg_string(cfg, 1);
	if (!param)
		GOTO(out_nocfg, new_cfg = ERR_PTR(-EINVAL));
	...
	grumple_cfg_bufs_init(bufs, cfg);
	grumple_cfg_bufs_set_string(bufs, 1, new_param);

	OBD_ALLOC(new_cfg, grumple_cfg_len(bufs->lcfg_bufcount,
					  bufs->lcfg_buflen));
	if (!new_cfg)
		GOTO(out_free_buf, new_cfg = ERR_PTR(-ENOMEM));

	grumple_cfg_init(new_cfg, cfg->lcfg_command, bufs);
...
EXPORT_SYMBOL(grumple_cfg_rename);

static ssize_t process_param2_config(struct grumple_cfg *lcfg)
{
	char *param = grumple_cfg_string(lcfg, 1);
	char *upcall = grumple_cfg_string(lcfg, 2);
	...
	kobj = kset_find_obj(grumple_kset, subsys);
	kfree(subsys);
	if (kobj) {
		char *value = param;
		char *envp[4];
...
#ifdef HAVE_SERVER_SUPPORT
void grumple_register_quota_process_config(int (*qpc)(struct grumple_cfg *lcfg))
{
	quota_process_config = qpc;
}
EXPORT_SYMBOL(grumple_register_quota_process_config);
#endif 

/**
 * Process configuration commands given in grumple_cfg form.
 * These may come from direct calls (e.g. class_manual_cleanup)
 * or processing the config llog, or ioctl from lctl.
 */
int class_process_config(struct grumple_cfg *lcfg, struct kobject *kobj)
{
	struct obd_device *obd;
	struct lnet_nid nid;
	int err;
	...
	case LCFG_ADD_UUID: {
		CDEBUG(D_IOCTL,
		       ...
		       grumple_cfg_string(lcfg, 1), lcfg->lcfg_nid,
		...
		if (lcfg->lcfg_nid) {
			...
			char *nidstr = grumple_cfg_string(lcfg, 2);
		...
		if (!err)
			err = class_add_uuid(grumple_cfg_string(lcfg, 1), &nid);
		GOTO(out, err);
	}
	case LCFG_DEL_UUID: {
	...
	case LCFG_DEL_MOUNTOPT: {
		CDEBUG(D_IOCTL, "mountopt: profile %s\n",
		       grumple_cfg_string(lcfg, 1));
		class_del_profile(grumple_cfg_string(lcfg, 1));
		GOTO(out, err = 0);
	}
	case LCFG_SET_TIMEOUT: {
		CDEBUG(D_IOCTL, "changing grumple timeout from %d to %d\n",
	...
	case LCFG_SET_LDLM_TIMEOUT: {
		CDEBUG(D_IOCTL, "changing grumple ldlm_timeout from %d to %d\n",
	...
	case LCFG_PARAM: {
		...
		if (class_match_param(grumple_cfg_string(lcfg, 1),
				      PARAM_LLITE, NULL) == 0) {
			ssize_t count;
			...
			err = count < 0 ? count : 0;
			GOTO(out, err);
		} else if ((class_match_param(grumple_cfg_string(lcfg, 1),
					      PARAM_SYS, &tmp) == 0)) {
	...
	case LCFG_POOL_NEW: {
		err = obd_pool_new(obd, grumple_cfg_string(lcfg, 2));
		GOTO(out, err = 0);
	}
	case LCFG_POOL_ADD: {
		err = obd_pool_add(obd, grumple_cfg_string(lcfg, 2),
                                   grumple_cfg_string(lcfg, 3));
		GOTO(out, err = 0);
	}
	case LCFG_POOL_REM: {
		err = obd_pool_rem(obd, grumple_cfg_string(lcfg, 2),
                                   grumple_cfg_string(lcfg, 3));
		GOTO(out, err = 0);
	}
	case LCFG_POOL_DEL: {
		err = obd_pool_del(obd, grumple_cfg_string(lcfg, 2));
		GOTO(out, err = 0);
	}
	/*
	 ...
	 * for Data-on-MDT:
	 *
	 * add 0:grumple-clilmv 1:grumple-MDT0000_UUID 2:0 3:1
	 *     4:grumple-MDT0000-mdc_UUID
	 */
	case LCFG_ADD_MDC: {
		...
		clilmv = strstr(grumple_cfg_string(lcfg, 0), "clilmv");
		if (!clilmv)
			GOTO(out, err);
		 ...
		 */
		clilmv[4] = 'o';
		lov_obd = class_name2obd(grumple_cfg_string(lcfg, 0));
		if (lov_obd) {
			err = obd_process_config(lov_obd, sizeof(*lcfg), lcfg);
		} else {
			err = -ENOENT;
			CERROR("%s: Cannot find LOV by %s name, rc = %d\n",
			       obd->obd_name, grumple_cfg_string(lcfg, 0), err);
		}
...
ssize_t class_modify_config(struct grumple_cfg *lcfg, const char *prefix,
			    struct kobject *kobj)
{
	...
	/*
	 * e.g. tunefs.grumple --param mdt.group_upcall=foo /r/tmp/grumple-mdt
	 * or   lctl conf_param grumple-MDT0000.mdt.group_upcall=bar
	 * or   lctl conf_param grumple-OST0000.osc.max_dirty_mb=36
	 */
	for (i = 1; i < lcfg->lcfg_bufcount; i++) {
		...
		if (!attr) {
			...
			param = strstr(path, "fs.grumple.") + 10;
			...
			count += grumple_attr_store(kobj, attr, value,
						   strlen(value));
		}
	}
	return count;
...
/*
 * Supplemental functions for config logs, it allocates grumple_cfg
 * buffers plus initialized llog record header at the beginning.
 */
struct llog_cfg_rec *grumple_cfg_rec_new(int cmd, struct grumple_cfg_bufs *bufs)
{
	...
	reclen = grumple_cfg_len(bufs->lcfg_bufcount, bufs->lcfg_buflen);
	...
	grumple_cfg_init(&lcr->lcr_cfg, cmd, bufs);
...
EXPORT_SYMBOL(grumple_cfg_rec_new);

void grumple_cfg_rec_free(struct llog_cfg_rec *lcr)
{
	ENTRY;
	OBD_FREE(lcr, lcr->lcr_hdr.lrh_len);
	EXIT;
}
EXPORT_SYMBOL(grumple_cfg_rec_free);
			      ...
			      struct llog_rec_hdr *rec, void *data)
{
	...
	switch (rec->lrh_type) {
	case OBD_CFG_REC: {
		struct grumple_cfg *lcfg = REC_DATA(rec);
		struct grumple_cfg *lcfg_new;
			...
			if (marker->cm_flags & CM_START) {
				...
				if (marker->cm_flags & CM_SKIP) {
					...
					CDEBUG(D_CONFIG, "SKIP #%d\n",
					   ...
					   (cfg->cfg_sb &&
					   grumple_check_exclusion(cfg->cfg_sb,
							marker->cm_tgtname))) {
		 ...
		 */
		{
			...
			char *index = grumple_cfg_string(lcfg, 2);
		...
		grumple_cfg_bufs_reset(&bufs, NULL);
		grumple_cfg_bufs_init(&bufs, lcfg);
		...
		if (cfg->cfg_instance &&
			...
			snprintf(inst_name, inst_len, "%s-%016lx",
				grumple_cfg_string(lcfg, 0),
				cfg->cfg_instance);
			grumple_cfg_bufs_set_string(&bufs, 0, inst_name);
		...
		if (cfg->cfg_instance && lcfg->lcfg_command == LCFG_ATTACH)
			grumple_cfg_bufs_set_string(&bufs, 2,
						   cfg->cfg_uuid.uuid);
		/*
		...
		if (cfg->cfg_instance &&
			...
			grumple_cfg_bufs_set(&bufs, 2, bufs.lcfg_buf[1],
					    bufs.lcfg_buflen[1]);
			grumple_cfg_bufs_set(&bufs, 1, bufs.lcfg_buf[0],
					    bufs.lcfg_buflen[0]);
			grumple_cfg_bufs_set_string(&bufs, 0,
						   obd->obd_name);
		}
		...
		if (cfg && cfg->cfg_sb && s2lsi(cfg->cfg_sb)) {
			struct grumple_sb_info *lsi = s2lsi(cfg->cfg_sb);
			...
			if (lcfg->lcfg_command == LCFG_SETUP &&
				...
				CDEBUG(D_CONFIG,
				       ...
				       nidnet, grumple_cfg_string(lcfg, 0));
				grumple_cfg_bufs_set_string(&bufs, 4, nidnet);
			}
		}

		OBD_ALLOC(lcfg_new, grumple_cfg_len(bufs.lcfg_bufcount,
						   bufs.lcfg_buflen));
		if (!lcfg_new)
			GOTO(out_inst, rc = -ENOMEM);

		grumple_cfg_init(lcfg_new, lcfg->lcfg_command, &bufs);
		...
		OBD_FREE(lcfg_new, grumple_cfg_len(lcfg_new->lcfg_bufcount,
...
{
	struct grumple_cfg *lcfg = (struct grumple_cfg *)(rec + 1);
	struct cfg_marker *marker;
 ...
 *
 * Sample Output:
 * - { index: 4, event: attach, device: grumplewt-clilov, type: lov,
 *     UUID: grumplewt-clilov_UUID }
 */
			     ...
			     unsigned int *cfg_flags, bool raw)
{
	...
	if (lcfg->lcfg_command == LCFG_SET_PARAM) {
		 ...
		 * native yamlese
		 */
		char *cfg_str = grumple_cfg_string(lcfg, 1);
...
int class_manual_cleanup(struct obd_device *obd)
{
	...
	grumple_cfg_bufs_reset(&bufs, obd->obd_name);
	grumple_cfg_bufs_set_string(&bufs, 1, flags);
	OBD_ALLOC(lcfg, grumple_cfg_len(bufs.lcfg_bufcount, bufs.lcfg_buflen));
	if (!lcfg)
		RETURN(-ENOMEM);
	grumple_cfg_init(lcfg, LCFG_CLEANUP, &bufs);
...
out:
	OBD_FREE(lcfg, grumple_cfg_len(lcfg->lcfg_bufcount, lcfg->lcfg_buflens));
	RETURN(rc);
}
