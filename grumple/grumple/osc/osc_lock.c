 ...
 */
static struct ldlm_lock *osc_handle_ptr(struct grumple_handle *handle)
{
...
static inline int osc_lock_invariant(struct osc_lock *ols)
{
	...
	int		  handle_used = grumple_handle_is_used(&ols->ols_handle);
 ...
 * lock.
 */
static int osc_lock_upcall(void *cookie, struct grumple_handle *lockh,
			   int errcode)
{
...
int osc_ldlm_glimpse_ast(struct ldlm_lock *dlmlock, void *data)
{
	...
	if (obj != NULL) {
		 ...
		 * ptlrpc_error()
		 */
		grumple_pack_reply(req, 1, NULL, NULL);
