...
#include <grumple_obdo.h>
#include <grumple_osc.h>
...
struct ost_body *osc_pack_req_body(struct ptlrpc_request *req, struct obdo *oa)
{
	...
	grumple_set_wire_obdo(&req->rq_import->imp_connect_data, &body->oa, oa);
	grumple_msg_set_projid(req->rq_reqmsg, oa->o_projid);
		       ...
		       struct obdo *oa)
{
	...
	grumple_get_wire_obdo(&req->rq_import->imp_connect_data, oa, &body->oa);
				 ...
				 struct ptlrpc_request *req, void *args, int rc)
{
	...
	grumple_get_wire_obdo(&req->rq_import->imp_connect_data, sa->sa_oa,
			     &body->oa);
out:
 ...
 * beyond the end of a stripe file; i.e. grumple is reading a sparse file
...
static inline int can_merge_pages(struct brw_page *p1, struct brw_page *p2)
{
	...
	if (f1 != f2) {
		...
		if (unlikely((p1->bp_flag & mask) != (p2->bp_flag & mask))) {
			CWARN("Saw flags 0x%x and 0x%x in the same brw, please report this at https:
...
no_bulk:
	...
	if (opc == OST_WRITE) {
		 ...
		 * grumple_set_wire_obdo(), and in the case a bulk-read is being
...
static int osc_brw_fini_request(struct ptlrpc_request *req, int rc)
{
	...
	if (grumple_msg_get_opc(req->rq_reqmsg) == OST_WRITE &&
...
out:
	if (rc >= 0)
		grumple_get_wire_obdo(&req->rq_import->imp_connect_data,
				     aa->aa_oa, &body->oa);
				...
				struct osc_brw_async_args *aa, int rc)
{
	...
	rc = osc_brw_prep_request(grumple_msg_get_opc(request->rq_reqmsg) ==
			 ...
			 struct ptlrpc_request *req, void *args, int rc)
{
	...
	if (grumple_msg_get_opc(req->rq_reqmsg) == OST_WRITE && rc == 0) {
	...
	if (aa->aa_request) {
		...
		ptlrpc_req_put(req);
		if (xid && grumple_msg_get_opc(req->rq_reqmsg) == OST_WRITE) {
			spin_lock(&cli->cl_loi_list_lock);
		  ...
		  struct list_head *ext_list, int cmd)
{
	...
	if (ndelay) {
		 ...
		 * grumple_msg_set_timeout(req, req->rq_timeout / 2);
		 */
	}
	...
	cl_req_attr_set(env, osc2cl(obj), crattr);
	grumple_msg_set_jobinfo(req->rq_reqmsg, &crattr->cra_jobinfo);
	grumple_msg_set_projid(req->rq_reqmsg, body->oa.o_projid);
...
static int osc_enqueue_fini(struct ptlrpc_request *req,
			    ...
			    void *cookie, struct grumple_handle *lockh,
			    ...
			    bool speculative, int errcode)
{
	...
	if (errcode == ELDLM_LOCK_MATCHED)
		errcode = ELDLM_OK;
	if (errcode == ELDLM_OK && grumple_handle_is_used(lockh))
				 ...
				 void *args, int rc)
{
	...
	struct grumple_handle *lockh = &aa->oa_lockh;
		     ...
		     bool speculative, __u32 projid)
{
	...
	*flags &= ~LDLM_FL_BLOCK_GRANTED;
	...
	grumple_msg_set_projid(req->rq_reqmsg, projid);
	...
	if (async) {
		...
		grumple_handle_copy(&aa->oa_lockh, &lockh);
		aa->oa_upcall	   = upcall;
		aa->oa_cookie	   = cookie;
...
int osc_setup_common(struct obd_device *obd, struct grumple_cfg *lcfg)
{
	struct client_obd *cli = &obd->u.cli;
	int rc;
...
int osc_setup(struct obd_device *obd, struct grumple_cfg *lcfg)
{
	struct client_obd *cli = &obd->u.cli;
	int adding;
	int added;
