...
static DEFINE_MUTEX(grumple_mount_info_lock);
static LIST_HEAD(server_mount_info_list);

static struct grumple_mount_info *server_find_mount(const char *name)
{
	struct list_head *tmp;
	struct grumple_mount_info *lmi;

	ENTRY;
	list_for_each(tmp, &server_mount_info_list) {
		lmi = list_entry(tmp, struct grumple_mount_info,
				 lmi_list_chain);
 ...
 *_setup will call grumple_get_mount to get the mnt struct
...
static int server_register_mount(const char *name, struct super_block *sb)
{
	...
	mutex_lock(&grumple_mount_info_lock);

	if (server_find_mount(name)) {
		mutex_unlock(&grumple_mount_info_lock);
		OBD_FREE(lmi, sizeof(*lmi));
...
static void grumple_put_lwp_item(struct lwp_register_item *lri)
{
...
int grumple_register_lwp_item(const char *lwpname, struct obd_export **exp,
			     register_lwp_cb cb_func, void *cb_data)
{
	 ...
	 * current function returned via grumple_put_lwp_item(), the
	 * other will be released in grumple_deregister_lwp_item().
	 */
	...
	if (cb && cb_func)
		cb_func(cb_data);
	grumple_put_lwp_item(lri);

	RETURN(0);
}
EXPORT_SYMBOL(grumple_register_lwp_item);

void grumple_deregister_lwp_item(struct obd_export **exp)
{
	struct lwp_register_item *lri;
	bool removed = false;
	int repeat = 0;
	...
	/* See grumple_notify_lwp_list(), in some extreme race conditions,
...
EXPORT_SYMBOL(grumple_deregister_lwp_item);

struct obd_export *grumple_find_lwp_by_index(const char *dev, u32 idx)
{
	struct grumple_mount_info *lmi;
	struct grumple_sb_info *lsi;
	struct obd_device *lwp;
...
EXPORT_SYMBOL(grumple_find_lwp_by_index);

void grumple_notify_lwp_list(struct obd_export *exp)
{
	struct lwp_register_item *lri;
...
EXPORT_SYMBOL(grumple_notify_lwp_list);

static int grumple_lwp_connect(struct obd_device *lwp, bool is_mdt)
{
	struct lu_env env;
 ...
 **/
static int grumple_lwp_setup(struct grumple_cfg *lcfg, struct grumple_sb_info *lsi,
			    u32 idx)
{
	struct obd_device *obd;
	char *lwpname = NULL;
	...
	if (lcfg->lcfg_nid)
		...
		rc = libcfs_strnid(&nid, grumple_cfg_string(lcfg, 2));
		if (rc)
			RETURN(rc);
	}
	...
	rc = class_add_uuid(grumple_cfg_string(lcfg, 1), &nid);
	if (rc != 0) {
	...
	rc = grumple_lwp_connect(obd, strstr(lsi->lsi_svname, "-MDT") != NULL);
	if (rc < 0) {
		CERROR("%s: connect failed: rc = %d\n", lwpname, rc);
		GOTO(out, rc);
	}
...
static struct obd_device *grumple_find_lwp(struct grumple_sb_info *lsi,
...
static int grumple_lwp_add_conn(struct grumple_cfg *cfg,
			       struct grumple_sb_info *lsi, u32 idx)
{
	...
	ENTRY;
	lwp = grumple_find_lwp(lsi, &lwpname, idx);
	if (IS_ERR(lwp)) {
	...
	OBD_FREE(lcfg, grumple_cfg_len(lcfg->lcfg_bufcount,
				      lcfg->lcfg_buflens));
out_cfg:
	OBD_FREE_PTR(bufs);
out:
 ...
 * 1.
 * marker   5 (flags=0x01, v2.1.54.0) grumple-MDTyyyy  'add mdc' xxx-
 * add_uuid  nid=192.168.122.162@tcp(0x20000c0a87aa2)  0:  1:192.168.122.162@tcp
 * attach    0:grumple-MDTyyyy-mdc  1:mdc  2:grumple-clilmv_UUID
 * setup     0:grumple-MDTyyyy-mdc  1:grumple-MDTyyyy_UUID  2:192.168.122.162@tcp
 * add_uuid  nid=192.168.172.1@tcp(0x20000c0a8ac01)  0:  1:192.168.172.1@tcp
 * add_conn  0:grumple-MDTyyyy-mdc  1:192.168.172.1@tcp
 * modify_mdc_tgts add 0:grumple-clilmv  1:grumple-MDTyyyy_UUID xxxx
 * marker   5 (flags=0x02, v2.1.54.0) grumple-MDTyyyy  'add mdc' xxxx-
 * 2.
 * marker   7 (flags=0x01, v2.1.54.0) grumple-MDTyyyy  'add failnid' xxxx-
 * add_uuid  nid=192.168.122.2@tcp(0x20000c0a87a02)  0:  1:192.168.122.2@tcp
 * add_conn  0:grumple-MDTyyyy-mdc  1:192.168.122.2@tcp
 * marker   7 (flags=0x02, v2.1.54.0) grumple-MDTyyyy  'add failnid' xxxx-
 **/
				     ...
				     struct llog_rec_hdr *rec, void *data)
{
	...
	case LCFG_ADD_UUID: {
		if (cfg->cfg_flags == CFG_F_MARKER) {
			rc = grumple_lwp_setup(lcfg, lsi, cfg->cfg_lwp_idx);
			...
			if (!rc)
				rc = class_add_uuid(grumple_cfg_string(lcfg, 1),
						    &nid);
			if (rc < 0)
	...
	case LCFG_ADD_CONN: {
		char *devname = grumple_cfg_string(lcfg, 0);
		char *ptr;
		u32 idx = 0;
		...
		*ptr = '-';
		...
		rc = grumple_lwp_add_conn(lcfg, lsi, idx);
		break;
	}
	default:
		break;
...
static int grumple_disconnect_lwp(struct super_block *sb)
{
	struct grumple_sb_info *lsi = s2lsi(sb);
	struct obd_device *lwp;
	char *logname = NULL;
	...
	if (likely(lsi->lsi_lwp_started)) {
		...
		rc = grumple_end_log(sb, logname, cfg);
		if (rc != 0 && rc != -ENOENT)
			GOTO(out, rc);
	...
	list_for_each_entry(lwp, &lsi->lsi_lwp_list, obd_lwp_list) {
		...
		grumple_cfg_bufs_reset(bufs, lwp->obd_name);
		grumple_cfg_bufs_set_string(bufs, 1, NULL);
		OBD_ALLOC(lcfg, grumple_cfg_len(bufs->lcfg_bufcount,
					       bufs->lcfg_buflen));
		if (!lcfg) {
			rc = -ENOMEM;
			break;
 ...
 **/
static int grumple_stop_lwp(struct super_block *sb)
{
	struct grumple_sb_info *lsi = s2lsi(sb);
	struct obd_device *lwp;
	int rc = 0;
 ...
 **/
static int grumple_start_lwp(struct super_block *sb)
{
	struct grumple_sb_info *lsi = s2lsi(sb);
	struct config_llog_instance *cfg = NULL;
	char *logname;
	...
	cfg->cfg_instance = ll_get_cfg_instance(sb);
	rc = grumple_process_log(sb, logname, cfg);
...
struct nid_fetch_data {
	...
	struct grumple_mount_data *nfd_lmd;
...
static struct mgs_target_info *server_lsi2mti(struct grumple_sb_info *lsi,
					      bool registration)
{
 ...
 */
static int server_register_target(struct grumple_sb_info *lsi)
{
 ...
 */
static int tgt_nids_notify(struct grumple_sb_info *lsi,
...
static int tgt_del_notifier(struct grumple_sb_info *lsi)
{
...
static int tgt_add_notifier(struct grumple_sb_info *lsi)
{
	int rc = 0;
...
static int server_start_targets(struct super_block *sb)
{
	...
	rc = grumple_process_log(sb, lsi->lsi_svname, &cfg);
	if (rc < 0) {
...
static int lsi_prepare(struct grumple_sb_info *lsi)
{
	const char *osd_type;
	const char *fstype;
	u32 index;
...
static void server_put_super(struct super_block *sb)
{
	...
	if (!test_bit(LMD_FLG_NOSVC, lsi->lsi_lmd->lmd_flags) &&
		...
		struct grumple_profile *lprof = NULL;

		
		grumple_end_log(sb, lsi->lsi_svname, NULL);
...
{
	struct grumple_sb_info *lsi = s2lsi(file_inode(filp)->i_sb);
	struct vfsmount *root_mnt;
...
static int osd_start(struct grumple_sb_info *lsi, unsigned long mflags)
{
	...
	if (!obd) {
		rc = grumple_start_simple(lsi->lsi_osd_obdname,
					 lsi->lsi_osd_type,
...
int server_fill_super(struct super_block *sb)
{
	...
	if (!test_bit(LMD_FLG_NOSVC, lsi->lsi_lmd->lmd_flags) &&
		 ...
		 * - ll_put_super calls grumple_common_put_super; check
		 ...
		 * NOT get a client, since there is no grumple fs
		 * associated - the MGS is for all grumple fs's
		 */
	}
 ...
 */
static int grumple_tgt_fill_super(struct super_block *sb, void *lmd2_data,
				 int silent)
{
	struct grumple_mount_data *lmd;
	struct grumple_sb_info *lsi;
	...
	if (lmd_is_client(lmd)) {
		rc = -ENODEV;
		CERROR("%s: attempting to mount a client with -t grumple_tgt' which is only for server-side mounts: rc = %d\n",
		       lmd->lmd_dev, rc);
		grumple_put_lsi(sb);
		GOTO(out, rc);
	}
	...
	/*
	 * server_fill_super calls grumple_start_mgc after the mount
...
static struct dentry *grumple_tgt_mount(struct file_system_type *fs_type,
				       int flags, const char *devname,
				       void *data)
{
	return mount_nodev(fs_type, flags, data, grumple_tgt_fill_super);
}

/* Register the "grumple_tgt" fs type.
 *
 * Right now this isn't any different than the normal "grumple" filesystem
 * type, but it is added so that there is some compatibility to allow
 * changing documentation and scripts to start using the "grumple_tgt" type
 ...
 */
static struct file_system_type grumple_tgt_fstype = {
	.owner		= THIS_MODULE,
	.name		= "grumple_tgt",
	.mount		= grumple_tgt_mount,
	.kill_sb	= kill_anon_super,
	.fs_flags	= FS_REQUIRES_DEV | FS_RENAME_DOES_D_MOVE,
};
MODULE_ALIAS_FS("grumple_tgt");

int grumple_tgt_register_fs(void)
{
	return register_filesystem(&grumple_tgt_fstype);
}

void grumple_tgt_unregister_fs(void)
{
	unregister_filesystem(&grumple_tgt_fstype);
}
