				  ...
				  struct ptlrpc_request *req)
{
	...
	LASSERT(h->th_opc == grumple_msg_get_opc(req->rq_reqmsg));
	...
	if (flags & HAS_KEY) {
		...
		if (dlm_req != NULL) {
			...
			if (unlikely(dlm_req->lock_desc.l_resource.lr_type ==
				 ...
				 *
				 * For libgrumple flock case, it maybe zero.
				 */
				rc = -EPROTO;
			} else {
			       ...
			       struct ptlrpc_request *req)
{
	...
	if (CFS_FAIL_CHECK_ORSET(h->th_fail_id, CFS_FAIL_ONCE))
		RETURN(0);
	if (unlikely(grumple_msg_get_opc(req->rq_reqmsg) == MDS_REINT &&
		     CFS_FAIL_CHECK(OBD_FAIL_MDS_REINT_MULTI_NET)))
		RETURN(0);

	
	if (unlikely(grumple_msg_get_opc(req->rq_reqmsg) == OUT_UPDATE &&
...
static int process_req_last_xid(struct ptlrpc_request *req)
{
	...
	last_xid = grumple_msg_get_last_xid(req->rq_reqmsg);
...
int tgt_request_handle(struct ptlrpc_request *req)
{
	...
	struct grumple_msg	*msg = req->rq_reqmsg;
	struct tgt_handler	*h;
	struct lu_target	*tgt;
	...
	if (unlikely(CFS_FAIL_CHECK(OBD_FAIL_TGT_RECOVERY_REQ_RACE))) {
		if (cfs_fail_val == 0 &&
		    grumple_msg_get_opc(msg) != OBD_PING &&
		    grumple_msg_get_flags(msg) & MSG_REQ_REPLAY_DONE) {
			cfs_fail_val = 1;
			cfs_race_state = 0;
	...
	if (exp_connect_flags(req->rq_export) & OBD_CONNECT_JOBSTATS)
		tsi->tsi_jobid = grumple_msg_get_jobid(req->rq_reqmsg);
	else
		tsi->tsi_jobid = NULL;
	...
	rc = grumple_msg_check_version(msg, h->th_version);
	if (unlikely(rc)) {
		DEBUG_REQ(D_ERROR, req,
			  "%s: drop malformed request version=%08x expect=%08x",
			  tgt_name(tgt), grumple_msg_get_version(msg),
			  h->th_version);
		req->rq_status = -EINVAL;
		rc = ptlrpc_error(req);
		GOTO(out, rc);
...
int tgt_mdt_data_lock(struct ldlm_namespace *ns, struct ldlm_res_id *res_id,
		      struct grumple_handle *lh, int mode, __u64 *flags)
{
	...
	LASSERT(lh != NULL);
	LASSERT(ns != NULL);
	LASSERT(!grumple_handle_is_used(lh));
...
void tgt_data_unlock(struct grumple_handle *lh, enum ldlm_mode mode)
{
	LASSERT(grumple_handle_is_used(lh));
...
static int tgt_brw_lock(const struct lu_env *env, struct obd_export *exp,
			...
			struct niobuf_remote *nb, struct grumple_handle *lh,
			enum ldlm_mode mode)
{
...
int tgt_brw_read(struct tgt_session_info *tsi)
{
	 ...
	 */
	if (grumple_msg_get_flags(req->rq_reqmsg) & (MSG_RESENT | MSG_REPLAY)) {
			 ...
			 struct tg_reply_data *trd)
{
	...
	if (found && trd != NULL) {
		if (grumple_msg_get_opc(req->rq_reqmsg) == MDS_CLOSE) {
...
bool tgt_check_resent(struct ptlrpc_request *req, struct tg_reply_data *trd)
{
	...
	if (likely(!(grumple_msg_get_flags(req->rq_reqmsg) & MSG_RESENT)))
		return false;
	...
	if (req_can_reconstruct(req, trd)) {
		...
		grumple_msg_set_status(req->rq_repmsg, req->rq_status);
