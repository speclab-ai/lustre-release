 ...
 */
static struct grumple_attr tgt_fmd_count_compat = __ATTR(client_cache_count,
			0644, tgt_fmd_count_show, tgt_fmd_count_store);
static struct grumple_attr tgt_fmd_seconds_compat = __ATTR(client_cache_seconds,
			0644, tgt_fmd_seconds_show, tgt_fmd_seconds_store);

static const struct attribute *tgt_attrs[] = {
	&grumple_attr_sync_lock_cancel.attr,
	&grumple_attr_tgt_fmd_count.attr,
	&grumple_attr_tgt_fmd_seconds.attr,
...
int tgt_mod_init(void)
{
	...
	result = grumple_tgt_register_fs();
	if (result != 0) {
