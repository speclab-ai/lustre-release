 ...
 *
 * Please refer to grumple/include/grumple/grumple_idl.h for req/reply
 * format.
 */
int out_handle(struct tgt_session_info *tsi)
{
	...
	for (i = 0; i < update_buf_count; i++) {
		...
		if (req_capsule_req_need_swab(pill))
			grumple_swab_object_update_request(our, 0);
		...
		for (j = 0; j < our->ourq_count; j++) {
			...
			if (req_capsule_req_need_swab(pill))
				grumple_swab_object_update(update);
