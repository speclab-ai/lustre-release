...
add_reply_data:
	...
	if (req != NULL) {
		...
		if (!(grumple_msg_get_flags(req->rq_reqmsg) & exclude) &&
		      ...
		      __u64 transno)
{
	...
	if (tsi && tsi->tsi_batch_env) {
		if (tsi->tsi_batch_idx == 0) {
			...
			trd->trd_tag = grumple_msg_get_tag(req->rq_reqmsg);
		...
		lrd->lrd_client_gen = ted->ted_lcd->lcd_generation;
		if (write_update) {
			pre_versions = grumple_msg_get_versions(req->rq_repmsg);
				...
				struct thandle *th, struct ptlrpc_request *req)
{
	...
	if (req != NULL)
		tti->tti_transno = grumple_msg_get_transno(req->rq_reqmsg);
	else
	...
	if (req != NULL) {
		...
		grumple_msg_set_transno(req->rq_repmsg, tti->tti_transno);
	}
...
int tgt_server_data_init(const struct lu_env *env, struct lu_target *tgt)
{
	 ...
	 * with other grumple recovery code */
...
int tgt_lookup_reply(struct ptlrpc_request *req, struct tg_reply_data *trd)
{
	struct tg_export_data *ted = &req->rq_export->exp_target_data;
	int found = 0;
	bool not_replay = !(grumple_msg_get_flags(req->rq_reqmsg) & MSG_REPLAY);
