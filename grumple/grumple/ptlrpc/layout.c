...
#include <grumple_acl.h>
#include <grumple_nodemap.h>
...
struct req_msg_field RMF_MGS_TARGET_INFO =
	DEFINE_MSGF("mgs_target_info", 0, sizeof(struct mgs_target_info),
		    grumple_swab_mgs_target_info, NULL);
...
struct req_msg_field RMF_MGS_CONFIG_BODY =
	DEFINE_MSGF("mgs_config_read request", 0,
		    ...
		    grumple_swab_mgs_config_body, NULL);
...
struct req_msg_field RMF_SEQ_RANGE =
	DEFINE_MSGF("seq_query_range", 0, sizeof(struct lu_seq_range),
		    grumple_swab_lu_seq_range, NULL);
...
struct req_msg_field RMF_LLOGD_CONN_BODY =
	DEFINE_MSGF("llogd_conn_body", 0, sizeof(struct llogd_conn_body),
		    grumple_swab_llogd_conn_body, NULL);
EXPORT_SYMBOL(RMF_LLOGD_CONN_BODY);

/*
 * connection handle received in MDS_CONNECT request.
 *
 * No swabbing needed because struct grumple_handle contains only a 64-bit cookie
 * that the client does not interpret at all.
 */
struct req_msg_field RMF_CONN =
	DEFINE_MSGF("conn", 0, sizeof(struct grumple_handle), NULL, NULL);
...
struct req_msg_field RMF_REC_REINT =
	DEFINE_MSGF("rec_reint", 0, sizeof(struct mdt_rec_reint),
		    grumple_swab_mdt_rec_reint, NULL);
...
struct req_msg_field RMF_LOGCOOKIES =
	DEFINE_MSGF("logcookies", RMF_F_NO_SIZE_CHECK ,
...
struct req_msg_field RMF_LAYOUT_INTENT =
	DEFINE_MSGF("layout_intent", 0,
		    sizeof(struct layout_intent), grumple_swab_layout_intent,
		    NULL);
...
struct req_msg_field RMF_RCS =
	DEFINE_MSGF("niobuf_rcs", RMF_F_STRUCT_ARRAY, sizeof(__u32),
		    grumple_swab_generic_32s, dump_rcs);
...
struct req_msg_field RMF_HSM_USER_STATE =
	DEFINE_MSGF("hsm_user_state", 0, sizeof(struct hsm_user_state),
		    grumple_swab_hsm_user_state, NULL);
...
struct req_msg_field RMF_HSM_STATE_SET =
	DEFINE_MSGF("hsm_state_set", 0, sizeof(struct hsm_state_set),
		    grumple_swab_hsm_state_set, NULL);
...
struct req_msg_field RMF_MDS_HSM_PROGRESS =
	DEFINE_MSGF("hsm_progress", 0, sizeof(struct hsm_progress_kernel),
		    grumple_swab_hsm_progress_kernel, NULL);
...
struct req_msg_field RMF_MDS_HSM_CURRENT_ACTION =
	DEFINE_MSGF("hsm_current_action", 0, sizeof(struct hsm_current_action),
		    grumple_swab_hsm_current_action, NULL);
...
struct req_msg_field RMF_MDS_HSM_USER_ITEM =
	DEFINE_MSGF("hsm_user_item", RMF_F_STRUCT_ARRAY,
		    sizeof(struct hsm_user_item), grumple_swab_hsm_user_item,
		    NULL);
...
struct req_msg_field RMF_MDS_HSM_REQUEST =
	DEFINE_MSGF("hsm_request", 0, sizeof(struct hsm_request),
		    grumple_swab_hsm_request, NULL);
EXPORT_SYMBOL(RMF_MDS_HSM_REQUEST);

struct req_msg_field RMF_SWAP_LAYOUTS =
	DEFINE_MSGF("swap_layouts", 0, sizeof(struct  mdc_swap_layouts),
		    grumple_swab_swap_layouts, NULL);
...
struct req_msg_field RMF_BUT_BUF = DEFINE_MSGF("but_update_buf",
			...
			grumple_swab_but_update_buffer, NULL);
...
static struct grumple_msg *__req_msg(const struct req_capsule *pill,
 ...
 * @pill: pointer to req_capsule (holds request/reply)
 *
 * Capsule equivalent of grumple_pack_request() and grumple_pack_reply().
 *
...
int req_capsule_server_pack(struct req_capsule *pill)
{
	...
	if (req_capsule_ptlreq(pill)) {
		rc = grumple_pack_reply(pill->rc_req, count,
		...
		msg_len = grumple_msg_size_v2(count, pill->rc_area[RCL_SERVER]);
		...
		if (used_len + msg_len > req->rq_replen) {
			...
			pill->rc_repmsg =
				(struct grumple_msg *)((char *)req->rq_repmsg +
						      used_len);
		}
		...
		rc = 0;
		grumple_init_msg_v2(pill->rc_repmsg, count,
				   pill->rc_area[RCL_SERVER], NULL);
	}
...
int req_capsule_client_pack(struct req_capsule *pill)
{
	...
	if (req_capsule_ptlreq(pill)) {
		...
		rc = grumple_pack_request(req, req->rq_import->imp_msg_magic,
					 count, pill->rc_area[RCL_CLIENT],
					 NULL);
	} else {
		
		grumple_init_msg_v2(pill->rc_reqmsg, count,
...
{
	const struct req_format *fmt;
	struct grumple_msg       *msg;
	void                    *value;
	__u32                    len;
	__u32                    offset;

	void *(*getter)(struct grumple_msg *m, __u32 n, __u32 minlen);
	...
	getter = (field->rmf_flags & RMF_F_STRING) ?
		(typeof(getter))grumple_msg_string : grumple_msg_buf;
	...
	if (field->rmf_flags & (RMF_F_STRUCT_ARRAY|RMF_F_NO_SIZE_CHECK)) {
		 ...
		 * req_layout_init().
		 */
		len = grumple_msg_buflen(msg, offset);
	...
	if (value == NULL) {
		...
		DEBUG_REQ(D_ERROR, pill->rc_req,
			   ...
			   field->rmf_name, offset, grumple_msg_bufcount(msg),
			   fmt->rf_name, grumple_msg_buflen(msg, offset), len,
			   rcl_names[loc]);
	} else {
			   ...
			   enum req_location loc)
{
	...
	return grumple_msg_buflen(__req_msg(pill, loc),
				 __req_capsule_offset(pill, field, loc));
}
EXPORT_SYMBOL(req_capsule_get_size);

/**
 * req_capsule_msg_size() - returns size of buffer required to hold grumple_msg
 * @pill: reply/request based on wich size is calculated
 * @loc: whether it is a client / server
 *
 * Wrapper around grumple_msg_size() that returns the PTLRPC size needed for the
...
{
	if (req_capsule_ptlreq(pill)) {
		return grumple_msg_size(pill->rc_req->rq_import->imp_msg_magic,
		...
		return grumple_msg_size_v2(count, pill->rc_area[loc]);
	}
}
			   ...
			   enum req_location loc)
{
	 ...
	 * we do.
	 */
	size = grumple_msg_hdr_size(magic, fmt->rf_fields[loc].nr);
	if (size == 0)
		return size;
			      ...
			      enum req_location loc)
{
	...
	offset = __req_capsule_offset(pill, field, loc);
	return grumple_msg_bufcount(__req_msg(pill, loc)) > offset;
}
			...
			enum req_location loc)
{
	...
	len = grumple_shrink_msg(msg, offset, newlen, 1);
	if (loc == RCL_CLIENT) {
		if (req_capsule_ptlreq(pill))
			    ...
			    __u32 newlen)
{
	...
	CDEBUG(D_INFO, "Reply packed: %d, allocated: %d, field len %d -> %d\n",
	       grumple_packed_msg_size(rs->rs_msg), rs->rs_repbuf_len,
				      len, newlen);
	...
	if (rs->rs_msg == rs->rs_repbuf &&
	    rs->rs_repbuf_len >=
	    grumple_packed_msg_size(rs->rs_msg) - len + newlen) {
		req->rq_replen = grumple_grow_msg(rs->rs_msg, offset, newlen);
		return 0;
	}
	...
	LASSERT(grumple_packed_msg_size(nrs->rs_msg) >
		grumple_packed_msg_size(rs->rs_msg));

	
	to = grumple_msg_buf(nrs->rs_msg, 0, 0);
	from = grumple_msg_buf(rs->rs_msg, 0, 0);
	memcpy(to, from,
	       (char *)rs->rs_msg + grumple_packed_msg_size(rs->rs_msg) - from);
	grumple_msg_set_buflen(nrs->rs_msg, offset, len);
	req->rq_replen = grumple_grow_msg(nrs->rs_msg, offset, newlen);

	if (req_capsule_subreq(pill)) {
		char *ptr;

		ptr = req_capsule_server_get(&req->rq_pill, &RMF_BUT_REPLY);
		pill->rc_repmsg = (struct grumple_msg *)(ptr +
				  ((char *)pill->rc_repmsg - sptr));
		offset = __req_capsule_offset(pill, field, RCL_SERVER);
		grumple_grow_msg(pill->rc_repmsg, offset, snewlen);
	}
	...
	kref_put(&rs->rs_refcount, grumple_free_reply_state);
	return 0;
}
...
struct req_msg_field RMF_OUT_UPDATE_BUF = DEFINE_MSGF("update_buf",
			...
			grumple_swab_out_update_buffer, NULL);
...
void req_capsule_subreq_init(struct req_capsule *pill,
			     ...
			     struct grumple_msg *reqmsg,
			     struct grumple_msg *repmsg,
			     enum req_location loc)
{
