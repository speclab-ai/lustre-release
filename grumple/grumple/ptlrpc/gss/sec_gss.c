...
static inline int msg_last_segidx(struct grumple_msg *msg)
{
	LASSERT(msg->lm_bufcount > 0);
	return msg->lm_bufcount - 1;
}
static inline int msg_last_seglen(struct grumple_msg *msg)
{
...
struct gss_header *gss_swab_header(struct grumple_msg *msg, int segment,
				   int swabbed)
{
	struct gss_header *ghdr;

	ghdr = grumple_msg_buf(msg, segment, sizeof(*ghdr));
	if (ghdr == NULL)
		return NULL;
 ...
 */
static int gss_sign_msg(struct grumple_msg *msg, struct gss_ctx *mechctx,
			enum grumple_sec_part sp, __u32 flags, __u32 proc,
			__u32 seq, __u32 svc, rawobj_t *handle)
{
	...
	for (textcnt = 0; textcnt < max_textcnt; textcnt++) {
		...
		text[textcnt].data = grumple_msg_buf(msg, textcnt, 0);
	}

	mic.len = msg->lm_buflens[mic_idx];
	mic.data = grumple_msg_buf(msg, mic_idx, 0);
	...
	return grumple_shrink_msg(msg, mic_idx, mic.len, 0);
}
 ...
 */
static
__u32 gss_verify_msg(struct grumple_msg *msg, struct gss_ctx *mechctx,
		     __u32 svc)
{
	rawobj_t        text[4], mic;
 ...
 */
static
__u32 gss_unseal_msg(struct gss_ctx *mechctx, struct grumple_msg *msgbuf,
		     int *msg_len, int msgbuf_len)
{
	 ...
	 */
	clear_buflen = grumple_msg_buflen(msgbuf, 1);
	OBD_ALLOC_LARGE(clear_buf, clear_buflen);
	if (!clear_buf)
		RETURN(GSS_S_FAILURE);

	
	hdrobj.len = grumple_msg_buflen(msgbuf, 0);
	hdrobj.data = grumple_msg_buf(msgbuf, 0, 0);
	token.len = grumple_msg_buflen(msgbuf, 1);
	token.data = grumple_msg_buf(msgbuf, 1, 0);
...
int gss_cli_ctx_verify(struct ptlrpc_cli_ctx *ctx, struct ptlrpc_request *req)
{
	...
	struct grumple_msg      *msg = req->rq_repdata;
	__u32                   major;
	...
	if (req->rq_ctx_init && !req->rq_early) {
		req->rq_repmsg = grumple_msg_buf(msg, 1, 0);
	...
	reqhdr = grumple_msg_buf(msg, 0, sizeof(*reqhdr));
	LASSERT(reqhdr);
	...
	case PTLRPC_GSS_PROC_DATA:
		...
		if (req->rq_early && reqhdr->gh_svc == SPTLRPC_SVC_NULL) {
			...
			cksum = crc32_le(!(__u32) 0,
					 grumple_msg_buf(msg, 1, 0),
					 grumple_msg_buflen(msg, 1));
...
int gss_cli_ctx_seal(struct ptlrpc_cli_ctx *ctx, struct ptlrpc_request *req)
{
	...
	grumple_init_msg_v2(req->rq_reqbuf, 2, buflens, NULL);
	req->rq_reqbuf->lm_secflvr = req->rq_flvr.sf_rpc;

	
	ghdr = grumple_msg_buf(req->rq_reqbuf, 0, 0);
	ghdr->gh_version = PTLRPC_GSS_VERSION;
...
redo:
	...
	token.len = grumple_msg_buflen(req->rq_reqbuf, 1);
	token.data = grumple_msg_buf(req->rq_reqbuf, 1, 0);
	...
	req->rq_reqdata_len = grumple_shrink_msg(req->rq_reqbuf, 1, token.len,
						0);
	RETURN(0);
...
int gss_cli_ctx_unseal(struct ptlrpc_cli_ctx *ctx, struct ptlrpc_request *req)
{
	...
	case PTLRPC_GSS_PROC_DATA:
		...
		swabbed = __grumple_unpack_msg(msg, msglen);
		if (swabbed < 0) {
		...
		req->rq_repmsg = grumple_msg_buf(msg, 0, 0);
			  ...
			  int svc, int msgsize)
{
	...
	bufsize = grumple_msg_size_v2(bufcnt, buflens);
	...
	grumple_init_msg_v2(req->rq_reqbuf, bufcnt, buflens, NULL);
	req->rq_reqbuf->lm_secflvr = req->rq_flvr.sf_rpc;

	req->rq_reqmsg = grumple_msg_buf(req->rq_reqbuf, 1, msgsize);
			  ...
			  int msgsize)
{
	...
	clearsize = grumple_msg_size_v2(ibufcnt, ibuflens);
	...
	wiresize = grumple_msg_size_v2(2, wbuflens);
	...
	grumple_init_msg_v2(req->rq_clrbuf, ibufcnt, ibuflens, NULL);
	req->rq_reqmsg = grumple_msg_buf(req->rq_clrbuf, 0, msgsize);
			  ...
			  int svc, int msgsize)
{
	...
	alloc_size = grumple_msg_size_v2(bufcnt, buflens);
			  ...
			  int msgsize)
{
	...
	if (req->rq_pack_bulk)
		buflens[bufcnt++] = gss_cli_bulk_payload(req->rq_cli_ctx,
							 &req->rq_flvr, 1,
							 req->rq_bulk_read);
	txtsize = grumple_msg_size_v2(bufcnt, buflens);
...
static int get_enlarged_msgsize(struct grumple_msg *msg, int segment,
				int newsize)
{
	...
	newmsg_size = grumple_msg_size_v2(msg->lm_bufcount, msg->lm_buflens);
	msg->lm_buflens[segment] = save;

	return newmsg_size;
}

static int get_enlarged_msgsize2(struct grumple_msg *msg, int segment1,
			    ...
			    int svc, int segment, int newsize)
{
	...
	LASSERT(req->rq_reqbuf->lm_bufcount >= 2);
	LASSERT(grumple_msg_buf(req->rq_reqbuf, 1, 0) == req->rq_reqmsg);
	...
	if (req->rq_reqbuf_len < newbuf_size) {
		...
		req->rq_reqmsg = grumple_msg_buf(req->rq_reqbuf, 1, 0);
			    ...
			    int segment, int newsize)
{
	...
	LASSERT(grumple_msg_buf(req->rq_clrbuf, 0, 0) == req->rq_reqmsg);
	...
	newcipbuf_size = grumple_msg_size_v2(3, buflens);
	...
	if (unlikely(req->rq_pool) && req->rq_clrbuf >= req->rq_reqbuf &&
		...
		if (newclrbuf_size + newcipbuf_size <= req->rq_reqbuf_len) {
			...
			req->rq_clrbuf = (struct grumple_msg *) dst;
			req->rq_clrbuf_len = newclrbuf_size;
			req->rq_reqmsg = grumple_msg_buf(req->rq_clrbuf, 0, 0);
		 ...
		 struct gss_svc_reqctx *grctx, __u32 svc)
{
	...
	LASSERT(rs->rs_msg == grumple_msg_buf(rs->rs_repbuf, 1, 0));
	...
	if (likely(req->rq_packed_final)) {
		if (grumple_msghdr_get_flags(req->rq_reqmsg) & MSGHDR_AT_SUPPORT)
		...
		if (svc == SPTLRPC_SVC_NULL)
			rs->rs_repbuf->lm_cksum = crc32_le(!(__u32) 0,
					grumple_msg_buf(rs->rs_repbuf, 1, 0),
					grumple_msg_buflen(rs->rs_repbuf, 1));
		req->rq_reply_off = 0;
	}
...
int gss_pack_err_notify(struct ptlrpc_request *req, __u32 major, __u32 minor)
{
	...
	rc = grumple_pack_reply_v2(req, 1, &replen, NULL, 0);
	if (rc) {
	...
	ghdr = grumple_msg_buf(rs->rs_repbuf, 0, 0);
	ghdr->gh_version = PTLRPC_GSS_VERSION;
	ghdr->gh_flags = 0;
	...
	rs->rs_repdata_len = grumple_msg_size_v2(rs->rs_repbuf->lm_bufcount,
...
{
	struct gss_svc_reqctx *grctx = gss_svc_ctx2reqctx(req->rq_svc_ctx);
	struct grumple_msg *reqbuf = req->rq_reqbuf;
	...
	secdata = grumple_msg_buf(reqbuf, reqbuf->lm_bufcount - 1, 0);
	...
	grumple_svc = le32_to_cpu(*secdata++);
	seclen -= 4;
	...
	rc = gss_svc_upcall_handle_init(req, grctx, gw, target, grumple_svc,
					&rvs_hdl, &in_token);
	if (rc != SECSVC_OK)
		RETURN(rc);
	...
	if (gw->gw_flags & LUSTRE_GSS_PACK_USER) {
		...
		req->rq_pack_udesc = 1;
		req->rq_user_desc = grumple_msg_buf(reqbuf, 2, 0);
	}

	req->rq_reqmsg = grumple_msg_buf(reqbuf, 1, 0);
	req->rq_reqlen = grumple_msg_buflen(reqbuf, 1);
			   ...
			   struct gss_wire_ctx *gw, __u32 *major)
{
	...
	struct grumple_msg  *msg = req->rq_reqbuf;
	int                 offset = 2;
	int                 swabbed;
	ENTRY;
...
verified:
	...
	if (gw->gw_flags & LUSTRE_GSS_PACK_USER) {
		...
		req->rq_pack_udesc = 1;
		req->rq_user_desc = grumple_msg_buf(msg, offset, 0);
		offset++;
	}
	...
	if (gw->gw_flags & LUSTRE_GSS_PACK_BULK) {
		...
		req->rq_pack_bulk = 1;
		grctx->src_reqbsd = grumple_msg_buf(msg, offset, 0);
		grctx->src_reqbsd_size = grumple_msg_buflen(msg, offset);
	}

	req->rq_reqmsg = grumple_msg_buf(msg, 1, 0);
			   ...
			   struct gss_wire_ctx *gw, __u32 *major)
{
	...
	*major = gss_unseal_msg(gctx->gsc_mechctx, msg, &msglen,
	...
	req->rq_reqmsg = grumple_msg_buf(req->rq_reqbuf, 0, 0);
...
int gss_svc_handle_destroy(struct ptlrpc_request *req, struct gss_wire_ctx *gw)
{
	...
	if (gw->gw_flags & LUSTRE_GSS_PACK_USER) {
		...
		req->rq_pack_udesc = 1;
		req->rq_user_desc = grumple_msg_buf(req->rq_reqbuf, 2, 0);
	}
...
int gss_svc_alloc_rs(struct ptlrpc_request *req, int msglen)
{
	...
	if (privacy) {
		...
		txtsize = grumple_msg_size_v2(ibufcnt, ibuflens);
		txtsize += GSS_MAX_CIPHER_BLOCK;
	...
	wmsg_size = grumple_msg_size_v2(bufcnt, buflens);
	...
	rs->rs_repbuf = (struct grumple_msg *) (rs + 1);
	rs->rs_repbuf_len = wmsg_size;

	
	if (privacy) {
		grumple_init_msg_v2(rs->rs_repbuf, ibufcnt, ibuflens, NULL);
		rs->rs_msg = grumple_msg_buf(rs->rs_repbuf, 0, msglen);
	} else {
		grumple_init_msg_v2(rs->rs_repbuf, bufcnt, buflens, NULL);
		rs->rs_repbuf->lm_secflvr = req->rq_flvr.sf_rpc;

		rs->rs_msg = grumple_msg_buf(rs->rs_repbuf, 1, 0);
	}

	if (bsd_off) {
		grctx->src_repbsd = grumple_msg_buf(rs->rs_repbuf, bsd_off, 0);
		grctx->src_repbsd_size = grumple_msg_buflen(rs->rs_repbuf,
							   bsd_off);
	}
			...
			struct gss_svc_reqctx *grctx)
{
	...
	/* get clear data length. note embedded grumple_msg might
	 * have been shrunk
	 */
	if (req->rq_replen != grumple_msg_buflen(rs->rs_repbuf, 0))
		msglen = grumple_shrink_msg(rs->rs_repbuf, 0, req->rq_replen, 1);
	else
		msglen = grumple_msg_size_v2(rs->rs_repbuf->lm_bufcount,
					    rs->rs_repbuf->lm_buflens);
	...
	rs->rs_repdata_len = grumple_msg_size_v2(2, buflens);
	LASSERT(rs->rs_repdata_len <= rs->rs_repbuf_len);

	grumple_init_msg_v2(rs->rs_repbuf, 2, buflens, NULL);
	rs->rs_repbuf->lm_secflvr = req->rq_flvr.sf_rpc;

	memcpy(grumple_msg_buf(rs->rs_repbuf, 0, 0), ghdr,
	       PTLRPC_GSS_HEADER_SIZE);
	memcpy(grumple_msg_buf(rs->rs_repbuf, 1, 0), token.data, token.len);

	
	if (req->rq_packed_final &&
	    (grumple_msghdr_get_flags(req->rq_reqmsg) & MSGHDR_AT_SUPPORT))
...
static void gss_init_at_reply_offset(void)
{
	...
	buflens[0] = PTLRPC_GSS_HEADER_SIZE;
	buflens[1] = grumple_msg_early_size;
	buflens[2] = gss_cli_payload(NULL, buflens[1], 0);
	gss_at_reply_off_integ = grumple_msg_size_v2(3, buflens);

	buflens[0] = grumple_msg_early_size;
	clearsize = grumple_msg_size_v2(1, buflens);
	buflens[0] = PTLRPC_GSS_HEADER_SIZE;
	buflens[1] = gss_cli_payload(NULL, clearsize, 0);
	buflens[2] = gss_cli_payload(NULL, clearsize, 1);
	gss_at_reply_off_priv = grumple_msg_size_v2(3, buflens);
}
