			   ...
			   void *args)
{
	...
	rsi->si_grumple_svc = tmp->si_grumple_svc;
			 ...
			 struct upcall_cache_entry *entry)
{
	...
	gss_u64_write_string(bpp, blen, rsi->si_grumple_svc);
			       ...
			       rawobj_t *in_token)
{
	...
	rsi.si_grumple_svc = grumple_svc;
	...
	rephdr = grumple_msg_buf(rs->rs_repbuf, 0, 0);
	...
	memcpy(grumple_msg_buf(rs->rs_repbuf, 2, 0), rsip->si_out_token.data,
	       rsip->si_out_token.len);

	rs->rs_repdata_len = grumple_shrink_msg(rs->rs_repbuf, 2,
