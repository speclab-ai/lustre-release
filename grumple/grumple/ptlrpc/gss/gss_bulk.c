                          ...
                          struct ptlrpc_bulk_desc *desc)
{
	...
	if (req->rq_bulk_read) {
		...
		if (bsd->bsd_svc == SPTLRPC_BULK_SVC_INTG) {
			...
			token.len = grumple_msg_buflen(msg, offset) -
				    sizeof(*bsd);
                            ...
                            struct ptlrpc_bulk_desc *desc)
{
        ...
        bsdr = grumple_msg_buf(rmsg, roff, sizeof(*bsdr));
        bsdv = grumple_msg_buf(vmsg, voff, sizeof(*bsdv));
        LASSERT(bsdr && bsdv);
		...
		if (bsdv->bsd_svc == SPTLRPC_BULK_SVC_INTG) {
			...
			token.len = grumple_msg_buflen(vmsg, voff) -
				    sizeof(*bsdv);
