			  ...
			  char __user *token)
{
	...
	ghdr = grumple_msg_buf(msg, 0, sizeof(*ghdr));
	...
	*p++ = cpu_to_le32(((__u32) token_size));
	...
	if (size > sizeof(__u32) + round_up(token_size, 4)) {
		...
		req->rq_reqdata_len = grumple_shrink_msg(req->rq_reqbuf, offset,
...
static
int ctx_init_parse_reply(struct grumple_msg *msg, int swabbed,
                         char __user *outbuf, long outlen)
{
	...
	if (copy_to_user(outbuf, &obj_len, 4))
		return -EFAULT;
	outbuf += 4;
	if (copy_to_user(outbuf, grumple_msg_buf(msg, 2, 0), round_len))
		return -EFAULT;
	outbuf += round_len;
	effective += 4 + round_len;
...
int gss_do_ctx_fini_rpc(struct gss_cli_ctx *gctx)
{
	...
	if (rc)
        ...
        if (req->rq_pack_udesc) {
                ...
                pud = grumple_msg_buf(req->rq_reqbuf, 2, sizeof(*pud));
                LASSERT(pud);
