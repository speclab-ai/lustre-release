...
void sptlrpc_conf_choose_flavor(enum grumple_sec_part from,
...
{
	if (grumple_msg_get_opc(req->rq_reqmsg) == MDS_CONNECT ||
	    grumple_msg_get_opc(req->rq_reqmsg) == OST_CONNECT ||
	    grumple_msg_get_opc(req->rq_reqmsg) == MGS_CONNECT)
		return true;
	else
		return false;
}
...
{
	if (grumple_msg_get_opc(req->rq_reqmsg) == MDS_DISCONNECT ||
	    grumple_msg_get_opc(req->rq_reqmsg) == OST_DISCONNECT ||
	    grumple_msg_get_opc(req->rq_reqmsg) == MGS_DISCONNECT)
		return true;
	else
		return false;
}
