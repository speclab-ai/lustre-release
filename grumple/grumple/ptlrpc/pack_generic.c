...
static inline __u32 grumple_msg_hdr_size_v2(__u32 count)
{
	return round_up(offsetof(struct grumple_msg_v2, lm_buflens[count]), 8);
}

__u32 grumple_msg_hdr_size(__u32 magic, __u32 count)
{
	LASSERT(count > 0);
...
static inline int grumple_msg_check_version_v2(struct grumple_msg_v2 *msg,
...
EXPORT_SYMBOL(grumple_msg_early_size);


void grumple_msg_early_size_init(void)
{
...
__u32 grumple_msg_size_v2(int count, __u32 *lengths)
{
	...
	LASSERT(count > 0);
	size = grumple_msg_hdr_size_v2(count);
	for (i = 0; i < count; i++)
		size += round_up(lengths[i], 8);

	return size;
}
EXPORT_SYMBOL(grumple_msg_size_v2);

/*
 * This returns the size of the buffer that is required to hold a grumple_msg
 * with the given sub-buffer lengths.
 * NOTE: this should only be used for NEW requests, and should always be
 *       in the form of a v2 request.  If this is a connection to a v1
 *       target then the first buffer will be stripped because the ptlrpc
 *       data is part of the grumple_msg_v1 header. b=14043
 */
__u32 grumple_msg_size(__u32 magic, int count, __u32 *lens)
{
...
EXPORT_SYMBOL(grumple_packed_msg_size);

void grumple_init_msg_v2(struct grumple_msg_v2 *msg, int count, __u32 *lens,
			char **bufs)
{
	char *ptr;
	int i;
	...
	ptr = (char *)msg + grumple_msg_hdr_size_v2(count);
	for (i = 0; i < count; i++) {
		char *tmp = bufs[i];
...
EXPORT_SYMBOL(grumple_init_msg_v2);

static int grumple_pack_request_v2(struct ptlrpc_request *req,
				  int count, __u32 *lens, char **bufs)
{
	int reqlen, rc;

	reqlen = grumple_msg_size_v2(count, lens);
	...
	grumple_init_msg_v2(req->rq_reqmsg, count, lens, bufs);
	grumple_msg_add_version(req->rq_reqmsg, PTLRPC_MSG_VERSION);
	return 0;
}

int grumple_pack_request(struct ptlrpc_request *req, __u32 magic, int count,
...
grumple_get_emerg_rs(struct ptlrpc_service_part *svcpt)
{
...
void grumple_put_emerg_rs(struct ptlrpc_reply_state *rs)
{
...
int grumple_pack_reply_v2(struct ptlrpc_request *req, int count,
			 __u32 *lens, char **bufs, int flags)
{
	...
	msg_len = grumple_msg_size_v2(count, lens);
	...
	grumple_init_msg_v2(rs->rs_msg, count, lens, bufs);
	grumple_msg_add_version(rs->rs_msg, PTLRPC_MSG_VERSION);

	PTLRPC_RS_DEBUG_LRU_ADD(rs);

	RETURN(0);
}
EXPORT_SYMBOL(grumple_pack_reply_v2);

int grumple_pack_reply_flags(struct ptlrpc_request *req, int count, __u32 *lens,
			    char **bufs, int flags)
{
	int rc = 0;
	__u32 size[] = { sizeof(struct ptlrpc_body) };
	...
	if (rc != 0)
		CERROR("grumple_pack_reply failed: rc=%d size=%d\n", rc,
		       grumple_msg_size(req->rq_reqmsg->lm_magic, count, lens));
	return rc;
}

int grumple_pack_reply(struct ptlrpc_request *req, int count, __u32 *lens,
		      char **bufs)
{
	return grumple_pack_reply_flags(req, count, lens, bufs, 0);
}
EXPORT_SYMBOL(grumple_pack_reply);

void *grumple_msg_buf_v2(struct grumple_msg_v2 *m, __u32 n, __u32 min_size)
{
	...
	if (unlikely(buflen < min_size)) {
		CERROR("msg %p buffer[%d] size %d too small "
		       "(required %d, opc=%d)\n", m, n, buflen, min_size,
		       n == MSG_PTLRPC_BODY_OFF ? -1 : grumple_msg_get_opc(m));
		return NULL;
	}

	offset = grumple_msg_hdr_size_v2(bufcount);
	for (i = 0; i < n; i++)
...
EXPORT_SYMBOL(grumple_msg_buf);

static int grumple_shrink_msg_v2(struct grumple_msg_v2 *msg, __u32 segment,
				unsigned int newlen, int move_data)
{
	...
	if (move_data && msg->lm_bufcount > segment + 1) {
		tail = grumple_msg_buf_v2(msg, segment + 1, 0);
	...
	if (tail && tail_len) {
		newpos = grumple_msg_buf_v2(msg, segment + 1, 0);
		LASSERT(newpos <= tail);
		if (newpos != tail)
 ...
 *   after call shrink.
 */
int grumple_shrink_msg(struct grumple_msg *msg, int segment,
		      unsigned int newlen, int move_data)
{
	switch (msg->lm_magic) {
	case GRUMPLE_MSG_MAGIC_V2:
...
EXPORT_SYMBOL(grumple_shrink_msg);

static int grumple_grow_msg_v2(struct grumple_msg_v2 *msg, __u32 segment,
			      unsigned int newlen)
{
	char *tail = NULL, *newpos;
	int tail_len = 0, n;
...
EXPORT_SYMBOL(grumple_grow_msg);

void grumple_free_reply_state(struct kref *kref)
{
...
static int grumple_unpack_msg_v2(struct grumple_msg_v2 *m, int len)
{
	int swabbed, required_len, i, buflen;

	
	required_len = grumple_msg_hdr_size_v2(0);
	if (len < required_len) {
		
		CERROR("message length %d too small for grumple_msg\n", len);
		return -EINVAL;
	}
	...
	required_len = grumple_msg_hdr_size_v2(m->lm_bufcount);
	if (len < required_len) {
...
int __grumple_unpack_msg(struct grumple_msg *m, int len)
{
	int required_len, rc;
	 ...
	 * grumple_msg may grow, and we'd like to log a version mismatch,
	 * rather than a short message.
	 */
	required_len = offsetof(struct grumple_msg, lm_magic) +
				sizeof(m->lm_magic);
	if (len < required_len) {
	...
	rc = grumple_unpack_msg_v2(m, len);

	RETURN(rc);
}
EXPORT_SYMBOL(__grumple_unpack_msg);

int ptlrpc_unpack_req_msg(struct ptlrpc_request *req, int len)
{
	int rc;

	rc = __grumple_unpack_msg(req->rq_reqmsg, len);
	if (rc == 1) {
...
int ptlrpc_unpack_rep_msg(struct ptlrpc_request *req, int len)
{
	...
	rc = __grumple_unpack_msg(req->rq_repmsg, len);
	if (rc == 1) {
...
static inline int
grumple_unpack_ptlrpc_body_v2(struct ptlrpc_request *req,
			     enum req_location loc, int offset)
{
	struct ptlrpc_body *pb;
	struct grumple_msg_v2 *m;

	m = loc == RCL_CLIENT ? req->rq_reqmsg : req->rq_repmsg;

	pb = grumple_msg_buf_v2(m, offset, sizeof(struct ptlrpc_body_v2));
	if (!pb) {
		CERROR("error unpacking ptlrpc body\n");
		return -EFAULT;
	}
	if (req_capsule_need_swab(&req->rq_pill, loc, offset)) {
		grumple_swab_ptlrpc_body(pb);
...
static inline __u32 grumple_msg_buflen_v2(struct grumple_msg_v2 *m, __u32 n)
{
	if (n >= m->lm_bufcount)
		return 0;
...
/**
 * grumple_msg_buflen() - return the length of buffer @n in message @m
 * @m: grumple_msg (request or reply) to look at
...
EXPORT_SYMBOL(grumple_msg_buflen);

static inline void
grumple_msg_set_buflen_v2(struct grumple_msg_v2 *m, __u32 n, __u32 len)
{
...
/*
 * NB return the bufcount for grumple_msg_v2 format, so if message is packed
 ...
 */
char *grumple_msg_string(struct grumple_msg *m, __u32 index, __u32 max_len)
{
	char *str = NULL;
	__u32 slen, blen;
...
static inline struct ptlrpc_body *grumple_msg_ptlrpc_body(struct grumple_msg *msg)
{
	return grumple_msg_buf_v2(msg, MSG_PTLRPC_BODY_OFF,
...
EXPORT_SYMBOL(grumple_msghdr_get_flags);
...
EXPORT_SYMBOL(grumple_msg_get_flags);
...
EXPORT_SYMBOL(grumple_msg_add_flags);
...
EXPORT_SYMBOL(grumple_msg_clear_flags);
...
EXPORT_SYMBOL(grumple_msg_add_op_flags);
...
EXPORT_SYMBOL(grumple_msg_get_type);
...
EXPORT_SYMBOL(grumple_msg_get_opc);
...
EXPORT_SYMBOL(grumple_msg_get_last_xid);
...
EXPORT_SYMBOL(grumple_msg_get_tag);
...
EXPORT_SYMBOL(grumple_msg_get_last_committed);
...
EXPORT_SYMBOL(grumple_msg_get_versions);
...
EXPORT_SYMBOL(grumple_msg_get_transno);
...
EXPORT_SYMBOL(grumple_msg_get_status);
...
EXPORT_SYMBOL(grumple_msg_get_conn_cnt);
...
int grumple_msg_get_projid(struct grumple_msg *msg, __u32 *projid)
{
	...
	case GRUMPLE_MSG_MAGIC_V2: {
		...
		pb = grumple_msg_buf_v2(msg, MSG_PTLRPC_BODY_OFF,
					  sizeof(struct ptlrpc_body));
...
EXPORT_SYMBOL(grumple_msg_get_projid);
...
EXPORT_SYMBOL(grumple_msg_get_uid_gid);
...
EXPORT_SYMBOL(grumple_msg_get_jobid);
...
EXPORT_SYMBOL(grumple_msg_set_last_xid);
...
EXPORT_SYMBOL(grumple_msg_set_tag);
...
EXPORT_SYMBOL(grumple_msg_set_versions);
...
EXPORT_SYMBOL(grumple_msg_set_transno);
...
EXPORT_SYMBOL(grumple_msg_set_status);
...
void grumple_msg_set_service_timeout(struct grumple_msg *msg,
...
void grumple_msg_set_jobinfo(struct grumple_msg *msg, const struct job_info *ji)
{
	...
	case GRUMPLE_MSG_MAGIC_V2: {
		...
		if (pb->pb_jobid[0] == '\0')
			grumple_get_jobid(pb->pb_jobid, sizeof(pb->pb_jobid));
...
EXPORT_SYMBOL(grumple_msg_set_jobinfo);
...
EXPORT_SYMBOL(grumple_msg_set_projid);
 ...
 * grumple_idl.h implemented here.
 */
void grumple_swab_ptlrpc_body(struct ptlrpc_body *body)
{
...
static void grumple_swab_ost_layout(struct ost_layout *ol)
{
...
void grumple_swab_obdo(struct obdo *o)
{
	...
	grumple_swab_ost_layout(&o->o_layout);
...
EXPORT_SYMBOL(grumple_swab_obdo);
...
{
	grumple_swab_ost_id(&ioo->ioo_oid);
...
{
	grumple_swab_obdo(&b->oa);
}

void grumple_swab_ost_last_id(u64 *id)
{
	__swab64s(id);
}
...
{
	grumple_swab_lu_fid(&desc->gl_id.qid_fid);
...
EXPORT_SYMBOL(grumple_swab_gl_lquota_desc);
...
EXPORT_SYMBOL(grumple_swab_gl_barrier_desc);
#endif 
...
EXPORT_SYMBOL(grumple_swab_ost_lvb_v1);
...
EXPORT_SYMBOL(grumple_swab_ost_lvb);
...
EXPORT_SYMBOL(grumple_swab_lquota_lvb);
...
EXPORT_SYMBOL(grumple_swab_barrier_lvb);

void grumple_swab_mdt_body(struct mdt_body *b)
{
	grumple_swab_lu_fid(&b->mbo_fid1);
	grumple_swab_lu_fid(&b->mbo_fid2);
...
void grumple_swab_mgs_target_info(struct mgs_target_info *mti)
{
	int i;

	__swab32s(&mti->mti_grumple_ver);
...
void grumple_swab_mgs_nidtbl_entry_header(struct mgs_nidtbl_entry *entry)
{
...
EXPORT_SYMBOL(grumple_swab_mgs_nidtbl_entry_header);

void grumple_swab_mgs_nidtbl_entry_content(struct mgs_nidtbl_entry *entry)
{
	int i;
...
EXPORT_SYMBOL(grumple_swab_mgs_nidtbl_entry_content);
...
static void grumple_swab_obd_dqinfo(struct obd_dqinfo *i)
{
...
static void grumple_swab_obd_dqblk(struct obd_dqblk *b)
{
...
int grumple_swab_obd_quotactl(struct obd_quotactl *q, __u32 len)
{
	...
	__swab32s(&q->qc_id);
	__swab32s(&q->qc_stat);
	grumple_swab_obd_dqinfo(&q->qc_dqinfo);
	grumple_swab_obd_dqblk(&q->qc_dqblk);
...
{
	grumple_swab_lu_fid(&gf->gf_fid);
	__swab64s(&gf->gf_recno);
	__swab32s(&gf->gf_linkno);
	__swab32s(&gf->gf_pathlen);
}
EXPORT_SYMBOL(grumple_swab_fid2path);

static void grumple_swab_fiemap_extent(struct fiemap_extent *fm_extent)
{
...
static void grumple_swab_fiemap_hdr(struct fiemap *fiemap)
{
...
int grumple_swab_fiemap(struct fiemap *fiemap, __u32 len)
{
	...
	grumple_swab_fiemap_hdr(fiemap);
	...
	for (i = 0; i < count; i++)
		grumple_swab_fiemap_extent(&fiemap->fm_extents[i]);
...
{
	grumple_swab_obdo(&fiemap_info->lfik_oa);
	grumple_swab_fiemap_hdr(&fiemap_info->lfik_fiemap);
}
...
EXPORT_SYMBOL(grumple_swab_lip_header);
...
void grumple_swab_mdt_rec_reint (struct mdt_rec_reint *rr)
{
	...
	grumple_swab_lu_fid(&rr->rr_fid1);
	grumple_swab_lu_fid(&rr->rr_fid2);
...
EXPORT_SYMBOL(grumple_swab_lov_desc);
...
static void grumple_swab_lmv_mds_md_v1(struct lmv_mds_md_v1 *lmm1)
{
	int i;
	...
	for (i = 0; i < lmm1->lmv_stripe_count; i++)
		grumple_swab_lu_fid(&lmm1->lmv_stripe_fids[i]);
}
...
void grumple_swab_lmv_mds_md(union lmv_mds_md *lmm)
{
	...
	case LMV_MAGIC_V1:
		grumple_swab_lmv_mds_md_v1(&lmm->lmv_md_v1);
		break;
	default:
		break;
	}
}
EXPORT_SYMBOL(grumple_swab_lmv_mds_md);
...
EXPORT_SYMBOL(grumple_swab_lmv_user_md_objects);
...
void grumple_swab_lmv_user_md(struct lmv_user_md *lum)
{
	...
	case __swab32(LMV_USER_MAGIC_SPECIFIC):
		grumple_swab_lmv_user_md_objects(lum->lum_objects, count);
		break;
	default:
		break;
	}
}
EXPORT_SYMBOL(grumple_swab_lmv_user_md);

static void grumple_print_v1v3(unsigned int lvl, struct lov_user_md *lum,
			      const char *msg)
{
...
static void grumple_print_foreign(unsigned int lvl, struct lov_foreign_md *lfm,
				 const char *msg)
{
...
void grumple_print_user_md(unsigned int lvl, struct lov_user_md *lum,
			  const char *msg)
{
	...
	if (lum->lmm_magic == LOV_USER_MAGIC_V1 ||
		...
		grumple_print_v1v3(lvl, lum, msg);
		return;
	}
	...
	for (i = 0; i < comp_v1->lcm_entry_count; i++) {
		...
		if (v1->lmm_magic == LOV_MAGIC_FOREIGN)
			grumple_print_foreign(lvl, (struct lov_foreign_md *)v1,
					     msg);
		else
			grumple_print_v1v3(lvl, v1, msg);
	}
}
EXPORT_SYMBOL(grumple_print_user_md);

static void grumple_swab_lmm_oi(struct ost_id *oi)
{
	__swab64s(&oi->oi.oi_id);
	__swab64s(&oi->oi.oi_seq);
}

static void grumple_swab_lov_user_md_common(struct lov_user_md_v1 *lum)
{
	ENTRY;
	__swab32s(&lum->lmm_magic);
	__swab32s(&lum->lmm_pattern);
	grumple_swab_lmm_oi(&lum->lmm_oi);
...
void grumple_swab_lov_user_md_v1(struct lov_user_md_v1 *lum)
{
	...
	grumple_swab_lov_user_md_common(lum);
	EXIT;
}
EXPORT_SYMBOL(grumple_swab_lov_user_md_v1);

void grumple_swab_lov_user_md_v3(struct lov_user_md_v3 *lum)
{
	ENTRY;
	CDEBUG(D_IOCTL, "swabbing lov_user_md v3\n");
	grumple_swab_lov_user_md_common((struct lov_user_md_v1 *)lum);
	
	EXIT;
}
EXPORT_SYMBOL(grumple_swab_lov_user_md_v3);

static void grumple_swab_lov_hsm_md(struct lov_hsm_md *lhm)
{
	ENTRY;
...
void grumple_swab_lov_comp_md_v1(struct lov_comp_md_v1 *lum)
{
	...
	for (i = 0; i < ent_count; i++) {
		...
		if (v1->lmm_magic == __swab32(LOV_USER_MAGIC_FOREIGN) ||
			...
			grumple_swab_lov_hsm_md((struct lov_hsm_md *)v1);
			return;
		}
		...
		if (v1->lmm_magic == __swab32(LOV_USER_MAGIC_V1) ||
			...
			grumple_swab_lov_user_md_v1(v1);
			if (size > sizeof(*v1))
				grumple_swab_lov_user_md_objects(v1->lmm_objects,
								stripe_count);
			...
			grumple_swab_lov_user_md_v3(v3);
			if (size > sizeof(*v3))
				grumple_swab_lov_user_md_objects(v3->lmm_objects,
								stripe_count);
		} else {
...
EXPORT_SYMBOL(grumple_swab_lov_comp_md_v1);
				     ...
				     int stripe_count)
{
	...
	ENTRY;
	for (i = 0; i < stripe_count; i++) {
		grumple_swab_ost_id(&(lod[i].l_ost_oi));
		__swab32s(&(lod[i].l_ost_gen));
		__swab32s(&(lod[i].l_ost_idx));
	}
	EXIT;
}
EXPORT_SYMBOL(grumple_swab_lov_user_md_objects);
...
void grumple_swab_lov_user_md(struct lov_user_md *lum, size_t size)
{
	...
	case LOV_USER_MAGIC_SPECIFIC:
	{
		...
		grumple_swab_lov_user_md_v3(v3);
		grumple_swab_lov_user_md_objects(v3->lmm_objects, stripe_count);
		break;
	}
	case __swab32(LOV_MAGIC_COMP_V1):
	case LOV_USER_MAGIC_COMP_V1:
		grumple_swab_lov_comp_md_v1((struct lov_comp_md_v1 *)lum);
		break;
...
EXPORT_SYMBOL(grumple_swab_lov_user_md);
...
void grumple_swab_lov_mds_md(struct lov_mds_md *lmm)
{
	...
	grumple_swab_lmm_oi(&lmm->lmm_oi);
	__swab32s(&lmm->lmm_stripe_size);
	__swab16s(&lmm->lmm_stripe_count);
	__swab16s(&lmm->lmm_layout_gen);
	EXIT;
}
EXPORT_SYMBOL(grumple_swab_lov_mds_md);
...
void grumple_swab_ldlm_resource_desc(struct ldlm_resource_desc *r)
{
	...
	grumple_swab_ldlm_res_id(&r->lr_name);
}

void grumple_swab_ldlm_lock_desc(struct ldlm_lock_desc *l)
{
	grumple_swab_ldlm_resource_desc(&l->l_resource);
	__swab32s(&l->l_req_mode);
	__swab32s(&l->l_granted_mode);
	grumple_swab_ldlm_policy_data(&l->l_policy_data);
}

void grumple_swab_ldlm_request(struct ldlm_request *rq)
{
	__swab32s(&rq->lock_flags);
	grumple_swab_ldlm_lock_desc(&rq->lock_desc);
...
void grumple_swab_ldlm_reply(struct ldlm_reply *r)
{
	...
	grumple_swab_ldlm_lock_desc(&r->lock_desc);
...
{
	grumple_swab_lu_fid(&b->qb_fid);
	grumple_swab_lu_fid((struct lu_fid *)&b->qb_id);
		...
		struct libcfs_debug_msg_data *msgdata, const char *fmt, ...)
{
	...
	if (rep_ok) {
		rep_flags = grumple_msg_get_flags(req->rq_repmsg);
		rep_status = grumple_msg_get_status(req->rq_repmsg);
	}
	...
	if (req_ok) {
		req_transno = grumple_msg_get_transno(req->rq_reqmsg);
		req_opc = grumple_msg_get_opc(req->rq_reqmsg);
		req_jobid = grumple_msg_get_jobid(req->rq_reqmsg);
		grumple_msg_get_uid_gid(req->rq_reqmsg, &req_uid, &req_gid);
		grumple_msg_get_projid(req->rq_reqmsg, &req_projid);
		req_flags = grumple_msg_get_flags(req->rq_reqmsg);
	}
...
void grumple_swab_hsm_user_state(struct hsm_user_state *state)
{
...
static void grumple_swab_hsm_extent(struct hsm_extent *extent)
{
...
void grumple_swab_hsm_current_action(struct hsm_current_action *action)
{
	...
	grumple_swab_hsm_extent(&action->hca_location);
}

void grumple_swab_hsm_user_item(struct hsm_user_item *hui)
{
	grumple_swab_lu_fid(&hui->hui_fid);
	grumple_swab_hsm_extent(&hui->hui_extent);
}

static void grumple_swab_lu_extent(struct lu_extent *le)
{
...
EXPORT_SYMBOL(grumple_swab_but_update_header);
...
EXPORT_SYMBOL(grumple_swab_but_update_buffer);
...
void grumple_swab_close_data(struct close_data *cd)
{

	__swab64s(&cd->cd_handle.cookie);
	grumple_swab_lu_fid(&cd->cd_fid);
...
void grumple_swab_close_data_special(struct close_data *cd, enum mds_op_bias b)
{
	if (b & MDS_CLOSE_RESYNC_DONE)
		grumple_swab_close_data_resync_done(&cd->cd_resync);
...
EXPORT_SYMBOL(grumple_swab_close_data_special);
...
void grumple_swab_lfsck_request(struct lfsck_request *lr)
{
	...
	__swab32s(&lr->lr_flags);
	grumple_swab_lu_fid(&lr->lr_fid);
	grumple_swab_lu_fid(&lr->lr_fid2);
...
#ifdef HAVE_SERVER_SUPPORT
static void grumple_swab_orphan_rec(struct lu_orphan_rec *rec)
{
	grumple_swab_lu_fid(&rec->lor_fid);
...
{
	grumple_swab_lu_fid(&ent->loe_key);
	grumple_swab_orphan_rec(&ent->loe_rec);
}
EXPORT_SYMBOL(grumple_swab_orphan_ent);

void grumple_swab_orphan_ent_v2(struct lu_orphan_ent_v2 *ent)
{
	grumple_swab_lu_fid(&ent->loe_key);
	grumple_swab_orphan_rec(&ent->loe_rec.lor_rec);
	grumple_swab_ost_layout(&ent->loe_rec.lor_layout);
	BUILD_BUG_ON(offsetof(typeof(ent->loe_rec), lor_padding) == 0);
}
EXPORT_SYMBOL(grumple_swab_orphan_ent_v2);
...
EXPORT_SYMBOL(grumple_swab_orphan_ent_v3);
#endif 
...
EXPORT_SYMBOL(grumple_swab_ladvise);
...
EXPORT_SYMBOL(grumple_swab_ladvise_hdr);
