 ...
 */
static inline
void null_encode_sec_part(struct grumple_msg *msg, enum grumple_sec_part sp)
{
	msg->lm_secflvr |= (((__u32) sp) & 0xFF) << 24;
}

static inline
enum grumple_sec_part null_decode_sec_part(struct grumple_msg *msg)
{
...
int null_ctx_verify(struct ptlrpc_cli_ctx *ctx, struct ptlrpc_request *req)
{
	...
	if (req->rq_early) {
		cksums = grumple_msg_get_cksum(req->rq_repdata);
		cksumc = grumple_msg_calc_cksum(req->rq_repmsg,
					       MSG_PTLRPC_BODY_OFF);
		      ...
		      int msgsize)
{
	...
	msgsize += grumple_msg_early_size;
			...
			int segment, int newsize)
{
	...
	struct grumple_msg      *oldbuf = req->rq_reqmsg;
	int                     oldsize, newmsg_size, alloc_size;

	LASSERT(req->rq_reqbuf);
	LASSERT(req->rq_reqbuf == req->rq_reqmsg);
	LASSERT(req->rq_reqbuf_len >= req->rq_reqlen);
	LASSERT(req->rq_reqlen == grumple_packed_msg_size(oldbuf));

	
	oldsize = req->rq_reqbuf->lm_buflens[segment];
	req->rq_reqbuf->lm_buflens[segment] = newsize;
	newmsg_size = grumple_packed_msg_size(oldbuf);
...
int null_authorize(struct ptlrpc_request *req)
{
	...
	if (likely(req->rq_packed_final)) {
		if (grumple_msghdr_get_flags(req->rq_reqmsg) & MSGHDR_AT_SUPPORT)
			req->rq_reply_off = grumple_msg_early_size;
	} else {
		__u32 cksum;

		cksum = grumple_msg_calc_cksum(rs->rs_repbuf,
					      MSG_PTLRPC_BODY_OFF);
		grumple_msg_set_cksum(rs->rs_repbuf, cksum);
	}
