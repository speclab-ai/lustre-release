 ...
 * @lcfg: grumple cfg for fileset operation
 ...
 * * %-EIO		undo operation failed during IAM update
 */
static int cfg_nodemap_fileset_cmd(struct grumple_cfg *lcfg,
				   bool *dynamic, bool *out_clean_llog_fileset)
{
	...
	nodemap_name = grumple_cfg_string(lcfg, 1);
	if (!nodemap_name || nodemap_name[0] == '\0')
		RETURN(-EINVAL);

	if (lcfg->lcfg_bufcount > 2) {
		fset = grumple_cfg_string(lcfg, 2);
	...
	case LCFG_NODEMAP_FILESET_ADD:
		...
		param = grumple_cfg_string(lcfg, 3);
		rc = kstrtobool(param, &fset_alt);
		if (rc)
			GOTO(out_unlock, rc);

		
		param = grumple_cfg_string(lcfg, 4);
			     ...
			     bool *out_clean_llog_fileset)
{
	...
	case LCFG_NODEMAP_TEST_NID:
		if (lcfg->lcfg_bufcount != 2)
			GOTO(out_lcfg, rc = -EINVAL);
		nidstr = grumple_cfg_string(lcfg, 1);
	...
	case LCFG_NODEMAP_TEST_ID:
		if (lcfg->lcfg_bufcount != 4)
			GOTO(out_lcfg, rc = -EINVAL);
		nidstr = grumple_cfg_string(lcfg, 1);
		idtype_str = grumple_cfg_string(lcfg, 2);
		client_idstr = grumple_cfg_string(lcfg, 3);
	...
	case LCFG_NODEMAP_BANLIST_DEL:
		if (lcfg->lcfg_bufcount != 3)
			GOTO(out_lcfg, rc = -EINVAL);
		nodemap_name = grumple_cfg_string(lcfg, 1);
		param = grumple_cfg_string(lcfg, 2);
		rc = cfg_nodemap_cmd(cmd, nodemap_name, param, dynamic,
				     out_clean_llog_fileset);
		break;
	case LCFG_NODEMAP_ADMIN:
