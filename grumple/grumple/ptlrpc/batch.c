...
static inline struct grumple_msg *
batch_update_reqmsg_next(struct batch_update_request *bur,
			 struct grumple_msg *reqmsg)
{
	if (reqmsg)
		return (struct grumple_msg *)((char *)reqmsg +
					     grumple_packed_msg_size(reqmsg));
	else
...
{
	if (repmsg)
		return (struct grumple_msg *)((char *)repmsg +
					     grumple_packed_msg_size(repmsg));
	else
				     ...
				     struct batch_update_reply *reply, int rc)
{
	...
	struct grumple_msg *repmsg = NULL;
	int count = 0;
	int index = 0;
				    ...
				    object_update_interpret_t interpreter)
{
	...
	for (; ;) {
		...
		reqmsg = (struct grumple_msg *)((char *)buf->bub_req +
						buf->bub_end);
...
static void cli_batch_resend_work(struct work_struct *data)
{
	...
	list_for_each_entry_safe(buf, tmp, &obuh->buh_buf_list, bub_item) {
		...
		struct grumple_msg *reqmsg = NULL;
		size_t max_len;
		int j;
...
repeat:
			...
			newmsg = (struct grumple_msg *)((char *)newbuf->bub_req +
						       newbuf->bub_end);
			msgsz = grumple_packed_msg_size(reqmsg);
			if (msgsz >= max_len) {
				int rc2;
