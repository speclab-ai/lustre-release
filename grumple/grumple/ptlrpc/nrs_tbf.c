...
nrs_tbf_jobid_cli_init(struct nrs_tbf_client *cli,
		       struct ptlrpc_request *req)
{
	char *jobid = grumple_msg_get_jobid(req->rq_reqmsg);
...
static int mdt_tbf_id_cli_set(struct ptlrpc_request *req,
			      struct tbf_id *id)
{
	u32 opc = grumple_msg_get_opc(req->rq_reqmsg);
	int rc = 0;
			      ...
			      enum nrs_tbf_flag ti_type)
{
	...
	rc = grumple_msg_get_uid_gid(req->rq_reqmsg, &id->ti_uid, &id->ti_gid);
				       ...
				       struct nrs_tbf_key *key)
{
	...
	if (key->tk_flags & NRS_TBF_FLAG_OPCODE)
		key->tk_opcode = grumple_msg_get_opc(req->rq_reqmsg);
...
nrs_tbf_opcode_cli_init(struct nrs_tbf_client *cli,
			struct ptlrpc_request *req)
{
	cli->tc_opcode = grumple_msg_get_opc(req->rq_reqmsg);
}
