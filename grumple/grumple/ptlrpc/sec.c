...
static int do_cli_unwrap_reply(struct ptlrpc_request *req)
{
	...
	rc = __grumple_unpack_msg(req->rq_repdata, req->rq_repdata_len);
	switch (rc) {
	case 1:
	...
	if (req->rq_repdata_len < sizeof(struct grumple_msg)) {
...
int sptlrpc_cli_unwrap_reply(struct ptlrpc_request *req)
{
	...
	req->rq_repdata = (struct grumple_msg *)
				   ...
				   struct ptlrpc_request **req_ret)
{
	...
	if (req->rq_nob_received < sizeof(struct grumple_msg)) {
	...
	early_req->rq_repdata = (struct grumple_msg *) early_buf;
 ...
 */
void _sptlrpc_enlarge_msg_inplace(struct grumple_msg *msg,
				  int segment, int newsize)
{
	...
	src = grumple_msg_buf(msg, segment + 1, 0);
	msg->lm_buflens[segment] = newsize;
	dst = grumple_msg_buf(msg, segment + 1, 0);
...
int sptlrpc_svc_unwrap_request(struct ptlrpc_request *req)
{
	...
	rc = __grumple_unpack_msg(msg, req->rq_reqdata_len);
	switch (rc) {
	case 1:
...
int sptlrpc_pack_user_desc(struct grumple_msg *msg, int offset)
{
	struct ptlrpc_user_desc *pud;
	int ngroups;

	pud = grumple_msg_buf(msg, offset, 0);
...
int sptlrpc_unpack_user_desc(struct grumple_msg *msg, int offset, int swabbed)
{
	struct ptlrpc_user_desc *pud;
	int i;
...
int bulk_sec_desc_unpack(struct grumple_msg *msg, int offset, int swabbed)
{
	struct ptlrpc_bulk_sec_desc *bsd;
	int size = msg->lm_buflens[offset];

	bsd = grumple_msg_buf(msg, offset, sizeof(*bsd));
	if (bsd == NULL) {
