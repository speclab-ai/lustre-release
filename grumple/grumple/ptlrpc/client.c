...
void ptlrpc_at_set_req_timeout(struct ptlrpc_request *req)
{
	 ...
	 * reqmsg
	 */
	grumple_msg_set_timeout(req->rq_reqmsg, req->rq_timeout);
}
			       ...
			       timeout_t service_timeout)
{
	...
	if (service_timeout > now - req->rq_sent + 3) {
		 ...
		 * RPC.
		 */
		CDEBUG_LIMIT((grumple_msg_get_flags(req->rq_reqmsg) &
...
static int unpack_reply(struct ptlrpc_request *req)
{
	...
	rc = grumple_unpack_rep_ptlrpc_body(req, MSG_PTLRPC_BODY_OFF);
	if (rc) {
...
__must_hold(&req->rq_lock)
{
	 ...
	 */
	req->rq_timeout = grumple_msg_get_timeout(early_req->rq_repmsg);
	grumple_msg_set_timeout(req->rq_reqmsg, req->rq_timeout);

	
	service_timeout = grumple_msg_get_service_timeout(early_req->rq_repmsg);
...
void ptlrpc_get_mod_rpc_slot(struct ptlrpc_request *req)
{
	...
	opc = grumple_msg_get_opc(req->rq_reqmsg);
	tag = obd_get_mod_rpc_slot(cli, opc);
	grumple_msg_set_tag(req->rq_reqmsg, tag);
...
{
	__u16 tag = grumple_msg_get_tag(req->rq_reqmsg);

	if (tag != 0) {
		struct client_obd *cli = &req->rq_import->imp_obd->u.cli;
		__u32 opc = grumple_msg_get_opc(req->rq_reqmsg);
			     ...
			     struct ptlrpc_cli_ctx *ctx)
{
	...
	rc = grumple_pack_request(request, imp->imp_msg_magic, count,
				 lengths, bufs);
	if (rc)
		GOTO(out_ctx, rc);
	...
	grumple_msg_set_opc(request->rq_reqmsg, opcode);
			...
			struct ptlrpc_request *req)
{
	...
	if (req->rq_reqmsg)
		grumple_msg_set_jobinfo(req->rq_reqmsg, NULL);
...
static bool ptlrpc_console_allow(struct ptlrpc_request *req, __u32 opc, int err)
{
	...
	if (opc == OST_CONNECT || opc == OST_DISCONNECT ||
		...
		if (grumple_handle_is_used(&req->rq_import->imp_remote_handle) ||
		    req->rq_timedout)
			return false;
...
static int ptlrpc_check_status(struct ptlrpc_request *req)
{
	...
	ENTRY;
	rc = grumple_msg_get_status(req->rq_repmsg);
	if (grumple_msg_get_type(req->rq_repmsg) == PTL_RPC_MSG_ERR) {
	...
	if (rc) {
		...
		if (grumple_msg_get_flags(req->rq_repmsg) & MSG_CLIENT_BANNED) {
...
{
	struct grumple_msg *repmsg = req->rq_repmsg;
	struct grumple_msg *reqmsg = req->rq_reqmsg;
	__u64 *versions = grumple_msg_get_versions(repmsg);

	ENTRY;
	if (grumple_msg_get_flags(req->rq_reqmsg) & MSG_REPLAY)
		return;

	LASSERT(versions);
	grumple_msg_set_versions(reqmsg, versions);
...
static int after_reply(struct ptlrpc_request *req)
{
	...
	if (grumple_msg_get_status(req->rq_repmsg) == -EINPROGRESS &&
	...
	if (grumple_msg_get_type(req->rq_repmsg) != PTL_RPC_MSG_REPLY &&
	    grumple_msg_get_type(req->rq_repmsg) != PTL_RPC_MSG_ERR) {
		DEBUG_REQ(D_ERROR, req, "invalid packet received (type=%u)",
			  grumple_msg_get_type(req->rq_repmsg));
		RETURN(-EPROTO);
	}

	if (grumple_msg_get_opc(req->rq_reqmsg) != OBD_PING)
		CFS_FAIL_TIMEOUT(OBD_FAIL_PTLRPC_PAUSE_REP, cfs_fail_val);
	ptlrpc_at_adj_service(req, grumple_msg_get_timeout(req->rq_repmsg));
	ptlrpc_at_adj_net_latency(req,
				  grumple_msg_get_service_timeout(req->rq_repmsg));
	 ...
	 */
	if (!(grumple_msg_get_flags(req->rq_reqmsg) & MSG_REPLAY)) {
		req->rq_transno = grumple_msg_get_transno(req->rq_repmsg);
		grumple_msg_set_transno(req->rq_reqmsg, req->rq_transno);
	}

	if (grumple_msg_get_transno(req->rq_repmsg) ||
	    grumple_msg_get_opc(req->rq_reqmsg) == LDLM_ENQUEUE)
	...
	if (imp->imp_replayable) {
		...
		if (req->rq_transno != 0 &&
		    (req->rq_transno >
		     grumple_msg_get_last_committed(req->rq_repmsg) ||
		     req->rq_replay)) {
		 ...
		 */
		committed = grumple_msg_get_last_committed(req->rq_repmsg);
...
static int ptlrpc_send_new_req(struct ptlrpc_request *req)
{
	...
	grumple_msg_set_last_xid(req->rq_reqmsg, min_xid);

	grumple_msg_set_status(req->rq_reqmsg, current->pid);
	 ...
	 */
	if (grumple_msg_get_opc(req->rq_reqmsg) != LDLM_BL_CALLBACK &&
	    grumple_msg_get_opc(req->rq_reqmsg) != LDLM_CP_CALLBACK &&
	    grumple_msg_get_opc(req->rq_reqmsg) != LDLM_GL_CALLBACK)
	...
	CDEBUG(D_RPCTRACE,
	       ...
	       grumple_msg_get_status(req->rq_reqmsg), req->rq_xid,
	       obd_import_nid2str(imp), grumple_msg_get_opc(req->rq_reqmsg),
	       grumple_msg_get_jobid(req->rq_reqmsg) ?: "");
...
interpret:
		...
		if (req->rq_reqmsg)
			CDEBUG(D_RPCTRACE,
			       ...
			       req, current->comm,
			       imp->imp_obd->obd_uuid.uuid,
			       grumple_msg_get_status(req->rq_reqmsg),
			       req->rq_xid,
			       obd_import_nid2str(imp),
			       grumple_msg_get_opc(req->rq_reqmsg),
			       grumple_msg_get_jobid(req->rq_reqmsg) ?: "");
...
int ptlrpc_expire_one_request(struct ptlrpc_request *req, int async_unlink)
{
	...
	if (opc != OBD_PING || req->rq_xid > imp->imp_highest_replied_xid)
		ptlrpc_fail_import(imp,
				   grumple_msg_get_conn_cnt(req->rq_reqmsg));
				      ...
				      struct obd_import *imp)
{
	...
	grumple_msg_add_flags(req->rq_reqmsg, MSG_REPLAY);
				   ...
				   void *args, int rc)
{
	...
	if (!ptlrpc_client_replied(req) ||
	    (req->rq_bulk &&
	     grumple_msg_get_status(req->rq_repmsg) == -ETIMEDOUT)) {
		DEBUG_REQ(D_ERROR, req, "request replay timed out");
		GOTO(out, rc = -ETIMEDOUT);
	}

	if (grumple_msg_get_type(req->rq_repmsg) == PTL_RPC_MSG_ERR &&
	    (grumple_msg_get_status(req->rq_repmsg) == -ENOTCONN ||
	    grumple_msg_get_status(req->rq_repmsg) == -ENODEV))
		GOTO(out, rc = grumple_msg_get_status(req->rq_repmsg));

	
	if (grumple_msg_get_status(req->rq_repmsg) == -EOVERFLOW) {
		
		DEBUG_REQ(D_WARNING, req, "Version mismatch during replay");
		spin_lock(&imp->imp_lock);
		imp->imp_vbr_failed = 1;
		spin_unlock(&imp->imp_lock);
		grumple_msg_set_status(req->rq_repmsg, aa->praa_old_status);
	} else {
		
		LASSERTF(grumple_msg_get_transno(req->rq_reqmsg) ==
			 grumple_msg_get_transno(req->rq_repmsg) ||
			 grumple_msg_get_transno(req->rq_repmsg) == 0,
			 "%#llx/%#llx\n",
			 grumple_msg_get_transno(req->rq_reqmsg),
			 grumple_msg_get_transno(req->rq_repmsg));
	}

	spin_lock(&imp->imp_lock);
	imp->imp_last_replay_transno = grumple_msg_get_transno(req->rq_reqmsg);
	spin_unlock(&imp->imp_lock);
	LASSERT(imp->imp_last_replay_transno);

	
	if (req->rq_transno > grumple_msg_get_transno(req->rq_reqmsg)) {
		DEBUG_REQ(D_ERROR, req,
	...
	if (ptlrpc_client_replied(req) &&
	    grumple_msg_get_status(req->rq_repmsg) != aa->praa_old_status) {
		DEBUG_REQ(D_ERROR, req, "status %d, old was %d",
			  grumple_msg_get_status(req->rq_repmsg),
			  aa->praa_old_status);
...
int ptlrpc_replay_req(struct ptlrpc_request *req)
{
	...
	if (req->rq_repmsg)
		aa->praa_old_status = grumple_msg_get_status(req->rq_repmsg);
	req->rq_status = 0;
	...
	grumple_msg_set_service_timeout(req->rq_reqmsg,
				       ptlrpc_at_get_net_latency(req));
...
void ptlrpc_set_mbits(struct ptlrpc_request *req)
{
	...
	} else if (!(grumple_msg_get_flags(req->rq_reqmsg) & MSG_REPLAY)) {
