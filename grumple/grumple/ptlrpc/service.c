 ...
 * @lock: pointer to grumple_handle (to be saved)
					 ...
					 struct ptlrpc_service_conf *conf)
{
	...
	/*
	 * NB: please see comments in grumple_lnet.h for definition
...
void ptlrpc_del_exp_list(struct ptlrpc_request *req)
{
	...
	if (likely(req->rq_reqmsg))
		tag = grumple_msg_get_tag(req->rq_reqmsg);
					...
					bool recovery)
{
	...
	if (recovery)
		netl = grumple_msg_get_service_timeout(req->rq_reqmsg);
	else
...
static int ptlrpc_check_req(struct ptlrpc_request *req)
{
	...
	if (unlikely(grumple_msg_get_conn_cnt(req->rq_reqmsg) <
	...
	if (unlikely(obd == NULL || obd->obd_fail)) {
		...
		rc = -ENODEV;
	} else if (grumple_msg_get_flags(req->rq_reqmsg) &
		...
		rc = -ENODEV;
	} else if (grumple_msg_get_transno(req->rq_reqmsg) != 0 &&
		   !test_bit(OBDF_RECOVERING, obd->obd_flags)) {
		DEBUG_REQ(D_ERROR, req,
...
static int ptlrpc_at_add_timed(struct ptlrpc_request *req)
{
	...
	if ((grumple_msghdr_get_flags(req->rq_reqmsg) & MSGHDR_AT_SUPPORT) == 0)
		return(-ENOSYS);
...
static int ptlrpc_at_send_early_reply(struct ptlrpc_request *req)
{
	...
	if ((grumple_msghdr_get_flags(req->rq_reqmsg) &
	     MSGHDR_AT_SUPPORT) == 0) {
	...
	*reqcopy = *req;
	...
	reqcopy->rq_export = class_conn2export(
			grumple_msg_get_handle(reqcopy->rq_reqmsg));
	...
	rc = grumple_pack_reply_flags(reqcopy, 1, NULL, NULL, LPRFL_EARLY_REPLY);
	if (rc)
		GOTO(out_put, rc);
	...
	/*
	 * Free the (early) reply state from grumple_pack_reply.
...
ptlrpc_server_mark_in_progress_obsolete(struct ptlrpc_request *req)
{
	...
	list_for_each_entry(tmp, &req->rq_export->exp_reg_rpcs, rq_exp_list) {
		if (tag == grumple_msg_get_tag(tmp->rq_reqmsg) &&
...
{
	int opc = grumple_msg_get_opc(req->rq_reqmsg);
				       ...
				       struct ptlrpc_thread *thread)
{
	...
	rc = grumple_unpack_req_ptlrpc_body(req, MSG_PTLRPC_BODY_OFF);
	if (rc) {
	...
	rc = -EINVAL;
	if (grumple_msg_get_type(req->rq_reqmsg) != PTL_RPC_MSG_REQUEST) {
		CERROR("wrong packet type received (type=%u) from %s\n",
		       grumple_msg_get_type(req->rq_reqmsg),
	...
	req->rq_export = class_conn2export(
		grumple_msg_get_handle(req->rq_reqmsg));
	if (req->rq_export) {
	...
	deadline = (grumple_msghdr_get_flags(req->rq_reqmsg) &
		    MSGHDR_AT_SUPPORT) ?
		    
		    grumple_msg_get_timeout(req->rq_reqmsg) : obd_timeout;
	...
	if (opc != OST_CONNECT && opc != MDS_CONNECT &&
		...
		if (exp_connect_flags2(req->rq_export) & OBD_CONNECT2_REP_MBITS)
			req->rq_rep_mbits = grumple_msg_get_mbits(req->rq_reqmsg);
	}
					...
					struct ptlrpc_thread *thread)
{
	...
	op = grumple_msg_get_opc(request->rq_reqmsg);
	...
	CDEBUG(D_RPCTRACE,
		...
		refcount_read(&request->rq_export->exp_handle.h_ref) : -99),
	       grumple_msg_get_status(request->rq_reqmsg), request->rq_xid,
	       libcfs_idstr(&request->rq_peer), op,
	       grumple_msg_get_jobid(request->rq_reqmsg) ?: "");
...
put_conn:
	...
	CDEBUG(D_RPCTRACE,
	       ...
	       (request->rq_export ?
		refcount_read(&request->rq_export->exp_handle.h_ref) : -99),
	       grumple_msg_get_status(request->rq_reqmsg),
	       request->rq_xid,
	       libcfs_idstr(&request->rq_peer), op,
	       grumple_msg_get_jobid(request->rq_reqmsg) ?: "",
	       timediff_usecs,
	       arrived_usecs,
	       (request->rq_repmsg ?
	       grumple_msg_get_transno(request->rq_repmsg) :
	       request->rq_transno),
	       request->rq_status,
	       (request->rq_repmsg ?
	       grumple_msg_get_status(request->rq_repmsg) : -999));
