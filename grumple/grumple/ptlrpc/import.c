...
static void import_set_state(struct obd_import *imp,
			     enum grumple_imp_state new_state)
{
...
int ptlrpc_connect_import_locked(struct obd_import *imp)
{
	...
	if (!grumple_handle_is_used(&imp->imp_remote_handle))
		initial_connect = 1;
	else
	 ...
	 */
	grumple_msg_set_service_timeout(request->rq_reqmsg,
	...
	if (aa->pcaa_initial_connect) {
		...
		spin_unlock(&imp->imp_lock);
		grumple_msg_add_op_flags(request->rq_reqmsg,
					MSG_CONNECT_INITIAL);
	}
				    ...
				    void *data, int rc)
{
	...
	msg_flags = grumple_msg_get_op_flags(request->rq_repmsg);
	 ...
	 */
	service_timeout = grumple_msg_get_service_timeout(request->rq_repmsg);
	...
	if (MSG_CONNECT_RECONNECT & msg_flags) {
		...
		if (!memcmp(&old_hdl, grumple_msg_get_handle(request->rq_repmsg),
			    sizeof(old_hdl))) {
	...
	if (grumple_msg_get_last_committed(request->rq_repmsg) > 0 &&
	    grumple_msg_get_last_committed(request->rq_repmsg) <
		...
		CERROR("%s: went back in time (transno %lld was previously committed, server now claims %lld)!\n",
		       ...
		       grumple_msg_get_last_committed(request->rq_repmsg));
		if (!printed) {
			CERROR("For further information, see http:
			printed = true;
		}
	}
