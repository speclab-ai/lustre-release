...
static int plain_unpack_bsd(struct grumple_msg *msg, int swabbed)
{
	...
	bsd = grumple_msg_buf(msg, PLAIN_PACK_BULK_OFF, PLAIN_BSD_SIZE);
	if (bsd == NULL) {
		CERROR("bulk sec desc has short size %d\n",
		       grumple_msg_buflen(msg, PLAIN_PACK_BULK_OFF));
		return -EPROTO;
	}
...
int plain_ctx_sign(struct ptlrpc_cli_ctx *ctx, struct ptlrpc_request *req)
{
	...
	phdr = grumple_msg_buf(msg, PLAIN_PACK_HDR_OFF, 0);
	phdr->ph_ver = 0;
	phdr->ph_flags = 0;
	...
	req->rq_reqdata_len = grumple_msg_size_v2(msg->lm_bufcount,
						 msg->lm_buflens);
	RETURN(0);
}
...
int plain_ctx_verify(struct ptlrpc_cli_ctx *ctx, struct ptlrpc_request *req)
{
	...
	phdr = grumple_msg_buf(msg, PLAIN_PACK_HDR_OFF, sizeof(*phdr));
	if (phdr == NULL) {
	...
	if (unlikely(req->rq_early)) {
		__u32 cksum = grumple_msg_calc_cksum(msg, PLAIN_PACK_MSG_OFF);
	...
	req->rq_repmsg = grumple_msg_buf(msg, PLAIN_PACK_MSG_OFF, 0);
	req->rq_replen = grumple_msg_buflen(msg, PLAIN_PACK_MSG_OFF);
	RETURN(0);
}
			...
			struct ptlrpc_bulk_desc *desc)
{
	...
	bsd = grumple_msg_buf(req->rq_reqbuf, PLAIN_PACK_BULK_OFF, 0);
			  ...
			  struct ptlrpc_bulk_desc *desc)
{
	...
	bsdv = grumple_msg_buf(req->rq_repdata, PLAIN_PACK_BULK_OFF, 0);
		       ...
		       int msgsize)
{
	...
	alloc_len = grumple_msg_size_v2(PLAIN_PACK_SEGMENTS, buflens);
	...
	grumple_init_msg_v2(req->rq_reqbuf, PLAIN_PACK_SEGMENTS, buflens, NULL);
	req->rq_reqmsg = grumple_msg_buf(req->rq_reqbuf, PLAIN_PACK_MSG_OFF, 0);
			 ...
			 int segment, int newsize)
{
	...
	LASSERT(grumple_msg_buf(req->rq_reqbuf, PLAIN_PACK_MSG_OFF, 0) ==
		req->rq_reqmsg);

	
	oldsize = req->rq_reqmsg->lm_buflens[segment];
	req->rq_reqmsg->lm_buflens[segment] = newsize;
	newmsg_size = grumple_msg_size_v2(req->rq_reqmsg->lm_bufcount,
	...
	newbuf_size = grumple_msg_size_v2(req->rq_reqbuf->lm_bufcount,
	...
	if (req->rq_reqbuf_len < newbuf_size) {
		...
		req->rq_reqmsg = grumple_msg_buf(req->rq_reqbuf,
...
static int plain_accept(struct ptlrpc_request *req)
{
	...
	if (phdr->ph_flags & PLAIN_FL_USER) {
		...
		req->rq_pack_udesc = 1;
		req->rq_user_desc = grumple_msg_buf(msg, PLAIN_PACK_USER_OFF, 0);
	}
	...
	req->rq_reqmsg = grumple_msg_buf(msg, PLAIN_PACK_MSG_OFF, 0);
...
int plain_alloc_rs(struct ptlrpc_request *req, int msgsize)
{
	...
	rs_size += grumple_msg_size_v2(PLAIN_PACK_SEGMENTS, buflens);
	...
	grumple_init_msg_v2(rs->rs_repbuf, PLAIN_PACK_SEGMENTS, buflens, NULL);
	rs->rs_msg = grumple_msg_buf_v2(rs->rs_repbuf, PLAIN_PACK_MSG_OFF, 0);
...
int plain_authorize(struct ptlrpc_request *req)
{
	...
	struct grumple_msg_v2 *msg = rs->rs_repbuf;
	struct plain_header *phdr;
	int len;
	...
	if (req->rq_replen != msg->lm_buflens[PLAIN_PACK_MSG_OFF])
		len = grumple_shrink_msg(msg, PLAIN_PACK_MSG_OFF,
					req->rq_replen, 1);
	else
		len = grumple_msg_size_v2(msg->lm_bufcount, msg->lm_buflens);
	...
	if (likely(req->rq_packed_final)) {
		...
		msg->lm_cksum = grumple_msg_calc_cksum(msg, PLAIN_PACK_MSG_OFF);
	}
			  ...
			  struct ptlrpc_bulk_desc *desc)
{
	...
	bsdr = grumple_msg_buf(req->rq_reqbuf, PLAIN_PACK_BULK_OFF, 0);
	tokenr = (struct plain_bulk_token *) bsdr->bsd_data;
	bsdv = grumple_msg_buf(rs->rs_repbuf, PLAIN_PACK_BULK_OFF, 0);
...
int sptlrpc_plain_init(void)
{
	...
	buflens[PLAIN_PACK_MSG_OFF] = grumple_msg_early_size;
	plain_at_offset = grumple_msg_size_v2(PLAIN_PACK_SEGMENTS, buflens);
