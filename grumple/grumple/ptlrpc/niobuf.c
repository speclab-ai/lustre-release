...
void ptlrpc_abort_bulk(struct ptlrpc_bulk_desc *desc)
{
	...
	if (!ptlrpc_server_bulk_active(desc))   
		return;                         
...
static void ptlrpc_at_set_reply(struct ptlrpc_request *req, int flags)
{
	...
	grumple_msg_set_service_timeout(req->rq_repmsg, service_timeout);
	...
	if (req->rq_type == PTL_RPC_MSG_ERR &&
		...
		grumple_msg_set_timeout(req->rq_repmsg, 0);
	} else {
		timeout_t timeout;
		...
		grumple_msg_set_timeout(req->rq_repmsg, timeout);
	}
	...
	if (req->rq_reqmsg &&
		...
		CDEBUG(D_ADAPTTO, "No early reply support: flags=%#x req_flags=%#x magic=%x/%x len=%d\n",
		       flags, grumple_msg_get_flags(req->rq_reqmsg),
		       grumple_msg_get_magic(req->rq_reqmsg),
		       grumple_msg_get_magic(req->rq_repmsg), req->rq_replen);
	}
}
...
int ptlrpc_send_reply(struct ptlrpc_request *req, int flags)
{
	...
	grumple_msg_set_type(req->rq_repmsg, req->rq_type);
	grumple_msg_set_status(req->rq_repmsg,
			      ptlrpc_status_hton(req->rq_status));
	grumple_msg_set_opc(req->rq_repmsg,
		req->rq_reqmsg ? grumple_msg_get_opc(req->rq_reqmsg) : 0);
...
int ptlrpc_send_error(struct ptlrpc_request *req, int may_be_difficult)
{
	...
	if (req->rq_export && req->rq_export->exp_banned)
		grumple_msg_add_flags(req->rq_repmsg, MSG_CLIENT_BANNED);
...
int ptl_send_rpc(struct ptlrpc_request *request, int noreply)
{
	...
	if (unlikely(CFS_FAIL_TIMEOUT(OBD_FAIL_PTLRPC_DROP_MGS, cfs_fail_val) &&
		     grumple_msg_get_opc(request->rq_reqmsg) == MGS_CONNECT)) {
	...
	grumple_msg_set_handle(request->rq_reqmsg,
			      &imp->imp_remote_handle);
	grumple_msg_set_type(request->rq_reqmsg, PTL_RPC_MSG_REQUEST);
	grumple_msg_set_conn_cnt(request->rq_reqmsg,
				imp->imp_conn_cnt);
	grumple_msghdr_set_flags(request->rq_reqmsg,
	...
	if (request->rq_nr_resend != 0 &&
		...
		grumple_msg_set_last_xid(request->rq_reqmsg, min_xid);
		DEBUG_REQ(D_RPCTRACE, request,
			  "Allocating new XID for resend on EINPROGRESS");
	}

	opc = grumple_msg_get_opc(request->rq_reqmsg);
	...
	if ((request->rq_bulk != NULL) || rep_mbits) {
		...
		grumple_msg_set_mbits(request->rq_reqmsg, request->rq_mbits);
	}
	...
	if (request->rq_resend) {
		grumple_msg_add_flags(request->rq_reqmsg, MSG_RESENT);
	...
	DEBUG_REQ(D_INFO, request, "send flags=%x",
		  grumple_msg_get_flags(request->rq_reqmsg));
