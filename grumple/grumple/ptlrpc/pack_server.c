...
static void
grumple_swab_object_update_result_no_len(struct object_update_result *our)
{
...
int grumple_swab_object_update_reply(struct object_update_reply *our, __u32 len)
{
	...
	for (i = 0; i < our->ourp_count; i++) {
		...
		if (ourp == NULL)
			return -EPROTO;
		grumple_swab_object_update_result_no_len(ourp);
	}
...
EXPORT_SYMBOL(grumple_swab_out_update_header);
...
EXPORT_SYMBOL(grumple_swab_out_update_buffer);
