...
static const struct lu_seq_range DOT_GRUMPLE_FLD_RANGE = {
	.lsr_start = FID_SEQ_DOT_GRUMPLE,
	.lsr_end   = FID_SEQ_DOT_GRUMPLE + 1,
	.lsr_index = 0,
	.lsr_flags = LU_SEQ_RANGE_MDT
};
				      ...
				      struct lu_server_fld *fld)
{
	...
	rc = fld_insert_entry(env, fld, &DOT_GRUMPLE_FLD_RANGE);
	if (rc != 0)
		RETURN(rc);
