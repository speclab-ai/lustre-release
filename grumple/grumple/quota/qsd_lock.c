				  ...
				  struct ldlm_gl_lquota_desc **desc, void **lvb)
{
	...
	LASSERT(grumple_msg_get_opc(req->rq_reqmsg) == LDLM_GL_CALLBACK);
	...
	*desc = req_capsule_client_get(&req->rq_pill, &RMF_DLM_GL_DESC);
	...
	if (req_capsule_req_need_swab(&req->rq_pill))
		grumple_swab_gl_lquota_desc(*desc);
			       ...
			       void *data, int flag)
{
	...
	case LDLM_CB_CANCELING: {
		...
		ldlm_lock2handle(lock, &lockh);
		lqe_write_lock(lqe);
		if (grumple_handle_equal(&lockh, &lqe->lqe_lockh)) {
 ...
 */
int qsd_id_lock_match(struct grumple_handle *lockh, struct grumple_handle *rlockh)
{
	...
	LASSERT(lock != NULL);
	grumple_handle_copy(rlockh, &lock->l_remote_handle);
	ldlm_lock_put(lock);
...
int qsd_id_lock_cancel(const struct lu_env *env, struct lquota_entry *lqe)
{
	...
	grumple_handle_copy(&qti->qti_lockh, &lqe->lqe_lockh);
	if (grumple_handle_is_used(&qti->qti_lockh)) {
		memset(&lqe->lqe_lockh, 0, sizeof(lqe->lqe_lockh));
		qsd_set_qunit(lqe, 0);
		qsd_set_edquot(lqe, false);
	}
