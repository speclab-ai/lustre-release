 ...
 */
static int qsd_ready(struct lquota_entry *lqe, struct grumple_handle *lockh)
{
	...
	grumple_handle_copy(lockh, &qqi->qqi_lockh);
	read_unlock(&qsd->qsd_lock);
	...
	grumple_handle_copy(lockh, &lock->l_remote_handle);
	ldlm_lock_put(lock);
...
static bool qsd_calc_adjust(struct lquota_entry *lqe, struct quota_body *qbody)
{
	...
	if (!grumple_handle_is_used(&lqe->lqe_lockh)) {
			       ...
			       void *arg, int ret)
{
	...
	if (grumple_handle_is_used(lockh) &&
	    !grumple_handle_equal(lockh, &lqe->lqe_lockh))
		grumple_handle_copy(&lqe->lqe_lockh, lockh);
