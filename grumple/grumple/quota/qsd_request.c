...
struct qsd_async_args {
	...
	struct grumple_handle   aa_lockh;
...
int qsd_send_dqacq(const struct lu_env *env, struct obd_export *exp,
		   ...
		   struct grumple_handle *lockh, struct lquota_entry *lqe)
{
	...
	*req_qbody = *qbody;
	...
	grumple_handle_copy(&aa->aa_lockh, lockh);
		    ...
		    struct lquota_lvb *lvb, void *arg)
{
	...
	LASSERT(exp != NULL);
	LASSERT(!grumple_handle_is_used(&qbody->qb_lockh));
	...
	*req_qbody = *qbody;
	...
	aa->aa_lvb = lvb;
	aa->aa_completion = completion;
	grumple_handle_copy(&aa->aa_lockh, &qti->qti_lockh);
