 ...
 * in grumple_quota.h). The QMT also borrows the LDLM namespace from the MDT.
 *
			      ...
			      struct grumple_cfg *cfg)
{
	...
	obd = class_name2obd(grumple_cfg_string(cfg, 3));
	if (obd == NULL) {
		CERROR("%s: can't locate backend osd device: %s\n",
		       qmt->qmt_svname, grumple_cfg_string(cfg, 3));
			    ...
			    struct lu_device_type *ldt, struct grumple_cfg *cfg)
{
	...
	char			*svname = grumple_cfg_string(cfg, 0);
	int			 rc;
	ENTRY;
	...
	mdt_obd = class_name2obd(grumple_cfg_string(cfg, 2));
	if (mdt_obd == NULL)
		RETURN(-ENOENT);
