			     ...
			     struct grumple_handle *lockh)
{
	...
	if (!oscl->ols_glimpse) {
		...
		grumple_handle_copy(&oscl->ols_handle, lockh);
 ...
 * lock.
 */
static int mdc_lock_upcall(void *cookie, struct grumple_handle *lockh,
			   int errcode)
{
...
static int mdc_enqueue_fini(struct obd_export *exp, struct ptlrpc_request *req,
			    ...
			    struct grumple_handle *lockh, enum ldlm_mode mode,
...
static int mdc_get_lock_handle(const struct lu_env *env, struct osc_object *osc,
			       pgoff_t index, struct grumple_handle *lh)
{
	struct ldlm_lock *lock;
