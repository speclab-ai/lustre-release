			 ...
			 struct lu_fid *rootfid)
{
	...
	*rootfid = body->mbo_fid1;
	CDEBUG(D_NET, "root fid="DFID", last_committed=%llu\n",
	       PFID(rootfid), grumple_msg_get_last_committed(req->rq_repmsg));
	EXIT;
out:
	ptlrpc_req_put(req);
...
static int mdc_get_grumple_md(struct obd_export *exp, struct req_capsule *pill,
...
void mdc_replay_open(struct ptlrpc_request *req)
{
	...
	struct grumple_handle old_open_handle = { };
	struct mdt_body *body;
	...
	if (req->rq_early_free_repbuf) {
		struct grumple_handle *file_open_handle;
	...
	if (close_req) {
		__u32 opc = grumple_msg_get_opc(close_req->rq_reqmsg);
		     ...
		     struct md_open_data *mod, struct ptlrpc_request **request)
{
	...
	*request = NULL;
	...
	grumple_msg_set_projid(req->rq_reqmsg, op_data->op_projid);
				...
				struct hsm_progress_kernel *hpk)
{
	...
	*req_hpk = *hpk;
	req_hpk->hpk_errval = grumple_errno_hton(hpk->hpk_errval);
...
static void grumple_swab_hai(struct hsm_action_item *h)
{
	__swab32s(&h->hai_len);
	__swab32s(&h->hai_action);
	grumple_swab_lu_fid(&h->hai_fid);
	grumple_swab_lu_fid(&h->hai_dfid);
...
static void grumple_swab_hal(struct hsm_action_list *h)
{
	...
	for (i = 0; i < h->hal_count; i++, hai = hai_next(hai))
		grumple_swab_hai(hai);
}

static void grumple_swab_kuch(struct kuc_hdr *l)
{
				 ...
				 size_t len, void *val)
{
	...
	if (lh->kuc_magic == __swab16(KUC_MAGIC)) {
		grumple_swab_kuch(lh);
		grumple_swab_hal(hal);
...
int mdc_setup(struct obd_device *obd, struct grumple_cfg *cfg)
{
	int rc;
...
static const struct md_ops mdc_md_ops = {
	...
	.m_get_grumple_md    = mdc_get_grumple_md,
