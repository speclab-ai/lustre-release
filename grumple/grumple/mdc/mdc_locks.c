...
int mdc_set_lock_data(struct obd_export *exp, const struct grumple_handle *lockh,
		      void *data, enum mds_ibits_locks *bits)
{
	...
	if (!grumple_handle_is_used(lockh))
		RETURN(0);
		     ...
		     struct md_op_data *op_data, __u32 acl_bufsize)
{
	...
	if (fid_is_sane(&op_data->op_fid2)) {
		if (it->it_open_flags & MDS_OPEN_LEASE) { 
...
int mdc_finish_enqueue(struct obd_export *exp,
		       ...
		       struct lookup_intent *it,
		       struct grumple_handle *lockh, int rc)
{
...
static int mdc_finish_intent_lock(struct obd_export *exp,
				  ...
				  struct grumple_handle *lockh)
{
	struct grumple_handle old_lock;
	struct ldlm_lock *lock;
	int rc = 0;
					      ...
					      void *args, int rc)
{
	...
	struct grumple_handle *lockh = &item->mop_lockh;
	struct req_capsule *pill = &req->rq_pill;
	struct ldlm_reply *lockrep;
