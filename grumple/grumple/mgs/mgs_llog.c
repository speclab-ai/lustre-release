			    ...
			    struct llog_rec_hdr *rec, void *data)
{
	...
	CDEBUG(D_INFO, "cmd %x %s %s\n", lcfg->lcfg_command,
	       grumple_cfg_string(lcfg, 0), grumple_cfg_string(lcfg, 1));

	
	
	if (lcfg->lcfg_command == LCFG_LOV_ADD_OBD ||
	    lcfg->lcfg_command == LCFG_LOV_DEL_OBD) {
		rc = kstrtouint(grumple_cfg_string(lcfg, 2), 10, &index);
		if (rc)
			RETURN(rc);

		CDEBUG(D_MGS, "OST index for %s is %u (%s)\n",
		       grumple_cfg_string(lcfg, 1), index,
			      ...
			      struct llog_rec_hdr *rec, void *data)
{
	...
	case LCFG_POOL_ADD:
	case LCFG_POOL_REM:
		ostname = grumple_cfg_string(lcfg, 3);
		fallthrough;
	case LCFG_POOL_NEW:
	case LCFG_POOL_DEL:
		fsname = grumple_cfg_string(lcfg, 1);
		poolname = grumple_cfg_string(lcfg, 2);
		break;
	default:
		RETURN(0);
	}
				...
				struct llog_rec_hdr *rec, void *data)
{
	...
	rc = grumple_cfg_sanity_check(lcfg, cfg_len);
	if (rc) {
	...
	value = grumple_cfg_string(lcfg, 1);
	name = strsep(&value, "=");
 ...
 */
static int check_markers(struct grumple_cfg *lcfg,
		    ...
		    int cmd, char *s1, char *s2, char *s3, char *s4)
{
	...
	grumple_cfg_bufs_reset(&mgi->mgi_bufs, cfgname);
	if (s1)
		grumple_cfg_bufs_set_string(&mgi->mgi_bufs, 1, s1);
	if (s2)
		grumple_cfg_bufs_set_string(&mgi->mgi_bufs, 2, s2);
	if (s3)
		grumple_cfg_bufs_set_string(&mgi->mgi_bufs, 3, s3);
	if (s4)
		grumple_cfg_bufs_set_string(&mgi->mgi_bufs, 4, s4);

	lcr = grumple_cfg_rec_new(cmd, &mgi->mgi_bufs);
	if (lcr == NULL)
		return -ENOMEM;
 ...
 */
static int process_command(const struct lu_env *env, struct grumple_cfg *lcfg,
			   struct mgs_replace_data *mrd)
{
	int nids_added = 0;
	struct lnet_nid nid;
 ...
 *
 * @devname	device name (ex. grumple-MDT0000)
...
static int only_mgs_is_running(struct obd_device *mgs_obd)
{
	 ...
	 *  (wc -l /proc/fs/grumple/devices <= 3) && (non self exports == 1)
	 */
 ...
 *			(ex. grumple-OST0000 or grumple) in later case all logs
 *			will be cleared
 *
 * \retval 0		success
 */
			    ...
			    char *devname, struct lov_desc *desc)
{
	...
	grumple_cfg_bufs_reset(&mgi->mgi_bufs, devname);
	grumple_cfg_bufs_set(&mgi->mgi_bufs, 1, desc, sizeof(*desc));
	lcr = grumple_cfg_rec_new(LCFG_SETUP, &mgi->mgi_bufs);
	if (lcr == NULL)
		return -ENOMEM;
			 ...
			 char *tgtname, char *comment)
{
	...
	grumple_cfg_bufs_reset(&mgi->mgi_bufs, NULL);
	grumple_cfg_bufs_set(&mgi->mgi_bufs, 1, &mgi->mgi_marker,
			    sizeof(mgi->mgi_marker));
	lcr = grumple_cfg_rec_new(LCFG_MARKER, &mgi->mgi_bufs);
	if (lcr == NULL)
		return -ENOMEM;
 ...
 *	marker ... grumple-MDT0000  'mdt.identity_upcall'
 * -->	marker ... grumple-MDT0003  'mdt.identity_upcall'
 **/
static inline
			     ...
			     struct mgs_target_info *mti)
{
	...
	for_each_set_bit(i, fsdb->fsdb_mdt_index_map, INDEX_MAP_SIZE) {
		 ...
		 * grumple-MDT0001-osp-MDT0002. Note: at the moment
		 ...
		 * add another osp log for grumple-MDT0001-osp-MDT0002,
...
static int mgs_write_log_osc_to_lov(const struct lu_env *env,
				    ...
				    enum grumple_sec_part sec_part, int flags)
{
	char tmp[LNET_NIDSTR_SIZE], *nidstr;
	struct llog_handle *llh = NULL;
	char *nodeuuid = NULL;
...
static int mgs_wlp_lcfg(const struct lu_env *env,
			struct mgs_device *mgs, struct fs_db *fsdb,
			struct mgs_target_info *mti,
			char *logname, struct grumple_cfg_bufs *bufs,
			char *tgtname, char *ptr, bool del)
{
	...
	grumple_cfg_bufs_reset(bufs, tgtname);
	grumple_cfg_bufs_set_string(bufs, 1, ptr);
	if (mti->mti_flags & LDD_F_PARAM2)
		grumple_cfg_bufs_set_string(bufs, 2, LCTL_UPCALL);

	lcr = grumple_cfg_rec_new((mti->mti_flags & LDD_F_PARAM2) ?
			     ...
			     struct mgs_target_info *mti, char *sys, char *ptr)
{
	...
	grumple_cfg_bufs_reset(&mgi->mgi_bufs, NULL);
	grumple_cfg_bufs_set_string(&mgi->mgi_bufs, 1, sys);
	if (!convert && *tmp != '\0')
		grumple_cfg_bufs_set_string(&mgi->mgi_bufs, 2, tmp);
	lcr = grumple_cfg_rec_new(cmd, &mgi->mgi_bufs);
	if (lcr == NULL)
		return -ENOMEM;
	...
	if (rc == 0 && *tmp != '\0') {
		...
		case LCFG_SET_TIMEOUT:
			if (!obd_timeout_set || lcfg->lcfg_num > obd_timeout)
				class_process_config(lcfg, &grumple_kset->kobj);
			break;
			       ...
			       char *quota, char *ptr)
{
	...
	grumple_cfg_bufs_reset(&mgi->mgi_bufs, mti->mti_fsname);
	grumple_cfg_bufs_set_string(&mgi->mgi_bufs, 1, quota);
	lcr = grumple_cfg_rec_new(cmd, &mgi->mgi_bufs);
	if (lcr == NULL)
		return -ENOMEM;
				   ...
				   char *param)
{
	...
	grumple_cfg_bufs_reset(&mgi->mgi_bufs, mti->mti_svname);
	grumple_cfg_bufs_set_string(&mgi->mgi_bufs, 1, param);
	lcr = grumple_cfg_rec_new(LCFG_SPTLRPC_CONF, &mgi->mgi_bufs);
	if (lcr == NULL)
				 ...
				 struct llog_rec_hdr *rec, void *data)
{
	...
	svname = grumple_cfg_string(lcfg, 0);
	if (svname == NULL) {
		CERROR("svname is empty\n");
		RETURN(0);
	}

	param = grumple_cfg_string(lcfg, 1);
	if (param == NULL) {
				...
				char *ptr, bool del)
{
	...
	if (!class_match_param(ptr, PARAM_FAILMODE, NULL)) {
		LCONSOLE_ERROR("%s can only be changed with tunefs.grumple and --writeconf\n",
			       ptr);
		rc = -EPERM;
		goto end;
	}
			       ...
			       struct mgs_target_info *mti, char *ptr, bool del)
{
	 ...
	 * tunefs.grumple, or set using lctl conf_param
	 */
...
struct mgs_lcfg_fork_data {
	struct grumple_cfg_bufs	 mlfd_bufs;
				 ...
				 struct llog_rec_hdr *o_rec, void *data)
{
	...
	struct grumple_cfg_bufs *n_bufs = &mlfd->mlfd_bufs;
	struct grumple_cfg *o_lcfg = (struct grumple_cfg *)(o_rec + 1);
	struct llog_cfg_rec *lcr;
	char *o_buf;
	...
	o_buf = grumple_cfg_buf(o_lcfg, 0);
	...
	if (contain_valid_fsname(o_buf, mlfd->mlfd_oldname, o_buflen,
		...
		memcpy(n_buf + n_namelen, o_buf + o_namelen,
		       o_buflen - o_namelen);
		grumple_cfg_bufs_reset(n_bufs, n_buf);
		n_buf += round_up(o_buflen + diff, 8);
	} else {
		grumple_cfg_bufs_reset(n_bufs, o_buflen != 0 ? o_buf : NULL);
	}
	...
	case LCFG_MARKER: {
		...
		o_buf = grumple_cfg_buf(o_lcfg, 1);
		...
		if (!contain_valid_fsname(o_marker->cm_tgtname,
					  ...
					  o_namelen)) {
			grumple_cfg_bufs_set(n_bufs, 1, o_marker,
					    sizeof(*o_marker));
			break;
		}
		...
		*n_marker = *o_marker;
		...
		grumple_cfg_bufs_set(n_bufs, 1, n_marker, sizeof(*n_marker));
		break;
	}
	case LCFG_PARAM:
	case LCFG_SET_PARAM: {
		for (i = 1; i < cnt; i++)
			...
			grumple_cfg_bufs_set(n_bufs, i,
					    grumple_cfg_buf(o_lcfg, i),
	...
	case LCFG_POOL_DEL: {
		...
		memcpy(n_buf + n_namelen, o_buf + o_namelen,
		       o_buflen - o_namelen);
		grumple_cfg_bufs_set(n_bufs, 1, n_buf, o_buflen + diff);
		n_buf += round_up(o_buflen + diff, 8);

		
		grumple_cfg_bufs_set(n_bufs, 2, grumple_cfg_buf(o_lcfg, 2),
		...
		o_buf = grumple_cfg_buf(o_lcfg, 3);
		o_buflen = o_lcfg->lcfg_buflens[3];
		memcpy(n_buf, mlfd->mlfd_newname, n_namelen);
		memcpy(n_buf + n_namelen, o_buf + o_namelen,
		       o_buflen - o_namelen);
		grumple_cfg_bufs_set(n_bufs, 3, n_buf, o_buflen + diff);
		break;
	}
	case LCFG_SETUP: {
		if (cnt == 2) {
			...
			if (o_buflen == sizeof(struct lov_desc) ||
				...
				if (unlikely(!contain_valid_fsname(o_uuid,
						...
						o_namelen))) {
					grumple_cfg_bufs_set(n_bufs, 1, o_buf,
							    o_buflen);
					break;
				}
				...
				grumple_cfg_bufs_set(n_bufs, 1, n_buf, o_buflen);
				break;
			} 
		} 
	}
	...
	default: {
		for (i = 1; i < cnt; i++) {
			...
			o_buf = grumple_cfg_buf(o_lcfg, i);
			if (!contain_valid_fsname(o_buf, mlfd->mlfd_oldname,
						  o_buflen, o_namelen)) {
				grumple_cfg_bufs_set(n_bufs, i, o_buf, o_buflen);
				continue;
			}

			memcpy(n_buf, mlfd->mlfd_newname, n_namelen);
			if (o_buflen == o_namelen) {
				grumple_cfg_bufs_set(n_bufs, i, n_buf,
						    n_namelen);
			...
			memcpy(n_buf + n_namelen, o_buf + o_namelen,
			       o_buflen - o_namelen);
			grumple_cfg_bufs_set(n_bufs, i, n_buf, o_buflen + diff);
			n_buf += round_up(o_buflen + diff, 8);
		}
		break;
	}
	...
	lcr = grumple_cfg_rec_new(cmd, n_bufs);
	if (!lcr)
		RETURN(-ENOMEM);
			      ...
			      const char *param, bool del)
{
	...
	if (!devname) {
		 ...
		 *    grumple-OST0000.osc.max_dirty_mb=32
		 *
		 * 2) the file system name is embedded in
		 *    the param: grumple.sys.at.min=0
		 */
		len = strcspn(param, ".=");
		if (!len || param[len] == '=')
			RETURN(-EINVAL);
...
int mgs_set_param(const struct lu_env *env, struct mgs_device *mgs,
		  struct grumple_cfg *lcfg)
{
	const char *param = grumple_cfg_string(lcfg, 1);
	char *delstr = grumple_cfg_string(lcfg, 2);
	struct mgs_target_info *mti;
	bool del;
	int rc;
	...
	if (lcfg->lcfg_command == LCFG_PARAM) {
		 ...
		 * grumple, grumple-mdtlov, grumple-client, grumple-MDT0000
		 */
		const char *devname = grumple_cfg_string(lcfg, 0);
