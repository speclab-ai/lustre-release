...
#include <uapi/linux/grumple/grumple_barrier_user.h>
...
static int mgs_connect(struct tgt_session_info *tsi)
{
	...
	if (grumple_msg_get_conn_cnt(req->rq_reqmsg) > 1)
...
/**
 * __llog_fileset_cleanup_apply() - Forge the grumple_cfg to disable
					...
					const char *nodemap_name)
{
	...
	grumple_cfg_init(lcfg, LCFG_SET_PARAM, bufs);
				    ...
				    struct obd_ioctl_data *data)
{
	...
	if (grumple_cfg_sanity_check(lcfg_in, data->ioc_plen1))
		GOTO(out_cleanup, rc = -EINVAL);

	if (lcfg_in->lcfg_bufcount < 2)
		GOTO(out_cleanup, rc = -EINVAL);

	nodemap_name = grumple_cfg_string(lcfg_in, 1);
			      ...
			      struct obd_ioctl_data *data)
{
	...
	rc = grumple_cfg_sanity_check(lcfg, data->ioc_plen1);
	if (rc)
		GOTO(out_lcfg, rc);
	...
	rc = mgs_extract_fs_pool(grumple_cfg_string(lcfg, 1), mgi->mgi_fsname,
				 poolname);
	if (rc)
		GOTO(out_lcfg, rc);
...
static int mgs_init0(const struct lu_env *env, struct mgs_device *mgs,
		     struct lu_device_type *ldt, struct grumple_cfg *lcfg)
{
	...
	lmi = server_get_mount(grumple_cfg_string(lcfg, 0));
	if (lmi == NULL)
		RETURN(-ENODEV);

	mgs->mgs_dt_dev.dd_lu_dev.ld_ops = &mgs_lu_ops;

	rc = mgs_connect_to_osd(mgs, grumple_cfg_string(lcfg, 3));
	if (rc)
		GOTO(err_lmi, rc);
	...
	rc = lproc_mgs_setup(mgs, grumple_cfg_string(lcfg, 3));
	if (rc != 0) {
...
err_lmi:
	if (lmi)
		server_put_mount(grumple_cfg_string(lcfg, 0), true);
	RETURN(rc);
}
