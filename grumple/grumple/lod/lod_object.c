 ...
 * described in the file grumple/include/dt_object.h and in
				  ...
				  struct thandle *th)
{
	...
	if (magic == __swab32(LOV_USER_MAGIC_COMP_V1)) {
		grumple_swab_lov_comp_md_v1(comp_v1);
		magic = comp_v1->lcm_magic;
	}
				  ...
				  struct thandle *th)
{
	...
	/* Please update allowed_grumple_lov macro if op
	 * groks more values in the future
	 */
					...
					struct dt_allocation_hint *dah)
{
	...
	if (magic == __swab32(LOV_USER_MAGIC_V1)) {
		grumple_swab_lov_user_md_v1(info->lti_ea_store);
	} else if (magic == __swab32(LOV_USER_MAGIC_V3)) {
		grumple_swab_lov_user_md_v3(info->lti_ea_store);
		...
		grumple_swab_lov_comp_md_v1(info->lti_ea_buf.lb_buf);
	}
...
{
	struct lod_object *lo = lod_dt_obj(dt);
	struct grumple_handle_array *slave_locks = einfo->ei_cbdata;
	...
	for (i = 0; i < slave_locks->ha_count; i++)
		LASSERT(!grumple_handle_is_used(&slave_locks->ha_handles[i]));
			   ...
			   union ldlm_policy_data *policy)
{
	struct lod_object *lo = lod_dt_obj(dt);
	int slave_locks_size;
	struct grumple_handle_array *slave_locks = NULL;
	int i;
	int rc;
	ENTRY;
