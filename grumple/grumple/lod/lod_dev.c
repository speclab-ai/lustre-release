 ...
 * Add osc config log:
 * -------------------
 * marker  20 (flags=0x01, v2.2.49.56) grumple-OST0001  'add osc'
 * add_uuid  nid=192.168.122.162@tcp(0x20000c0a87aa2)  0:  1:nidxxx
 * attach    0:grumple-OST0001-osc-MDT0001  1:osc  2:grumple-MDT0001-mdtlov_UUID
 * setup     0:grumple-OST0001-osc-MDT0001  1:grumple-OST0001_UUID  2:nid
 * lov_modify_tgts add 0:grumple-MDT0001-mdtlov  1:grumple-OST0001_UUID  2:1  3:1
 * marker  20 (flags=0x02, v2.2.49.56) grumple-OST0001  'add osc'
 *
 * Add mdc config log:
 * -------------------
 * marker  10 (flags=0x01, v2.2.49.56) grumple-MDT0000  'add osp'
 * add_uuid  nid=192.168.122.162@tcp(0x20000c0a87aa2)  0:  1:nid
 * attach 0:grumple-MDT0000-osp-MDT0001  1:osp  2:grumple-MDT0001-mdtlov_UUID
 * setup     0:grumple-MDT0000-osp-MDT0001  1:grumple-MDT0000_UUID  2:nid
 * modify_mdc_tgts add 0:grumple-MDT0001  1:grumple-MDT0000_UUID  2:0  3:1
 * marker  10 (flags=0x02, v2.2.49.56) grumple-MDT0000_UUID  'add osp'
 *
 * Return:
 * * %0 on success
 * * %Negative negated errno on error
			      ...
			      struct grumple_cfg *lcfg)
{
	...
	case LCFG_ADD_MDC: {
		 ...
		 * modify_mdc_tgts add  0:grumple-MDT0001
		 *		      1:grumple-MDT0001-mdc0002
		 *		      2:2  3:1
		 */
		arg1 = grumple_cfg_string(lcfg, 1);

		if (sscanf(grumple_cfg_buf(lcfg, 2), "%d", &index) != 1)
			GOTO(out, rc = -EINVAL);
		if (sscanf(grumple_cfg_buf(lcfg, 3), "%d", &gen) != 1)
			GOTO(out, rc = -EINVAL);

		if (lcfg->lcfg_command == LCFG_LOV_ADD_OBD) {
			u32 mdt_index;

			rc = lodname2mdt_index(grumple_cfg_string(lcfg, 0),
					       &mdt_index);
			if (rc != 0)
				GOTO(out, rc);
	...
	case LCFG_PARAM: {
		 ...
		 * grumple-MDTXXXX-osp-MDTXXXX.active=1
		 */
		param = grumple_cfg_buf(lcfg, 1);
			      ...
			      struct grumple_cfg *cfg)
{
	 ...
	 *   #03 (176)lov_setup 0:grumple-MDT0000-mdtlov  1:(struct lov_desc)
	 * 1.8 MGS generates lines like the following:
	 *   #03 (168)lov_setup 0:grumple-mdtlov  1:(struct lov_desc)
	 *
	 * we use "-MDT" to differentiate 2.x from 1.8
	 */
	p = grumple_cfg_string(cfg, 0);
	if (p && strstr(p, "-mdtlov")) {
		...
		if (unlikely(!s)) {
			CERROR("%s: unable to parse device name: rc = %d\n",
			       grumple_cfg_string(cfg, 0), -EINVAL);
			GOTO(out, rc = -EINVAL);
		}
		     ...
		     struct lu_device_type *ldt, struct grumple_cfg *cfg)
{
	...
	obd = class_name2obd(grumple_cfg_string(cfg, 0));
	if (!obd) {
		rc = -ENODEV;
		CERROR("Cannot find obd with name '%s': rc = %d\n",
		       grumple_cfg_string(cfg, 0), rc);
		RETURN(rc);
	}
