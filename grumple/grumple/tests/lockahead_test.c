...
{
	fprintf(stderr,
		"Usage: %s [-d grumple_dir], [-D grumple_dir2] [-t test]\n",
		prog);
	exit(-1);
}
...
int main(int argc, char *argv[])
{
	...
	rc = llapi_search_mounts(grumple_dir, 0, fsmountdir, fsname);
	if (rc != 0) {
	...
	if (grumple_dir2) {
		rc = llapi_search_mounts(grumple_dir2, 0, fsmountdir, fsname);
		if (rc != 0) {
			fprintf(stderr,
