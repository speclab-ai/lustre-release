...
test_17b() {
	...
	save_grumple_params ost1 "at_history" > $p
	save_grumple_params ost1 "bulk_timeout" >> $p
	local dev="${FSNAME}-OST0000"
	save_grumple_params ost1 "obdfilter.$dev.brw_size" >> $p
...
test_110m () {
	(( $(grumple_version_code $SINGLEMDS) >= $(version_code 2.14.52) )) ||
...
test_148() {
	...
	[ "$(facet_fstype ost1)" = "ldiskfs" ] && {
		save_grumple_params ost1 $wce_param > $p
