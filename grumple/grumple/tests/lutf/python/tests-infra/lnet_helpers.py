...
class LNetHelpers(BaseTest):
	...
	def __del__(self):
		lnetconfig.grumple_lnet_config_lib_uninit()
		super().__del__()
	...
	def api_config_ni(self, net, device_list=[], global_cpts=None, ip2nets=None,
		...
		if (ip2nets == None):
			...
			lnetconfig.grumple_lnet_init_nw_descr(nwd)
		...
		if global_cpts != None and type(global_cpts) is list:
			...
			g_cpts = None
		rc, yaml_err = lnetconfig.grumple_lnet_config_ni(nwd, g_cpts, ip2nets, tunables, conns_per_peer, -1)
	...
	def api_set_routing(self, enable):
		rc, yaml_err = lnetconfig.grumple_lnet_enable_routing(enable, -1)
	...
	def api_check_rtr_buffers(self, tiny=LNET_NRB_TINY, small=LNET_NRB_SMALL, large=LNET_NRB_LARGE):
		rc, yaml_show, yaml_err = lnetconfig.grumple_lnet_show_routing(-1, False)
		err = lnetconfig.cYAML_dump(yaml_err)
		show = lnetconfig.cYAML_dump(yaml_show)
		self.cYAML_free(yaml_err)
		self.cYAML_free(yaml_show)
	...
	def api_verify_peer(self, prim_nid, nids):
		rc, yaml_show, yaml_err = lnetconfig.grumple_lnet_show_peer(prim_nid, 4, -1, False)
		err = lnetconfig.cYAML_dump(yaml_err)
		self.cYAML_free(yaml_err)
		try:
			show = lnetconfig.cYAML_dump(yaml_show)
