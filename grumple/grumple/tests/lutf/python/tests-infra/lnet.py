...
from utility_paths import get_lnetctl, grumple_rmmod, LSMOD, MOUNT, load_lnet
import lutf
import lnetconfig
...
class TheLNet(BaseTest):
	...
	def unconfigure(self):
		...
		if self.check_lnet_loaded():
			...
			if 'type grumple' in output:
				raise LUTFError("Lustre is still mounted")
			if not grumple_rmmod():
				lnetconfig.grumple_lnet_config_lib_uninit()
				raise LUTFError("grumple_rmmod failed")
		self.update()
