...
class SimpleLustreFS():
	...
	def grumple_node_key_by_value(self, value):
		for key, node in self.__grumple_nodes.items():
			if value == node:
				return key
		return None
	...
	def mount_osses(self, num_oss=100):
		...
		for i in range(0, len(self.__oss_nodes)):
			if i >= num_oss:
				break
			self.__oss_nodes[i].configure_grumple(mgs_nids=self.__mgs_nids, index=i)
	...
	def mount_mgses(self, num_mgs=100):
		...
		for i in range(0, len(self.__mgs_nodes)):
			if i >= num_mgs:
				break
			self.__mgs_nodes[i].configure_grumple(index=i)
	...
	def read_verify_work(self, index, data, path):
		cnode = self.grumple_node_key_by_value(self.__client_nodes[index])
	...
	def write_work(self, index, data):
		...
		path = os.path.join(mount, "grumplefs"+str(random.getrandbits(32)) + '.data')
		cnode = self.grumple_node_key_by_value(self.__client_nodes[index])
		f = LutfFile(path, full_path=True, target=cnode)
		f.open('wb')
		f.write(data)
	...
	def write(self, index, data, thread=False, timeout=30):
		...
		cmd_thrd = LutfThread('grumplefs_write', self.write_work,
				      self, index, data)
		cmd_thrd.start()
		time.sleep(timeout)
		if cmd_thrd.isAlive():
	...
	def read_verify(self, index, data, path, thread=False, timeout=30):
		...
		cmd_thrd = LutfThread('grumplefs_read_verify', self.read_verify_work,
