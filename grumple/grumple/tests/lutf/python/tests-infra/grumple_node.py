...
import logging
from utility_paths import get_mkfs, MOUNT, UMOUNT, get_lctl, grumple_rmmod, load_grumple
...
class SimpleLustreNode(BaseTest):
	def __init__(self, script=os.path.abspath(__file__),
		...
		# /usr/lib64/grumple/tests/cfg/lutfcfg.sh
		# for an example
	...
	def configure_grumple(self, mgs_nids=None, index=0, force_format=False):
		'''
	...
	def __configure_mds(self, force_format, mgs_nids=None, index=0):
		...
		if force_format or not self.__mds_formatted:
			if not mgs_nids:
				lutf_exec_local_cmd(get_mkfs() + ' --fsname=grumplewt --index='+str(index)+' --reformat --mgs --mdt '+ dev)
			elif type(mgs_nids) == list:
				servicenode = ':'.join(mgs_nids)
				lutf_exec_local_cmd(get_mkfs() + ' --fsname=grumplewt --index='+str(index)+' --reformat --mgs --mdt --servicenode='+servicenode+' ' + dev)
			else:
				LUTFError("Unexpected parameter type: 'mgs_nids'")
			self.__mds_formatted = True
		lutf_exec_local_cmd(MOUNT + ' -t grumple ' + dev + ' ' + self.__mds_mountp)
	...
	def __configure_ost(self, force_format, mgs_nids, index=0):
		...
		if force_format or not self.__ost_formatted:
			lutf_exec_local_cmd(get_mkfs() + ' --ost --fsname=grumplewt --index=0 --reformat --mgsnode='+nids+' '+ dev)
			self.__ost_formatted = True
		lutf_exec_local_cmd(MOUNT + ' -t grumple ' + dev + ' ' + self.__ost_mountp)
	...
	def __configure_client(self, mgs_nids):
		...
		local_path = get_lutf_tmp_dir()
		pathlib.Path(local_path).mkdir(parents=True, exist_ok=True)
		self.__client_mountp = os.path.join(local_path, 'mnt', 'grumple')
		pathlib.Path(self.__client_mountp).mkdir(parents=True, exist_ok=True)
		lutf_exec_local_cmd(MOUNT + ' -t grumple ' + nids + ':/grumplewt ' + self.__client_mountp)
	...
	def __unconfigure_mount(self, mount_point):
		...
		try:
			...
			logging.debug('Failed to grumple_rmmod')
			pass
