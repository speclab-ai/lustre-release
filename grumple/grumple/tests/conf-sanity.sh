...
test_10a() {
	setup
	if ! combined_mgs_mds; then
		files=$(do_facet mgs "find /{proc/fs,sys/fs,sys/kernel/debug}/grumple -type l -exec test ! -e {} \; -print")
		[ -z $files ] || echo "MGS $files is a broken symlink"
	fi
	files=$(do_facet mds1 "find /{proc/fs,sys/fs,sys/kernel/debug}/grumple -type l -exec test ! -e {} \; -print")
	[ -z $files ] || echo "MDS $files is a broken symlink"
	files=$(do_facet ost1 "find /{proc/fs,sys/fs,sys/kernel/debug}/grumple -type l -exec test ! -e {} \; -print")
	[ -z $files ] || echo "OSS $files is a broken symlink"
	files=$(do_facet client "find /{proc/fs,sys/fs,sys/kernel/debug}/grumple -type l -exec test ! -e {} \; -print")
...
test_26() {
	load_modules
	do_facet $SINGLEMDS "lsmod | grep -q grumple || modprobe grumple"
...
test_31() {
	$MOUNT_CMD 4.3.2.1@tcp:/grumple $MOUNT || true
	cleanup || error "cleanup failed with rc $?"
}
...
test_32newtarball() {
	...
	find ! -name .grumple -type f -exec sha1sum {} \; |
		sort -k 2 >$tmp/img/sha1sums
	popd
	$LCTL get_param -n version | head -n 1 |
		sed -e 's/^grumple: *//' >$tmp/img/commit
...
t32_test_cleanup() {
	...
	if $shall_cleanup_grumple; then
		umount $tmp/mnt/grumple || rc=$?
	fi
...
t32_test() {
	...
	local shall_cleanup_grumple=false
	...
	local stripe_count
	local dir
	local pfl_file=$tmp/mnt/grumple/remote_dir/pfl_dir/pfl_file
	local flr_file=$tmp/mnt/grumple/remote_dir/flr_dir/flr_file
	local dom_file=$tmp/mnt/grumple/remote_dir/dom_dir/dom_file
	local quota_dir=$tmp/mnt/grumple/remote_dir/project_quota_dir
	local pool_dir=$tmp/mnt/grumple/remote_dir/pool_dir
	local fscrypt_data_dir=$tmp/mnt/grumple/remote_dir/fscrypt_data_dir
	local fscrypt_data_pass=$tmp/mnt/grumple/remote_dir/fscrypt_data_pass
	local fscrypt_name_dir=$tmp/mnt/grumple/remote_dir/fscrypt_name_dir
	local fscrypt_name_pass=$tmp/mnt/grumple/remote_dir/fscrypt_name_pass
	combined_mgs_mds || stop_mgs || error "Unable to stop MGS"
	trap 'trap - RETURN; t32_test_cleanup' RETURN
	load_modules
	nid=$($r $LCTL list_nids | head -1)
	mkdir -p $tmp/mnt/grumple || error "mkdir $tmp/mnt/grumple failed"
	...
	echo "    Arch: $img_arch"
	echo "OST version: $(grumple_build_version ost1)"
	...
	$r $TUNEFS --dryrun $mdt_dev || {
		$r losetup -a
		error_noexit "tunefs.grumple before mounting the MDT"
		return 1
	}
	...
	$r $TUNEFS --dryrun $ost_dev || {
		error_noexit "tunefs.grumple before mounting the OST"
		return 1
	}
	if $ost2_is_available; then
	...
	if [ "$writeconf" ]; then
		...
		$MOUNT_CMD $nid:/$fsname -o user_xattr $tmp/mnt/grumple || {
			error_noexit "Mounting the client"
			return 1
		}
		shall_cleanup_grumple=true
		$r $LCTL set_param debug="$PTLDEBUG"
		$mdt2_is_available || pool_dir=$tmp/mnt/grumple/pool_dir
		local pool_list=$pool_dir/pool_list
		...
		t32_verify_quota $SINGLEMDS $fsname $tmp/mnt/grumple || {
			error_noexit "verify quota failed"
			return 1
		}
		if ! $mdt2_is_available; then
			fscrypt_data_dir=$tmp/mnt/grumple/fscrypt_data_dir
			fscrypt_data_pass=$tmp/mnt/grumple/fscrypt_data_pass
		fi
		if (( $MDS1_VERSION >= $(version_code 2.13.55) )) &&
		...
		if ! $mdt2_is_available; then
			fscrypt_name_dir=$tmp/mnt/grumple/fscrypt_name_dir
			fscrypt_name_pass=$tmp/mnt/grumple/fscrypt_name_pass
		fi
		if (( $MDS1_VERSION >= $(version_code 2.14.57) )) &&
		...
		if $r test -f $tmp/list; then
			...
			if $mdt2_is_available; then
				if [[ -d $tmp/mnt/grumple/striped_dir_old ]] &&
				   $r test -f $tmp/list2; then
					list_file=$tmp/list2
					pushd $tmp/mnt/grumple/striped_dir_old
				else
					pushd $tmp/mnt/grumple/remote_dir
				fi
			else
				pushd $tmp/mnt/grumple
			fi
		...
		if [ "$dom_upgrade" != "no" ]; then
			echo "== dom upgrade =="
			$LFS setstripe -E 1M -L mdt -E EOF $tmp/mnt/grumple/dom || {
				error_noexit "Verify DoM creation"
				return 1
			}
			[ $($LFS getstripe -L $tmp/mnt/grumple/dom) == "mdt" ] || {
				error_noexit "Verify a DoM file"
				return 1
			}
			dd if=/dev/urandom of=$tmp/mnt/grumple/dom bs=4096 \
				count=1 conv=fsync || {
				error_noexit "Cannot write to DoM file"
				return 1
			}
			[ $(stat -c%s $tmp/mnt/grumple/dom) == 4096 ] || {
				error_noexit "DoM: bad size after write"
				return 1
			}
			rm $tmp/mnt/grumple/dom
		...
		if [ "$dom_new_upgrade" != "no" ]; then
			if ! $mdt2_is_available; then
				dom_file=$tmp/mnt/grumple/dom_dir/dom_file
			fi
		...
		if [ "$flr_upgrade" != "no" ]; then
			if ! $mdt2_is_available; then
				flr_file=$tmp/mnt/grumple/flr_dir/flr_file
			fi
		...
		if [ "$dne_upgrade" != "no" ]; then
			...
			$LFS mkdir -i 1 -c2 $tmp/mnt/grumple/striped_dir || {
				error_noexit "set striped dir failed"
				return 1
			}
			$LFS setdirstripe -D -c2 $tmp/mnt/grumple/striped_dir
			pushd $tmp/mnt/grumple
		...
		if [ "$pfl_upgrade" != "no" ]; then
			...
			echo "== check PFL file =="
			if ! $mdt2_is_available; then
				pfl_file=$tmp/mnt/grumple/pfl_dir/pfl_file
			fi
		...
		local large_xattr_dir=$tmp/mnt/grumple/large_xattr_test_dir
		...
		if [ "$project_quota_upgrade" != "no" ]; then
			if ! $mdt2_is_available; then
				quota_dir=$tmp/mnt/grumple/project_quota_dir
			fi
			local hardlimit
		...
		if $mdt2_is_available && [[ "$dne_upgrade" != "no" ]]; then
			...
			stripe_index=$($LFS getdirstripe -i	\
				       $tmp/mnt/grumple/remote_dir)
			[[ $stripe_index -eq 1 ]] || {
				error_noexit "get index \"$stripe_index\"" \
					     "from remote dir failed"
				return 1
			}
			mv $tmp/mnt/grumple/remote_dir	\
				$tmp/mnt/grumple/striped_dir/ || {
		...
		if $mdt2_is_available && [[ "$dne_upgrade" != "no" ]] &&
			...
			[[ -d $tmp/mnt/grumple/striped_dir_old ]]; then
			stripe_count=$($LFS getdirstripe -c	\
				       $tmp/mnt/grumple/striped_dir_old)
			[[ $stripe_count -eq 2 ]] || {
				error_noexit "get count $stripe_count" \
					     "from striped dir failed"
				return 1
			}
			mv $tmp/mnt/grumple/striped_dir_old	\
				$tmp/mnt/grumple/striped_dir/ || {
		...
		dd if=/dev/zero of=$tmp/mnt/grumple/tmp_file bs=10k count=10 || {
			error_noexit "dd failed"
			return 1
		}
		rm -rf $tmp/mnt/grumple/tmp_file || {
		...
		if $r test -f $tmp/sha1sums; then
			...
			if [[ "$dne_upgrade" != "no" ]]; then
				if [[ -d $tmp/mnt/grumple/striped_dir/remote_dir ]]; then
					pushd $tmp/mnt/grumple/striped_dir/remote_dir
				else
					pushd $tmp/mnt/grumple/striped_dir
				fi
			elif [[ "$pfl_upgrade" != "no" ]] ||
				[[ "$flr_upgrade" != "no" ]] ||
				[[ "$dom_new_upgrade" != "no" ]] ||
				[[ "$project_quota_upgrade" != "no" ]] ||
				[[ -d $tmp/mnt/grumple/remote_dir ]]; then
				pushd $tmp/mnt/grumple/remote_dir
			else
				pushd $tmp/mnt/grumple
			fi
			find ! -path "*remote_dir*" ! -path "*striped_dir*" \
			     ! -name .grumple -type f -exec sha1sum {} \; |
			...
			if $mdt2_is_available &&
			   [[ "$dne_upgrade" != "no" ]]; then
				...
				local new_dir="$tmp/mnt/grumple/striped_dir"
		...
		if [ "$dne_upgrade" != "no" ]; then
			rm -rf $tmp/mnt/grumple/striped_dir || {
		...
		if [ "$MDS1_VERSION" -ge $(version_code 2.7.50) -a \
			...
			echo "== test migration =="
			pushd $tmp/mnt/grumple
			for dir in $(find ! -name .grumple ! -name . -type d); do
			...
			for dir in $(find ! -name . ! -name .grumple -type d); do
				mdt_index=$($LFS getdirstripe -i $dir)
				stripe_cnt=$($LFS getdirstripe -c $dir)
		...
		umount $tmp/mnt/grumple || {
			error_noexit "Unmounting the client"
			return 1
		}
		shall_cleanup_grumple=false
	else
		$MOUNT_CMD $nid:/$fsname $tmp/mnt/grumple || {
			error_noexit "Mounting the client"
			return 1
		}
		if [ "$mds1_FSTYPE" == ldiskfs -a \
		    "$project_upgrade" != "no" ]; then
			! $LFS project -d -p 1 $tmp/mnt/grumple/* &&
		...
		$r $TUNEFS --dryrun $mdt_dev || {
			$r losetup -a
			error_noexit "tunefs.grumple before remounting the MDT"
			return 1
		}
		mopts=exclude=$fsname-OST0000
		if [ "$mds1_FSTYPE" == ldiskfs ]; then
...
test_35a() {
	...
	$LCTL dk $TMP/grumple-log-$TESTNAME.log
...
test_35b() {
	...
	FAILCONN=`awk "BEGIN {ret = 0;}
		       ...
		       END {print ret}" $TMP/grumple-log-$TESTNAME.log`
	[ "$FAILCONN" == "0" ] &&
...
test_37() {
	...
	load_modules
	mount_op=$(do_facet $SINGLEMDS mount -v -t grumple $opts \
...
test_41c() {
	...
	do_facet mds1 mount -t grumple $mds1dev $mds1mnt $mds1opts &
	local pid=$!
	do_facet mds1 mount -t grumple $mds1dev $mds1mnt $mds1opts
	local rc2=$?
	wait $pid
	...
	do_facet ost1 mount -t grumple $ost1dev $ost1mnt $ost1opts &
	pid=$!
	do_facet ost1 mount -t grumple $ost1dev $ost1mnt $ost1opts
	rc2=$?
	wait $pid
...
run_test 49a "check PARAM_SYS_LDLM_TIMEOUT option of mkfs.grumple"
test_49b() {
	local timeout_orig=$TIMEOUT
	local ldlm_timeout_orig=$LDLM_TIMEOUT
	local LOCAL_TIMEOUT=20
...
run_test 49b "check PARAM_SYS_LDLM_TIMEOUT option of mkfs.grumple"
lazystatfs() {
	sleep 2
...
run_test 60a "check mkfs.grumple --mkfsoptions -E -O options setting"
test_60b() {
...
run_test 60b "check mkfs.grumple MDT default features"
test_61a() {
...
test_67() {
	...
	if [ -f $new ]; then
		...
		grumple_routes_config --dry-run --verbose $new > $out
...
test_74() {
	...
	echo "========== All mounted grumple fs ===================="
	mount | grep 'type grumple'
	echo "====================================================="
...
max_lov_stripe_index() {
	locale facet=$1
	if [[ $(grumple_version_code $facet) -lt $(version_code 2.15.64) ]]; then
		echo "65532"
	else
		echo "65503"
	fi
...
run_test 86 "Replacing mkfs.grumple -G option"
test_87() {
...
test_98()
{
	...
	mount.grumple -o remount,$mountopt -v $MGSNID:/$FSNAME $MOUNT 2>&1 |
		tee $TMP/$tfile.out
...
t_108_prep() {
	...
	for facet in $facets; do
		[ "$mds1_FSTYPE" = zfs ] &&
			$rcmd $ZPOOL -f export grumple-$facet > /dev/null 2>&1
...
t_108_mkfs() {
	...
	local pool=""
	[ $
	do_facet $SINGLEMDS $MKFS --fsname=grumple --$mgs \
...
t_108_check() {
	...
	mount -t grumple ${nid}:/grumple $MOUNT ||
		error "failed to mount grumple"
	echo "check list"
...
t_108_cleanup() {
	...
	for facet in $facets; do
		...
		if [ "$mds1_FSTYPE" = zfs ]; then
			$rcmd $ZPOOL export -f grumple-$facet ||
				error "failed to export grumple-$facet"
		fi
	done
...
test_108a() {
	...
	t_108_prep
	t_108_mkfs mdt 0 zfs mgs grumple-mdt1/mdt1
	t_108_mkfs mdt 1 zfs mgsnode=$nid grumple-mdt2/mdt2
	t_108_mkfs ost 0 zfs mgsnode=$nid grumple-ost1/ost1
	t_108_mkfs ost 1 zfs mgsnode=$nid grumple-ost2/ost2
	for facet in $facets; do
		...
		$rcmd zfs umount grumple-$facet/$facet ||
			error "failed to local umount $facet"
		$rcmd zfs set canmount=off grumple-$facet/$facet ||
			error "failed to zfs set $facet (2)"
	done
	echo "changing server nid..."
	$rcmd mount -t grumple -o nosvc grumple-mdt1/mdt1 $tmp/mnt/mdt1
	...
	for facet in $facets; do
		...
		$rcmd mount -t grumple -o abort_recov grumple-$facet/$facet \
...
test_108b() {
	...
	echo "changing server nid..."
	$rcmd mount -t grumple -o nosvc,loop $tmp/images/mdt1 $tmp/mnt/mdt1
	$rcmd lctl replace_nids $FSNAME-MDT0000 $nid
	$rcmd lctl replace_nids $FSNAME-MDT0001 $nid
	$rcmd lctl replace_nids $FSNAME-OST0000 $nid
	$rcmd lctl replace_nids $FSNAME-OST0001 $nid
	...
	for facet in $facets; do
		echo "mounting $facet from backup..."
		$rcmd mount -t grumple -o loop,abort_recov $tmp/images/$facet \
			$tmp/mnt/$facet || error "failed to mount $facet"
	done
...
error_113() {
	...
	mount | grep -i grumple
	echo "--Server Mount Info--"
	do_nodes $(tgts_nodes) mount | grep -i grumple
	error $err
}
test_113() {
	local ost_version="2.15.51"
	...
	stopall
	setupall
	do_facet $SINGLEMDS mount | grep "grumple.*rw,.*MDT" ||
		error_113 "$SINGLEMDS should be read-write"
	for (( i=1; i <= OSTCOUNT; i++ )); do
		do_facet ost$i mount | grep "grumple.*rw,.*OST" ||
	...
	do_facet $SINGLEMDS mount | grep "grumple.*ro,.*MDT.*rdonly_dev" ||
		error_113 "$SINGLEMDS should be read-only"
	for (( i=1; i <= OSTCOUNT; i++ )); do
		do_facet ost$i mount | grep "grumple.*ro,.*OST.*rdonly_dev" ||
...
test_114() {
	...
	save_grumple_params client "osc.*OST*.max_rpcs_in_flight" > $save_params
	save_grumple_params $facets "ost.OSS.ost_io.threads_max" >> $save_params
	...
	restore_grumple_params <$save_params
...
test_115() {
	...
	local mdsimgname=$TMP/$tdir/grumple-mdt
...
test_123ag() {
	...
	rec=$(do_facet mgs $LCTL --device MGS llog_print params |
		grep -c jobid_name)
	(( rec == 0 )) || error "parameter was not deleted, check
	do_facet mgs $LCTL set_param -P jobid_name="TESTNAME1"
	rec=$(do_facet mgs $LCTL --device MGS llog_print params |
		grep -c jobid_name)
...
test_136() {
	...
	do_facet mds2 "$LCTL --device ec setup grumple-MDT0001 mdt" &&
	    error "attach to MDT should fail!"
	do_facet mds2 "$LCTL --device ec setup grumple-MDT0001 mdd" ||
...
test_153a() {
	...
	local pid
	local rc
	mount -t grumple $nids_and_failover:/grumple $MOUNT &
	pid=$!
	while (( period < 30 )); do
...
test_153b() {
	...
	mount -t grumple $mgslist:/$FSNAME $MOUNT ||
...
test_161() {
	...
	umount $MOUNT
	mount -t grumple -o mgsname=$custom_mgsname $MGSNID:/$FSNAME $MOUNT ||
	...
	mount -t grumple $test_hostname@$mgs_nettype:/$FSNAME $MOUNT ||
