...
test_10b() {
	...
	dd if=$DIR2/f10b of=$TMP/f10b-grumple bs=4k count=1 || error "dd $DIR2"
	dd if=$TMP/f10b-seed of=$TMP/f10b bs=3k count=1 || error "dd random"
	$TRUNCATE $TMP/f10b 4096 || error "truncate 4096"
	cmp $TMP/f10b $TMP/f10b-grumple || error "file miscompare"
	rm $TMP/f10b $TMP/f10b-grumple $TMP/f10b-seed
}
...
test_21() {
	test_mkdir $DIR1/$tdir
	mount /etc $DIR1/$tdir --bind || error "mount failed"
...
test_25a() {
	...
	$RUNAS $CHECKSTAT $DIR2/$tdir/f1 || error "checkstat $DIR2/$tdir/f1
	setfacl -m u:$RUNAS_ID:--- -m g:$RUNAS_GID:--- $DIR1/$tdir ||
		error "setfacl $DIR2/$tdir
	$RUNAS $CHECKSTAT $DIR2/$tdir/f1 && error "checkstat $DIR2/$tdir/f1
...
test_33a() {
	...
	restore_grumple_params < $param_file
	rm -f $param_file
	return 0
}
...
test_84() {
	[ $MDS1_VERSION -lt $(version_code 2.12.55) ] &&
		skip "grumple < 2.12.55 does not contain LU-12485 fix"
	local mtime
...
test_100a() {
	...
	$CHECKSTAT -t file -s 4096 $DIR/$tdir/dom || error "stat
