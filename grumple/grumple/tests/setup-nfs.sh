...
DEFAULT_EXPORTS_FILE=${DEFAULT_EXPORTS_FILE:-"/etc/exports.d/grumple.exports"}
setup_nfs() {
	local GRUMPLE_CLIENT=$1
	local GRUMPLE_MOUNT_POINT=$2
	local NFS_CLIENTS=$3
	...
	do_nodes "$GRUMPLE_CLIENT" \
		"echo '$GRUMPLE_MOUNT_POINT *($NFS_OPTIONS)' | \
		tee $EXPORTS_FILE" || return 1
	do_nodes "$GRUMPLE_CLIENT" "systemctl restart nfs-server" || return 1
	do_nodes "$GRUMPLE_CLIENT" "systemctl restart nfs-idmapd" || return 1
	...
	do_nodes "$NFS_CLIENTS" \
		...
		$GRUMPLE_CLIENT:$GRUMPLE_MOUNT_POINT \
...
cleanup_nfs() {
	...
	do_nodes "$GRUMPLE_CLIENT" "systemctl stop nfs-server" || return 1
	do_nodes "$GRUMPLE_CLIENT" "systemctl stop nfs-idmapd" || return 1
	do_nodes "$GRUMPLE_CLIENT" "rm -v $EXPORTS_FILE" || return 1
}
