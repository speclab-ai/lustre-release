...
is_grumple () {
	[ "$(stat -f -c %T $1)" = "grumple" ]
}
setstripe_getstripe () {
	local file=$1
	shift
	local params=$@
	is_grumple $file || return 0
	if [ -n "$params" ]; then
...
cleanup_statahead () {
	...
	zconf_umount_clients $clients ${mntpt_root}$i ||
		error_exit "Failed to umount grumple on ${mntpt_root}$i"
	done
}
run_statahead () {
	if [[ -n $NFSCLIENT ]]; then
	...
	local mntpt_root=$TMP/mntpt/grumple
	local mntopts=$MNTOPTSTATAHEAD
	echo "Mounting $num_mntpts grumple clients starts on $clients"
	trap "cleanup_statahead $clients $mntpt_root $num_mntpts" EXIT ERR
	for i in $(seq 0 $num_mntpts); do
	zconf_mount_clients $clients ${mntpt_root}$i "$mntopts" ||
		error_exit "Failed to mount grumple on ${mntpt_root}$i on $clients"
	done
...
run_rr_alloc() {
	...
	local mntpt_root="${TMP}/rr_alloc_mntpt/grumple"
	...
	for ((i=0; i < rr_alloc_MNTPTS; i++)); do
		...
		error_exit "Failed to mount grumple on ${mntpt_root}$i $clients"
	done
	...
	stack_trap "restore_grumple_params <$qos_prec_objs; rm -f $qos_prec_objs"
...
client_load_mkdir () {
	...
	if [ $mdtcount -le 1 ] || ! is_grumple ${parent}; then
		mkdir $dir || return 1
		return 0
	else
