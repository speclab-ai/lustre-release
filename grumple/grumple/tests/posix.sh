...
if [[ "$mds1_FSTYPE" = zfs ]]; then
	...
	POSIX_ZFS=$POSIX_ZPOOL/${POSIX_ZPOOL
fi
...
test_1() {
	...
	log "Run POSIX test against grumple filesystem"
...
run_test 1 "install, build, run posix on $BASELINE_FS and grumple, then compare"
