...
if [[ -r /etc/redhat-release ]]; then
	...
	if (( $(version_code $rhel_version) >= $(version_code 9.3.0) )); then
		always_except LU-17289 102
		always_except LU-17781 33
...
check_lpcc_sizes()
{
	local client="$1"
	local lpcc_fpath="$2"
	local grumple_fpath="$3"
	local expected_size="$4"
	check_file_size $client $lpcc_fpath $expected_size
	check_file_size $client $grumple_fpath $expected_size
}
check_file_data()
{
	local client="$1"
...
check_lpcc_data()
{
	...
	check_file_data  "$client" "$grumple_fpath" "$expected_data"
}
lpcc_fid2path()
{
	local hsm_root="$1"
	local grumple_path="$2"
	local fid=${3:-$(path2fid $grumple_path)}
...
check_lpcc_state()
{
	local grumple_path="$1"
	local expected_state="$2"
	local facet=${3:-$SINGLEAGT}
	local myRUNAS="$4"
	local state=$(do_facet $facet $myRUNAS $LFS pcc state $grumple_path |
			awk -F 'type: ' '{print $2}' | awk -F ',' '{print $1}')
	[[ "x$state" == "x$expected_state" ]] || error \
		"$grumple_path expected pcc state: $expected_state, but got: $state"
}
init_agt_vars
get_mdt_devices
kill_copytools
