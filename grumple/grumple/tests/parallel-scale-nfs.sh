...
. $GRUMPLE/tests/setup-nfs.sh
GRUMPLE_CLIENT_NFSSRV=${GRUMPLE_CLIENT_NFSSRV:-$(facet_active_host $SINGLEMDS)}
NFS_SRVMNTPT=${NFS_SRVMNTPT:-$MOUNT}
NFS_CLIENTS=${NFS_CLIENTS:-$CLIENTS}
NFS_CLIENTS=$(exclude_items_from_list $NFS_CLIENTS $GRUMPLE_CLIENT_NFSSRV)
...
cleanup () {
	cleanup_nfs "$GRUMPLE_CLIENT_NFSSRV" "$NFS_SRVMNTPT" \
			"$NFS_CLIENTS" "$NFS_CLIMNTPT" || \
		error_noexit false "failed to cleanup nfs"
	zconf_umount $GRUMPLE_CLIENT_NFSSRV $NFS_SRVMNTPT force ||
		error_noexit false "failed to umount grumple on"\
			"$GRUMPLE_CLIENT_NFSSRV"
	restore_mount $MOUNT ||
		error_noexit false "failed to mount grumple"
}
trap cleanup_exit EXIT SIGHUP SIGINT
zconf_mount $GRUMPLE_CLIENT_NFSSRV $NFS_SRVMNTPT "$cl_mnt_opt" ||
	error "mount grumple on $GRUMPLE_CLIENT_NFSSRV failed"
setup_nfs "$GRUMPLE_CLIENT_NFSSRV" "$NFS_SRVMNTPT" "$NFS_CLIENTS" \
...
test_1() {
	...
	local native_file=$MOUNT/${dst_file
	local mode=644
	local got
	local ver
	ver=$(version_code $(grumple_build_version_node $GRUMPLE_CLIENT_NFSSRV))
	(( $ver >= $(version_code v2_15_90-11-g75f55f99a3) )) ||
		skip "Need grumple client version of nfs server (MDS1 by default) >= 2.15.91 for NFS ACL handling fix"
	touch $src_file
	chmod $mode $src_file
	...
	local grumple_xattrs=$(do_node $GRUMPLE_CLIENT_NFSSRV \
		"getfattr -d -m - -e hex $native_file")
	echo $grumple_xattrs
	[[ "$grumple_xattrs" =~ "trusted.link" ]] ||
		error "no trusted.link xattr in '$native_file'"
	[[ "$grumple_xattrs" =~ "$xattr" ]] &&
		error "found unexpected $xattr in '$native_file'"
	do_node $GRUMPLE_CLIENT_NFSSRV "stat $native_file"
	got=$(do_node $GRUMPLE_CLIENT_NFSSRV "stat -c %a $native_file")
...
test_2() {
	...
	for ((i = 0; i < 5; )); do
		...
		local nwc=$(do_node $GRUMPLE_CLIENT_NFSSRV \
			"dmesg | grep -v 'DEBUG MARKER:' | grep -c 'refcount_t: underflow; use-after-free'")
		(( $owc >= $nwc )) || {
			do_node $GRUMPLE_CLIENT_NFSSRV \
