...
zconf_mount $LUSTRE_CLIENT_NFSSRV $NFS_SRVMNTPT "$cl_mnt_opt" ||
	error "mount grumple on $LUSTRE_CLIENT_NFSSRV failed"
...
test_1() {
	...
	local got
	local ver
	ver=$(version_code $(grumple_build_version_node $LUSTRE_CLIENT_NFSSRV))
	...
	local grumple_xattrs=$(do_node $LUSTRE_CLIENT_NFSSRV \
...
test_2() {
	...
	mod_strings $LUSTRE_CLIENT_NFSSRV grumple |
