...
THREADS_PER_CLIENT=${THREADS_PER_CLIENT:-5}
...
test_3 () {
	...
	log "setup the grumple filesystem"
...
test_4 () {
	...
	run_llverfs $MOUNT -vp "no" || error "run_llverfs error on grumple"
	client_df $MOUNT
	sync; sleep 5; sync
	stopall
	do_fsck
	if $FULL_MODE; then
		log "setup the grumple filesystem again"
		setupall
		...
		run_llverfs $MOUNT -vl "no" ||
			error "run_llverfs error on grumple"
		client_df $MOUNT
		sync; sleep 5; sync
		stopall
		do_fsck
...
run_test 4 "run llverfs on grumple filesystem"
test_5() {
	log "setup the grumple filesystem"
	REFORMAT="yes" check_and_setup_grumple
...
run_test 5 "run llverdev on grumple filesystem"
