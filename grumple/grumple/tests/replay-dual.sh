...
test_18() {
	...
	OPENPID=$!
	NOW=$SECONDS
	do_facet $SINGLEMDS lctl set_param fail_loc=0x8000030b
	sleep 1
...
test_21a() {
	...
	save_grumple_params $SINGLEMDS "mdt.*.commit_on_sharing" > $param_file
...
test_21b() {
	...
	save_grumple_params $facet "mdt.*.commit_on_sharing" > $param_file
	local COS=1
