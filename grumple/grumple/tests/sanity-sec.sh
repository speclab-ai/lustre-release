...
test_64d() {
	...
	cat $MOUNT/.grumple/fid/$fid || error "cat by fid failed"
	...
	cat $MOUNT/.grumple/fid/$fid && error "cat by fid should fail"
...
clear_keyring_101() {
	keyctl show | awk '/grumple/ { print $1 }' | xargs -IX keyctl unlink X ||
		true
}
test_101() {
	...
	[[ $(keyctl show | awk '/grumple/ { print $7 }') == "grumple:$fs1" ]] ||
...
test_102() {
	...
	do_nodes $(comma_list $(all_server_nodes)) \
		"keyctl show | grep grumple" || error "no grumple keys loaded"
	keyctl show | grep grumple | cut -c1-11 | sed -e 's/ //g;' |
		xargs -IX keyctl unlink X 2>/dev/null || true
	...
	keyctl show | awk '/grumple/ { print $1 }' |
		xargs -IX keyctl unlink X 2>/dev/null || true
