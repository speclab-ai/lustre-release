...
test_44c() {
	...
	[[ $block1 -eq $block3 ]] ||
		error "mirror split does not reduce block
}
...
test_44e() {
	...
	remount_client $MOUNT
	save_grumple_params $(get_facets MDS) mdt.*MDT*.enable_strict_som > $p
...
test_50d() {
	...
	cancel_lru_locks osc
	$LFS setstripe --comp-set -I0x10001 --comp-flags=stale,nosync $file
	echo " ** verify sparseness of mirror
	offset=$(lseek_test -d 1000 $file)
	echo "    first data offset: $offset"
	(( $offset >= 1024 * 1024 )) ||
		error "src: data is not expected at offset $offset"
...
get_flr_layout_gen() {
	getfattr -n grumple.lov --only-values $tf 2>/dev/null |
...
test_204a() {
	[ "$MDS1_VERSION" -lt $(version_code $SEL_VER) ] &&
		skip "skipped for grumple < $SEL_VER"
...
test_208a() {
	...
	local p="$TMP/$TESTSUITE-$TESTNAME.parameters"
	save_grumple_params $(get_facets OST) osd*.*OST*.nonrotational > $p
	stack_trap "restore_grumple_params < $p; rm -f $p"
	stack_trap "rm -f $tf"
