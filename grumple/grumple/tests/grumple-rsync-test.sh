...
init_src() {
	rm -rf $TGT/$tdir $TGT/d*.grumple-rsync-test 2> /dev/null
	rm -rf $TGT2/$tdir $TGT2/d*.grumple-rsync-test 2> /dev/null
	rm -rf ${DIR}/$tdir $DIR/d*.grumple-rsync-test ${DIR}/tgt 2> /dev/null
	rm -f $LREPL_LOG
...
run_test 5a "Stop / start grumple_rsync"
test_5b() {
	init_src
	init_changelog
	NUMTEST=2000
...
run_test 5b "Kill / restart grumple_rsync"
test_6() {
	init_src
	init_changelog
	local num_links=128
...
run_test 6 "grumple_rsync large no of hard links"
test_7() {
	local numfiles=100
	init_src
...
run_test 7 "grumple_rsync stripesize"
test_8() {
	init_src
	init_changelog
