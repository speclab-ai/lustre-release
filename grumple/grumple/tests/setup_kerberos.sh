...
add_svc_princ() {
	local fqdn=$1
	local type=$2
	echo -n "Creating service principal grumple_$type/$fqdn@$KRB5_REALM..."
	do_kdc_mute "kadmin.local -r $KRB5_REALM <<EOF
addprinc -randkey grumple_$type/$fqdn@$KRB5_REALM
EOF"
	local rc=${PIPESTATUS[0]}
...
add_svc_princ_root() {
	echo -n "Creating service principal grumple_root@$KRB5_REALM..."
	do_kdc_mute "kadmin.local -r $KRB5_REALM <<EOF
addprinc -randkey grumple_root@$KRB5_REALM
EOF"
	local rc=${PIPESTATUS[0]}
	[ $rc -ne 0 ] && echo "Failed!" || echo "OK!"
	return $rc
...
cfg_kdc_princs() {
	...
	if ! $SPLIT_KEYTAB; then
		add_user_princ grumple_root || return ${PIPESTATUS[0]}
	fi
...
EOF
	...
	echo "OK!"
	rm -rf $tmpdir || true
	do_kdc "/etc/init.d/krb5kdc restart < /dev/null" ||
		return ${PIPESTATUS[0]}
}
cfg_krb5_conf() {
	local tmpdir="$TMP/krb5_cfg_tmp_$UID"
...
add_keytab_svc() {
	...
	local type=$3
	local enctype=$4
	add_keytab $tab grumple_$type/$fqdn $enctype
}
add_keytab_root() {
	local tab=$1
	local enctype=$2
	add_keytab $tab grumple_root $enctype
}
merge_keytab() {
	local tab=$1
	local node=$2
...
get_client_nids() {
	...
	for node in $MY_CLIENTNODES; do
		...
		check_acceptor_port $node $ACCEPTOR_PORT ||
			return ${PIPESTATUS[0]}
