...
build_test_filter
FAIL_ON_ERROR=false
proc_regexp="/{proc,sys}/{fs,sys,kernel/debug}/{grumple,lnet}/"
cleanup() {
	echo -n "cln.."
...
test_0d() {
	...
	imp_val=$(version_code $(grumple_build_version client | cut -d. -f1,2,3))
	[ "$exp_val" == "$imp_val" ] ||
		error "exp version '$exp_client_version'($exp_val) != " \
			"'$(grumple_build_version client)'($imp_val)"
}
...
test_17n() {
	...
	[ $MDS1_VERSION -lt $(version_code 2.4.50) ] &&
		skip "grumple < 2.4.50 does not support migrate mv"
	for ((i = 0; i < 10; i++)); do
...
test_27s() {
	test_mkdir $DIR/$tdir
	local stripe_size=$((4096 * 1024 * 1024))
...
test_27O() {
	[[ $(grumple_version_code $SINGLEMDS) -le $(version_code 2.12.51) ]] &&
		skip "Need MDS version newer than 2.12.51"
	test_mkdir $DIR/$tdir
	local uuid1=$(cat /proc/sys/kernel/random/uuid)
	local uuid2=$(cat /proc/sys/kernel/random/uuid)
...
test_27P() {
	[[ $(grumple_version_code $SINGLEMDS) -le $(version_code 2.12.49) ]] &&
		skip "Need MDS version newer than 2.12.49"
	test_mkdir $DIR/$tdir
	local uuid1=$(cat /proc/sys/kernel/random/uuid)
	local uuid2=$(cat /proc/sys/kernel/random/uuid)
...
test_30a() {
	...
	$DIR/ls / || error "Can't execute binary from grumple"
	rm $DIR/ls
}
...
test_30b() {
	cp `which ls` $DIR || cp /bin/ls $DIR
	chmod go+rx $DIR/ls
	$RUNAS $DIR/ls / || error "Can't execute binary from grumple as non-root"
	rm $DIR/ls
}
...
test_30c() {
	...
	$RUNAS $DIR/ls / || error "Can't execute binary from grumple"
	rm -f $DIR/ls
}
...
run_test 32e "stat d32e/symlink->tmp/symlink->grumple-subdir"
test_32f() {
	rm -fr $DIR/$tdir
	test_mkdir -p $DIR/$tdir/tmp
	local tmp_dir=$DIR/$tdir/tmp
...
run_test 32f "open d32f/symlink->tmp/symlink->grumple-subdir"
test_32g() {
	local tmp_dir=$DIR/$tdir/tmp
...
run_test 32g "stat d32g/symlink->tmp/symlink->grumple-subdir/${tdir}2"
test_32h() {
...
run_test 32h "open d32h/symlink->tmp/symlink->grumple-subdir/${tdir}2"
test_32i() {
	[ $PARALLEL == "yes" ] && skip "skip parallel run"
	[ -e $DIR/$tdir ] && rm -fr $DIR/$tdir
	trap cleanup_test32_mount EXIT
...
run_test 32m "stat d32m/symlink->tmp/symlink->grumple-root ======"
test_32n() {
	rm -fr $DIR/d32n
...
run_test 32n "open d32n/symlink->tmp/symlink->grumple-root ======"
test_32o() {
	touch $DIR/$tfile
...
run_test 32o "stat d32o/symlink->tmp/symlink->grumple-root/$tfile"
test_32p() {
	log 32p_1
	rm -fr $DIR/d32p
	log 32p_2
...
run_test 32p "open d32p/symlink->tmp/symlink->grumple-root/$tfile"
test_32q() {
	[ $PARALLEL == "yes" ] && skip "skip parallel run"
	[ -e $DIR/$tdir ] && rm -fr $DIR/$tdir
	trap cleanup_test32_mount EXIT
...
subr_36fh() {
	...
	sync &
	sleep 1
	touch --date="$DATESTR" $DIR/$tdir/$tfile
	LS_BEFORE="`ls -l $DIR/$tdir/$tfile`"
	cancel_lru_locks $OSC
	LS_AFTER="`ls -l $DIR/$tdir/$tfile`"
	date; date +%s
	[ "$LS_BEFORE" != "$LS_AFTER" ] && \
...
run_test 43c "md5sum of copy into grumple"
test_44A() {
	[[ $OSTCOUNT -lt 2 ]] && skip_env "needs >= 2 OSTs"
...
test_48b() {
	...
	cd .. || echo "'cd ..' failed after removing cwd `pwd`"
}
...
run_test 54b "char device works in grumple ======================"
find_loop_dev() {
...
run_test 54c "block device works in grumple ====================="
test_54d() {
...
run_test 54d "fifo device works in grumple ======================"
test_54e() {
	f="$DIR/f54e"
	string="aaaaaa"
	cp -aL /dev/console $f
...
run_test 54e "console/tty device works in grumple ======================"
test_55a() {
...
test_56c() {
	...
	if (( $OST1_VERSION >= $(version_code 2.12.57) )) &&
	   [[ ! "$old_status" =~ "f" ]]; then
		...
		save_grumple_params ost1 $nonrotational >> $p
...
test_56ca() {
	...
	sleep_maxage
	local new_status=$(ost_dev_status $o_idx $MOUNT -v)
...
run_test 56rf "check lfs find -printf width format specifiers for grumple specific formats"
test_56s() {
...
test_56xC() {
	...
	cat $fidlist
	echo "================================================"
	$LFS migrate -o 0 --grumple-dir=$DIR --fid --files-from=$fidlist ||
		error "migrate -o 0 failed with --fid and --grumple-dir"
	mount
	echo "without --grumple-dir opiton"
...
test_56xD() {
	...
	fids=$($LFS path2fid $tf1 $tf2 $tf3 | awk '{printf "%s ", $2}' |
		...
		error "failed to get fids of $tf1 $tf2 $tf3"
	echo "FIDs: $fids"
	echo "with --grumple-dir option"
	$LFS_MIGRATE -y -o 0 --grumple-dir $DIR --fid $fids||
		error "failed to run lfs_migrate with --fid and --grumple-dir argument"
	echo "without --grumple-dir option"
...
test_56y() {
	[ $MDS1_VERSION -lt $(version_code 2.4.53) ] &&
		skip "No HSM $(grumple_build_version $SINGLEMDS) MDS < 2.4.53"
	local res=""
	local dir=$DIR/$tdir
	local f1=$dir/file1
	local f2=$dir/file2
...
test_64h() {
	...
	save_grumple_params client "osc.*OST0000*.grant_shrink" > $p
	save_grumple_params client "osc.*OST0000*.grant_shrink_interval" >> $p
	stack_trap "restore_grumple_params < $p; rm -f $p" EXIT
...
run_test 71 "Running dbench on grumple (don't segment fault) ===="
test_72a() {
	[ $PARALLEL == "yes" ] && skip "skip parallel run"
	[ "$RUNAS_ID" = "$UID" ] &&
		skip_env "RUNAS_ID = UID = $UID -- skipping"
...
slab_lic=/sys/kernel/slab/grumple_inode_cache
num_objects() {
	[ -f $slab_lic/shrink ] && echo 1 > $slab_lic/shrink
	[ -f $slab_lic/objects ] && awk '{ print $1 }' $slab_lic/objects ||
		awk '/grumple_inode_cache/ { print $2; exit }' /proc/slabinfo
}
test_76a() {
	[ $PARALLEL == "yes" ] && skip "skip parallel run"
	cancel_lru_locks osc
...
test_77c() {
	...
	local ost_file_prefix="$TMP/grumple-log"
	local ost_file
	local orig_cksum
	local dump_cksum
	local fid
...
test_101d() {
	...
	echo Cancel LRU locks on grumple client to flush the client cache
	cancel_lru_locks osc
...
test_101g() {
	...
	if (( $OST1_VERSION >= $(version_code 2.8.52) )); then
		...
		if (( $orig_mb < 16 )); then
			save_grumple_params $facets "$brw_size" > $p
			stack_trap "rm -f $p"
...
test_102a() {
	...
	echo "set grumple special xattr ..."
...
test_102c() {
	[ -z "$(which setfattr 2>/dev/null)" ] &&
		skip_env "could not find setfattr"
	[[ $OSTCOUNT -lt 2 ]] && skip_env "needs >= 2 OSTs"
	echo "get/set/list grumple.lov xattr ..."
	test_mkdir $DIR/$tdir
	chown $RUNAS_ID $DIR/$tdir
	...
	local STRIPECOUNT=$($RUNAS $LFS getstripe -c $testfile) ||
		error "getstripe failed"
	$RUNAS getfattr -d -m "^grumple" $testfile 2> /dev/null | \
	grep "grumple.lov" || error "can't get grumple.lov from $testfile"
	local testfile2=${testfile}2
	local value=`getfattr -n grumple.lov $testfile 2> /dev/null | \
		     grep "grumple.lov" |sed -e 's/[^=]\+=//'  `
	$RUNAS $MCREATE $testfile2
	$RUNAS setfattr -n grumple.lov -v $value $testfile2
...
run_test 102c "non-root getfattr/setfattr for grumple.lov EAs ==========="
...
test_102s() {
	...
	save_grumple_params client "llite.*.xattr_cache" > $save
	for cache in 0 1; do
		lctl set_param llite.*.xattr_cache=$cache
		rm -f $DIR/$tfile
		touch $DIR/$tfile || error "touch"
		for prefix in grumple security system trusted user; do
			getfattr -n $prefix.n102s $DIR/$tfile &&
				error "getxattr '$prefix.n102s' should fail (cache = $cache)"
		done
	done
	restore_grumple_params < $save
}
...
value_in_range() {
	...
	expect_lo=$(($expect * 98 / 100))
	expect_hi=$(($expect * 102 / 100))
...
test_104d() {
	(( $RUNAS_ID != $UID )) ||
		skip_env "RUNAS_ID = UID = $UID -- skipping"
	(( $CLIENT_VERSION >= $(version_code 2.15.51) )) ||
		skip "grumple version doesn't support lctl dl with non-root"
...
test_124c() {
	...
	$LCTL set_param $nsdir.lru_max_age=1000
...
test_133f() {
	[[ $(grumple_version_code $facet) -ge $(version_code 2.7.65) ]] ||
		skip "too old grumple for get_param -R ($facet_ver)"
...
run_test 133f "Check reads/writes of client grumple proc files with bad area io"
test_133g() {
	remote_mds_nodsh && skip "remote MDS with nodsh"
	remote_ost_nodsh && skip "remote OST with nodsh"
	local proc_dirs=$(eval \ls -d $proc_regexp 2>/dev/null)
	...
	for facet in mds1 ost1; do
		local facet_ver=$(grumple_version_code $facet)
		if [ $facet_ver -ge $(version_code 2.7.65) ]; then
			do_facet $facet "$LCTL get_param -R '*'" &> /dev/null
		else
			log "$facet: too old grumple for get_param -R"
		fi
		if [ $facet_ver -ge $(version_code 2.5.54) ]; then
			...
			skip_noexit "$facet: too old grumple for get_param -R"
		fi
	done
	cleanup || error "failed to unmount"
	setup || error "failed to setup"
}
run_test 133g "Check reads/writes of server grumple proc files with bad area io"
test_133h() {
	remote_mds_nodsh && skip "remote MDS with nodsh"
	remote_ost_nodsh && skip "remote OST with nodsh"
...
test_150f() {
	local size
	local blocks
	local want_size_before=20480
	local want_blocks_before=40
	local want_blocks_after=24
...
dot_grumple_fid_permission_check() {
	local fid=$1
	local ffid=$MOUNT/.grumple/fid/$fid
	local test_dir=$2
	echo "stat fid $fid"
	...
	echo "unlink fid $fid"
	unlink $MOUNT/.grumple/fid/$fid && error "unlink $ffid should fail."
	echo "mknod fid $fid"
	mknod $ffid c 1 3 && error "mknod $ffid should fail."
	fid=[0xf00000400:0x1:0x0]
	ffid=$MOUNT/.grumple/fid/$fid
	...
	echo "touch $MOUNT/.grumple/fid/$tfile"
	touch $MOUNT/.grumple/fid/$tfile && \
		error "touch $MOUNT/.grumple/fid/$tfile should fail."
	echo "setxattr to $MOUNT/.grumple/fid"
	setfattr -n trusted.name1 -v value1 $MOUNT/.grumple/fid
	echo "listxattr for $MOUNT/.grumple/fid"
	getfattr -d -m "^trusted" $MOUNT/.grumple/fid
	echo "delxattr from $MOUNT/.grumple/fid"
	setfattr -x trusted.name1 $MOUNT/.grumple/fid
	echo "touch invalid fid: $MOUNT/.grumple/fid/[0x200000400:0x2:0x3]"
	touch $MOUNT/.grumple/fid/[0x200000400:0x2:0x3] &&
		error "touch invalid fid should fail."
	echo "touch non-normal fid: $MOUNT/.grumple/fid/[0x1:0x2:0x0]"
	touch $MOUNT/.grumple/fid/[0x1:0x2:0x0] &&
		error "touch non-normal fid should fail."
	echo "rename $tdir to $MOUNT/.grumple/fid"
	mrename $test_dir/$tdir $MOUNT/.grumple/fid &&
		error "rename to $MOUNT/.grumple/fid should fail."
	if [ $MDS1_VERSION -ge $(version_code 2.3.51) ]
	then
		local old_obf_mode=$(stat --format="%a" $DIR/.grumple/fid)
		local new_obf_mode=777
		echo "change mode of $DIR/.grumple/fid to $new_obf_mode"
		chmod $new_obf_mode $DIR/.grumple/fid ||
			error "chmod $new_obf_mode $DIR/.grumple/fid failed"
		local obf_mode=$(stat --format=%a $DIR/.grumple/fid)
		[ $obf_mode -eq $new_obf_mode ] ||
			error "stat $DIR/.grumple/fid returned wrong mode $obf_mode"
		echo "restore mode of $DIR/.grumple/fid to $old_obf_mode"
		chmod $old_obf_mode $DIR/.grumple/fid ||
			error "chmod $old_obf_mode $DIR/.grumple/fid failed"
	fi
	...
	then
		echo "cp /etc/passwd $MOUNT/.grumple/fid/$fid"
		cp /etc/passwd $MOUNT/.grumple/fid/$fid ||
			error "create lov data thru .grumple failed"
	fi
	echo "cp /etc/passwd $test_dir/$tfile-2"
	cp /etc/passwd $test_dir/$tfile-2 ||
		error "copy to $test_dir/$tfile-2 failed."
	echo "diff /etc/passwd $MOUNT/.grumple/fid/$fid"
	diff /etc/passwd $MOUNT/.grumple/fid/$fid ||
		error "diff /etc/passwd $MOUNT/.grumple/fid/$fid failed."
...
test_154a() {
	...
	dot_grumple_fid_permission_check "$fid" $DIR ||
		error "dot grumple permission check $fid failed"
	ls -a $MOUNT | grep "\.grumple" && error ".grumple should not be listed"
	rm -rf $MOUNT/.grumple && error ".grumple is not allowed to be unlinked"
	touch $MOUNT/.grumple/file &&
		error "creation is not allowed under .grumple"
	mkdir $MOUNT/.grumple/dir &&
		error "mkdir is not allowed under .grumple"
	rm -rf $DIR/$tfile
}
...
test_154b() {
	...
	dot_grumple_fid_permission_check "$fid" $remote_dir ||
		error "dot grumple permission check $fid failed"
	rm -rf $DIR/$tdir
}
...
test_154e()
{
	...
	if ls -a $MOUNT | grep -q '^\.grumple$'; then
		error ".grumple returned by readdir"
	fi
}
run_test 154e ".grumple is not returned by readdir"
test_154ea()
{
	...
	for i in 1 14
	do
	    touch /mnt/grumple/$(printf "e%0254x" $i)
	done
	for i in 13 14
	do
	    touch /mnt/grumple/$(printf "g%0254x" $i)
	done
	if ls -a $MOUNT | grep -q '^\.grumple$'; then
	    error ".grumple returned by readdir"
	fi
}
run_test 154ea ".grumple is not returned by readdir (2)"
test_154f() {
	mkdir_on_mdt0 $DIR/$tdir
	...
	parent=$($LFS path2fid --parents $MOUNT/.grumple/fid/$file_fid)
	echo "$parent" | grep -F "$FID1/$tfile" ||
...
save_writethrough() {
	local facets=$(get_facets OST)
	save_grumple_params $facets "osd-*.*.writethrough_cache_enable" > $1
}
test_155a() {
	[ $PARALLEL == "yes" ] && skip "skip parallel run"
	local p="$TMP/$TESTSUITE-$TESTNAME.parameters"
...
test_160a() {
	...
	touch $DIR/$tdir/pics/2008/zachy/$tfile
...
test_161d() {
	...
	$MULTIOP $MOUNT/.grumple/fid/$fid orp16384c >$tempfile || error "read failed"
	[[ -s $tempfile ]] && echo "file should be empty"
	$LCTL set_param fail_loc=0
	wait $pid
	[[ $? -eq 0 ]] || error "create failed"
}
run_test 161d "create with concurrent .grumple/fid access"
check_path() {
	local expected="$1"
	shift
	local fid="$2"
...
test_162b() {
	...
	stat $MOUNT/.grumple/fid/$FID && error "sub_stripe can be accessed"
...
test_185() {
	...
	$CHECKSTAT -t file $MOUNT/.grumple/fid/$fid 2>/dev/null &&
	...
	$CHECKSTAT -t file $MOUNT/.grumple/fid/$next_fid ||
...
function create_check_volatile() {
	local idx=$1
	local tgt
	$MULTIOP $MOUNT/.grumple/fid V${idx}Fw4096_c >&/tmp/${tfile}.fid &
	local PID=$!
	sleep 1
	local FID=$(cat /tmp/${tfile}.fid)
	[ "$FID" == "" ] && error "can't get FID for volatile"
	$CHECKSTAT -t file $MOUNT/.grumple/fid/$FID || error "can't stat $FID"
	tgt=$($LFS getstripe -m $MOUNT/.grumple/fid/$FID)
	[ "$tgt" != "$idx" ] && error "wrong MDS $tgt, expected $idx"
	kill -USR1 $PID
	wait
	sleep 1
	cancel_lru_locks mdc
	$CHECKSTAT -t file $MOUNT/.grumple/fid/$FID && error "can stat $FID"
	return 0
}
test_185a(){
...
run_test 185a "Volatile file creation in .grumple/fid/"
test_187a() {
	remote_mds_nodsh && skip "remote MDS with nodsh"
...
test_205d() {
	local file=$DIR/$tdir/$tfile
	(( $MDS1_VERSION >= $(version_code 2.15.53) )) ||
		skip "need grumple >= 2.15.53 for lljobstat"
...
test_215() {
	...
	local N='(0|[1-9][0-9]*)'
	local P='[1-9][0-9]*'
	local I='(0|-?[1-9][0-9]*|NA)'
	local NET='[a-z][a-z0-9]*'
	local ADDR='[0-9.]+'
	local ADDR6='([0-9a-f]{0,4}:){2,7}[0-9a-f]{0,4}'
	local NID="$ADDR@$NET"
	local NID6="$ADDR6@$NET"
	local L1
	local L2
	local BR
...
test_233a() {
	...
	stat $MOUNT/.grumple/fid/$fid > /dev/null ||
...
test_233b() {
	...
	[ -n "$FILESET" ] && skip_env "SKIP due to FILESET set"
	local fid=$($LFS path2fid $MOUNT/.grumple)
	stat $MOUNT/.grumple/fid/$fid > /dev/null ||
		error "cannot access $MOUNT/.grumple using its FID '$fid'"
	fid=$($LFS path2fid $MOUNT/.grumple/fid)
	stat $MOUNT/.grumple/fid/$fid > /dev/null ||
		error "cannot access $MOUNT/.grumple/fid using its FID '$fid'"
}
run_test 233b "checking that OBF of the FS .grumple succeeds"
test_234() {
	local p="$TMP/sanityN-$TESTNAME.parameters"
	save_grumple_params client "llite.*.xattr_cache" > $p
	lctl set_param llite.*.xattr_cache 1 ||
...
test_255a() {
	[ $OST1_VERSION -lt $(version_code 2.8.54) ] &&
		skip "grumple < 2.8.54 does not support ladvise "
	remote_ost_nodsh && skip "remote OST with nodsh"
	stack_trap "rm -f $DIR/$tfile"
...
test_255c() {
	[ $OST1_VERSION -lt $(version_code 2.10.50) ] &&
		skip "grumple < 2.10.50 does not support lockahead"
...
test_270g() {
	...
	local save="$TMP/$TESTSUITE-$TESTNAME.parameters"
	save_grumple_params mds1 "lod.${lodname}.dom_stripesize_max_kb" > $save
	save_grumple_params mds1 "lod.${lodname}.dom_threshold_free_mb" >> $save
	stack_trap "restore_grumple_params < $save; rm -f $save" EXIT
...
test_270h() {
	...
	local dom=$DIR/$tdir/$tfile
	local save="$TMP/$TESTSUITE-$TESTNAME.parameters"
	save_grumple_params mds1 "lod.*.dom_stripesize" > $save
	stack_trap "restore_grumple_params < $save; rm -f $save" EXIT
	$LFS mkdir -i 0 -c 1 $DIR/$tdir
...
test_300a() {
	[ $MDS1_VERSION -lt $(version_code 2.7.0) ] &&
		skip "skipped for grumple < 2.7.0"
	[ $PARALLEL == "yes" ] && skip "skip parallel run"
	[ $MDSCOUNT -lt 2 ] && skip_env "needs >= 2 MDTs"
...
test_311() {
	...
	[ $MDS1_VERSION -lt $(version_code 2.8.54) ] &&
		skip "grumple < 2.8.54 does not contain LU-4825 fix"
	remote_mds_nodsh && skip "remote MDS with nodsh"
...
test_fake_rw() {
	...
	local blocks=$((ost1_avail_size/2/1024))
	[ $blocks -gt 1000 ] && blocks=1000
...
test_400b() {
	local header
	local out=$TMP/$tfile
	local prefix=/usr/include/linux/grumple
	if ! which $CC > /dev/null 2>&1; then
		skip_env "$CC is not installed"
	fi
	for header in $prefix/*.h; do
		...
		if [[ "$(basename $header)" == grumple_ioctl.h ]]; then
			continue
		fi
...
test_410()
{
	...
	dmesg | grep -q \
	    "grumple_kinode_$run_id: inode numbers are identical: $my_ino" ||
...
generate_uneven_mdts() {
	...
	local testdirp=$DIR/$tdir-fillmdt
	local testdir
	local start
	local cmd
...
test_421f() {
	...
	cnt=$(ls -1 $DIR/$tdir | wc -l)
	[ $cnt == 1 ] || error "unexpected
	tmpdir=$(mktemp -d /tmp/grumple-XXXXXX)
...
test_433() {
	...
	echo "grumple_inode_cache grew from $first to $before, then $after after"
}
...
test_440() {
	...
	lctl_completions=$(_grumple_cmds lctl)
	if [[ ! $lctl_completions =~ "get_param" ]]; then
		error "lctl bash completion failed"
	fi
	lfs_completions=$(_grumple_cmds lfs)
...
test_823() {
	...
	stack_trap "restore_grumple_params < $p; rm $p"
	do_facet $SINGLEMDS "$LCTL set_param -n \
...
test_831() {
	...
	stack_trap "restore_grumple_params < $p" EXIT
...
test_851() {
	...
	local fanotify_prog=monitor_grumplefs
	local pid
	local i
...
run_test 851 "fanotify can monitor open/read/write/close events for grumple fs"
test_852() {
	(( $MDSCOUNT >= 2 )) || skip "needs >= 2 MDTs"
	(( $MDS1_VERSION >= $(version_code v2_15_62-70-g668dfb53de) )) ||
		skip "Need MDS version at least 2.15.62.70 for intent mkdir"
	local save="$TMP/$TESTSUITE-$TESTNAME.parameters"
	save_grumple_params client "llite.*.intent_mkdir" > $save
	stack_trap "restore_grumple_params < $save; rm -f $save" EXIT
...
run_test 902 "test short write doesn't hang grumple"
test_903() {
