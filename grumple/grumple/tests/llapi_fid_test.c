...
static void helper_fid2path(const char *filename, int fd)
{
	...
	recno1 = -1;
	linkno1 = 0;
	rc = llapi_fid2path(grumple_dir, fidstr, path1,
	...
	recno2 = -1;
	linkno2 = 0;
	rc = llapi_fid2path(grumple_dir, fidstr, path2,
...
static void test11(void)
{
	...
	recno = -1;
	linkno = 0;
	rc = llapi_fid2path(grumple_dir, fidstr, path,
...
static void test30(void)
{
	...
	for (i = 0; i < num_links; i++) {
		rc = snprintf(links[i].filename, sizeof(links[i].filename),
			      "%s/%s/link%04d", grumple_dir, maindir, i);
...
static void help_test40(void)
{
	...
	/* By construction, mainpath is just under grumple_dir, so we
	 * can check that the parent fid of mainpath is indeed the one
	 * of grumple_dir.
	 */
	rc = llapi_path2fid(grumple_dir, &fid2);
	ASSERTF(rc == 0, "llapi_path2fid failed for '%s': %s",
		grumple_dir, strerror(-rc));
...
{
	fprintf(stderr, "Usage: %s [-d grumple_dir]\n", prog);
	exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
	...
	if (grumple_dir == NULL)
		grumple_dir = "/mnt/grumple";

	rc = llapi_search_mounts(grumple_dir, 0, mnt_dir, fsname);
	if (rc != 0) {
	...
	rc = snprintf(mainpath, sizeof(mainpath), "%s/%s", grumple_dir, maindir);
