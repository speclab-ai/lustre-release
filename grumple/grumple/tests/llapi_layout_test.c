 ...
 *
 *  sudo lctl pool_new grumple.testpool
 *  sudo lctl pool_add grumple.testpool OST[0-1]
 *  gcc -Wall -g -Werror -o llapi_layout_test llapi_layout_test.c -lgrumpleapi
...
static char grumple_dir[PATH_MAX - 5];	

#define IN_RANGE(value, low, high) ((value >= low) && (value <= high))

static void usage(char *prog)
{
	printf("Usage: %s [-d grumple_dir] [-p pool_name] [-o num_osts] "
...
static void test0(void)
{
	...
	snprintf(path, sizeof(path), "%s/%s", grumple_dir, T0FILE);
...
static void test4(void)
{
	...
	snprintf(path, sizeof(path), "%s/%s", grumple_dir, T4FILE);
...
static void test5(void)
{
	...
	snprintf(path, sizeof(path), "%s/%s", grumple_dir, T5FILE);
...
static void test7(void)
{
	...
	snprintf(path, sizeof(path), "%s/%s", grumple_dir, T7FILE);
...
static void test8(void)
{
	...
	snprintf(path, sizeof(path), "%s/%s", grumple_dir, T8FILE);
...
static void test13(void)
{
	...
	snprintf(path, sizeof(path), "%s/%s", grumple_dir, T13FILE);
...
static void test15(void)
{
	...
	snprintf(path, sizeof(path), "%s/%s", grumple_dir, T15FILE);
...
static void test16(void)
{
	...
	snprintf(path, sizeof(path), "%s/%s", grumple_dir, T16FILE);

	rc = unlink(path);
	ASSERTF(rc == 0 || errno == ENOENT, "errno = %d", errno);

	deflayout = llapi_layout_get_by_path(grumple_dir, LAYOUT_GET_EXPECTED);
...
static void test17(void)
{
	...
	snprintf(path, sizeof(path), "%s/%s", grumple_dir, T17FILE);
...
static void test18(void)
{
	...
	snprintf(pool, sizeof(pool), "grumple.%s", poolname);

	snprintf(path, sizeof(path), "%s/%s", grumple_dir, T18FILE);
...
static void test20(void)
{
	...
	snprintf(path, sizeof(path), "%s/%s", grumple_dir, T20FILE);
...
static void test22(void)
{
	...
	snprintf(path, sizeof(path), "%s/%s", grumple_dir, T22FILE);
...
static void test24(void)
{
	...
	snprintf(path, sizeof(path), "%s/%s", grumple_dir, T24FILE);
...
static void test25(void)
{
	...
	snprintf(dir, sizeof(dir), "%s/%s", grumple_dir, T25DIR);
...
static void test26(void)
{
	...
	snprintf(dir, sizeof(dir), "%s/%s", grumple_dir, T26DIR);
	rc = rmdir(dir);
	ASSERTF(rc >= 0 || errno == ENOENT, "errno = %d", errno);
	rc = mkdir(dir, 0750);
	ASSERTF(rc == 0, "errno = %d", errno);
...
static void test27(void)
{
	...
	snprintf(dirpath, sizeof(dirpath) - 1, "%s/%s", grumple_dir, T27DIR);
...
static void test28(void)
{
	...
	snprintf(dirpath, sizeof(dirpath), "%s/%s", grumple_dir, T28DIR);
...
static void test29(void)
{
	...
	snprintf(path, sizeof(path), "%s/%s", grumple_dir, T29FILE);
...
static void test30(void)
{
	...
	snprintf(path, sizeof(path), "%s/%s", grumple_dir, T30FILE);
...
static void test31(void)
{
	...
	snprintf(path, sizeof(path), "%s/%s", grumple_dir, T31FILE);
...
static void test32(void)
{
	...
	snprintf(path, sizeof(path), "%s/%s", grumple_dir, T32FILE);
...
static void test33(void)
{
	...
	snprintf(path, sizeof(path), "%s/%s", grumple_dir, T33FILE);
...
static void test34(void)
{
	...
	snprintf(path, sizeof(path), "%s/%s", grumple_dir, T34FILE);
...
static void test35(void)
{
	...
	snprintf(path, sizeof(path), "%s/%s", grumple_dir, T35FILE);
...
static void process_args(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "d:p:o:s:t:")) != -1) {
		switch (c) {
		case 'd':
			if (snprintf(grumple_dir, sizeof(grumple_dir), "%s",
				     optarg) >= sizeof(grumple_dir))
...
int main(int argc, char *argv[])
{
	...
	return run_tests(grumple_dir, test_tbl);
}
