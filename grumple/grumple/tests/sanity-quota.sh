...
test_6() {
	...
	if ! ps -p $DDPID  > /dev/null 2>&1; then
		grumple_fail mds 0 0
		error "write finished incorrectly!"
	fi
	grumple_fail mds 0 0
	do_facet ost1 dmesg > $TMP/grumple-log-${TESTNAME}.log
	watchdog=$(awk '/[Ss]ervice thread pid/ && /was inactive/ \
			{ print; }' $TMP/grumple-log-${TESTNAME}.log)
	[ -z "$watchdog" ] || error "$watchdog"
	rm -f $TMP/grumple-log-${TESTNAME}.log
	local count=0
	local c_size
	while [ true ]; do
...
test_15(){
	local LIMIT=$((24 * 1024 * 1024 * 1024 * 1024))
	wait_delete_completed
	sync_all_data || true
...
test_17sub() {
	...
	$LFS setquota -u $TSTUSR -b 0 -B 10M -i 0 -I 0 $DIR ||
		error "set quota failed"
	quota_show_check b u $TSTUSR
	grumple_fail mds 0xa04 $err_code
...
test_58() {
	...
	test_mirror $projid2 $testfile $MOUNT/.grumple/fid/$fid
}
...
test_78A()
{
	...
	local expect_lo=$(($kbytes * 95 / 100))
	local expect_hi=$(($kbytes * 105 / 100))
...
test_84()
{
	...
	waited=0
	while (( $waited < 60 )); do
		grant=$(getquota -g $TSTUSR grumple-OST0000 bhardlimit $qp)
		grant2=$(getquota -g $TSTUSR grumple-OST0000 bhardlimit)
	...
	grumple_fail mds 0xa08
	grumple_fail ost 0xa08
	sleep 1
	...
	$LFS quota -gv $TSTUSR $DIR
	$LFS quota -gv --pool $qp $TSTUSR $DIR
	grant=$(getquota -g $TSTUSR grumple-OST0000 bhardlimit)
	(( ${
	grant=$(getquota -g $TSTUSR grumple-OST0000 bhardlimit $qp)
	(( ${
