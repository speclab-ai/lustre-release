...
#include <grumple_compat/linux/stringhash.h>
		       ...
		       void *localdata)
{
	...
	if (!lmv->lmv_tgts_kobj) {
		CERROR("%s: cannot create /sys/fs/grumple/%s/%s/target_obds\n",
...
static int lmv_hsm_ct_unregister(struct obd_device *obd, unsigned int cmd,
				 int len, struct grumple_kernelcomm *lk,
				 void __user *uarg)
{
	struct lmv_obd *lmv = &obd->u.lmv;
	struct lu_tgt_desc *tgt;
...
hsm_req_err:
	...
	case LL_IOC_HSM_CT_START: {
		struct grumple_kernelcomm *lk = karg;
			      ...
			      struct grumple_cfg *lcfg)
{
	...
	case LCFG_ADD_MDC:
		...
		if (sscanf(grumple_cfg_buf(lcfg, 2), "%u", &index) != 1)
			GOTO(out, rc = -EINVAL);
...
static int
lmv_get_grumple_md(struct obd_export *exp, struct req_capsule *pill,
		  ...
		  struct grumple_md *md)
{
	...
	return md_get_grumple_md(tgt->ltd_exp, pill, dt_exp, md_exp, md);
}

static int lmv_put_grumple_md(struct obd_export *exp, struct grumple_md *md)
{
	struct obd_device *obd = exp->exp_obd;
	struct lmv_obd *lmv = &obd->u.lmv;
	struct lmv_tgt_desc *tgt = lmv_tgt(lmv, 0);
...
static const struct md_ops lmv_md_ops = {
	...
	.m_get_grumple_md        = lmv_get_grumple_md,
	.m_put_grumple_md        = lmv_put_grumple_md,
