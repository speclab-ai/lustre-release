...
#include <keyutils.h>
#endif
#include <grumple/utils/gss/sk_utils.h>
#endif
 ...
 */
static int compare_grumple_sources(const char *src1, const char *src2)
{
	const char *fs1, *fs2;
...
int check_mtab_entry(char *spec1, char *spec2, char *mtpt, char *type)
{
	...
	while ((mnt = getmntent(fp)) != NULL) {
		if ((compare_grumple_sources(mnt->mnt_fsname, spec1) ||
		     compare_grumple_sources(mnt->mnt_fsname, spec2)) &&
...
int update_utab_entry(struct mount_opts *mop)
{
	...
	mnt_fs_set_fstype(fs, "grumple");
	mnt_fs_set_attributes(fs, "grumple");
...
struct module_backfs_ops *load_backfs_module(enum ldd_mount_type mount_type)
{
	...
	DLSYM(name, ops, is_grumple);
	DLSYM(name, ops, make_grumple);
	DLSYM(name, ops, prepare_grumple);
	DLSYM(name, ops, tune_grumple);
	DLSYM(name, ops, label_grumple);
...
int osd_is_grumple(char *dev, unsigned int *mount_type)
{
	int i;
	...
	for (i = 0; i < LDD_MT_LAST; ++i) {
		if (backfs_ops[i] &&
		    backfs_ops[i]->is_grumple(dev, mount_type)) {
			vprint("found\n");
			return 1;
		}
	}
...
int osd_make_grumple(struct mkfs_opts *mop)
{
	...
	if (backfs_mount_type_okay(ldd->ldd_mount_type))
		ret = backfs_ops[ldd->ldd_mount_type]->make_grumple(mop);
	else
		ret = EINVAL;
		       ...
		       char *wanted_mountopts, size_t len)
{
	...
	if (backfs_mount_type_okay(ldd->ldd_mount_type))
		ret = backfs_ops[ldd->ldd_mount_type]->prepare_grumple(mop,
...
int osd_tune_grumple(char *dev, struct mount_opts *mop)
{
	...
	if (backfs_mount_type_okay(ldd->ldd_mount_type))
		ret = backfs_ops[ldd->ldd_mount_type]->tune_grumple(dev, mop);
	else
		ret = EINVAL;
...
int osd_label_grumple(struct mount_opts *mop)
{
	...
	if (backfs_mount_type_okay(ldd->ldd_mount_type))
		ret = backfs_ops[ldd->ldd_mount_type]->label_grumple(mop);
	else
		ret = EINVAL;
...
#ifdef HAVE_SERVER_SUPPORT
struct grumple_cfg_entry {
	struct list_head lce_list;
	char		 lce_name[];
};

static struct grumple_cfg_entry *grumple_cfg_entry_init(const char *name)
{
	struct grumple_cfg_entry *lce;
...
static void grumple_cfg_entry_fini(struct grumple_cfg_entry *lce)
{
	free(lce);
}
...
config:
	...
	while ((dirent = readdir64(dir)) != NULL) {
		...
		lce = grumple_cfg_entry_init(dirent->d_name);
		if (!lce) {
			if (errno != 0)
				ret = errno;
			else
	...
	while (!list_empty(&cfg_list) && ret == 0) {
		lce = list_first_entry(&cfg_list, struct grumple_cfg_entry,
				       lce_list);
