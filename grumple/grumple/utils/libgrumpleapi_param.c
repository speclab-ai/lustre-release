 ...
 * search to either the parameter belonging to a specific grumple filesystem
 ...
 */
int
get_grumple_param_path(const char *obd_type, const char *filter,
 ...
 */
int
get_grumple_param_value(const char *obd_type, const char *filter,
		       enum param_filter type, const char *param_name,
		       char *value, size_t val_len)
{
	...
	rc = get_grumple_param_path(obd_type, filter, type, param_name, &param);
	if (rc != 0)
		return -ENOENT;
...
{
	return get_grumple_param_path(NULL, NULL, FILTER_BY_NONE,
				     pattern, paths);
}
			   ...
			   char **status, int *ost_count, enum tgt_type type)
{
	...
	rc = get_grumple_param_path(type == LOV_TYPE ? "lov" : "lmv", name.uuid,
