...
int lcfg_ioctl(char *func, int dev_id, struct grumple_cfg *lcfg)
{
...
int lcfg_mgs_ioctl(const char *func, int dev_id, struct grumple_cfg *lcfg)
{
	struct obd_ioctl_data data;
	char rawbuf[MAX_IOC_BUFLEN], *buf = rawbuf;
	int rc;
 ...
 * -t grumple <MDT partition> -o nosvc <mount point>
		    ...
		    char *fullpoolname, char *fsname, char *ostname)
{
	...
	grumple_cfg_bufs_set_string(&bufs, 0, cmdname);
	grumple_cfg_bufs_set_string(&bufs, 1, fullpoolname);
	if (ostname)
		grumple_cfg_bufs_set_string(&bufs, 2, ostname);
		       ...
		       void *ret_data, unsigned int ret_size, ...)
{
	...
	arg = va_arg(ap, char *);
	while (arg) {
		grumple_cfg_bufs_set_string(&bufs, i, arg);
		i++;
		arg = va_arg(ap, char *);
	}
	va_end(ap);
