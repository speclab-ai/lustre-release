 ...
 * E.g. obdfilter.grumple-OST0000.stats
 *
...
/**
 * The application lctl can perform three operations for grumple
...
static int listparam_cmdline(int argc, char **argv, struct param_opts *popt)
{
	 ...
	 * are split between under both /sys/kernel/debug/grumple
	 * and /sys/fs/grumple. The parameter files containing
	 * small amounts of data, less than a page in size, are
	 * located under /sys/fs/grumple and in the case of large
...
int jt_lcfg_setparam_client(int argc, char **argv, struct param_opts *popt)
{
	...
	if (popt->po_show_name)
		printf("params %s /etc/grumple/mount.%s.params:\n",
