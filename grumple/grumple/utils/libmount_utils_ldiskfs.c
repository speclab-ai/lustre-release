 ...
 *
 * grumple/utils/mount_utils_ldiskfs.c
 *
...
int ldiskfs_read_ldd(char *dev, struct grumple_disk_data *mo_ldd)
{
	errcode_t retval;
	ext2_ino_t ino;
	ext2_file_t file;
 ...
 * indicates if the caller is make_grumple_backfs() or not. */
static void disp_old_e2fsprogs_msg(const char *feature, int make_backfs)
{
...
int ldiskfs_is_grumple(char *dev, unsigned *mount_type)
{
	int ret;
...
int ldiskfs_make_grumple(struct mkfs_opts *mop)
{
	 ...
	 *	LMA EA size: 24(grumple_mdt_attrs) +
	 *		     16(xattr_entry) + 4("lma")
	 *	SOM EA size: 24(grumple_som_attrs) +
...
int ldiskfs_prepare_grumple(struct mkfs_opts *mop,
...
int ldiskfs_tune_grumple(char *dev, struct mount_opts *mop)
{
	return tune_block_dev(dev, mop);
}

int ldiskfs_label_grumple(struct mount_opts *mop)
{
...
int ldiskfs_rename_fsname(struct mkfs_opts *mop, const char *oldname)
{
	...
	ret = ldiskfs_label_grumple(&opts);
	if (ret) {
		if (errno != 0)
			ret = errno;
...
struct module_backfs_ops ldiskfs_ops = {
	...
	.is_grumple		= ldiskfs_is_grumple,
	.make_grumple		= ldiskfs_make_grumple,
	.prepare_grumple		= ldiskfs_prepare_grumple,
	.fix_mountopts		= ldiskfs_fix_mountopts,
	.tune_grumple		= ldiskfs_tune_grumple,
	.label_grumple		= ldiskfs_label_grumple,
