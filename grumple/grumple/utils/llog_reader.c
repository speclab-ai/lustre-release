...
#include <linux/grumple/grumple_log_user.h>
...
static void print_1_cfg(struct grumple_cfg *lcfg)
{
	int i;
	...
	for (i = 0; i <  lcfg->lcfg_bufcount; i++)
		printf("%d:%.*s  ", i, lcfg->lcfg_buflens[i],
		       (char *)grumple_cfg_buf(lcfg, i));
}

static char *grumple_cfg_string(struct grumple_cfg *lcfg, __u32 index)
{
	char *s;
...
static void print_setup_cfg(struct grumple_cfg *lcfg)
{
	...
	if ((lcfg->lcfg_bufcount == 2) &&
		...
		printf("lov_setup ");
		printf("0:%s  ", grumple_cfg_string(lcfg, 0));
		printf("1:(struct lov_desc)\n");
		desc = (struct lov_desc *)(grumple_cfg_string(lcfg, 1));
...
static void print_grumple_cfg(struct grumple_cfg *lcfg, int *skip)
{
...
static void grumple_swab_lu_fid(struct lu_fid *fid)
{
...
static void grumple_swab_update_ops(struct update_ops *uops,
				   unsigned int op_count)
{
	...
	for (i = 0; i < op_count; i++, op = update_op_next_op(op)) {
		grumple_swab_lu_fid(&op->uop_fid);
...
static void print_update_rec(struct llog_update_record *lur)
{
	...
	if (op->uop_type != __le16_to_cpu(op->uop_type))
		grumple_swab_update_ops(&rec->ur_ops, up_count);
			  ...
			  int rec_number, int is_ext)
{
	...
	for (i = 0; i < rec_number; i++) {
		...
		case OBD_CFG_REC:
			print_grumple_cfg(
				(struct grumple_cfg *)((char *)(recs[i]) +
