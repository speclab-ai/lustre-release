 ...
 *
 * grumpleapi library for Persistent Client Cache.
 *
...
static struct cYAML *read_pin_xattr_object(const char *path)
{
	...
	rc = getxattr(path, XATTR_GRUMPLE_PIN, buff, sizeof(buff));
	if (rc < 0)
		goto out;
...
set:
	rc = setxattr(path, XATTR_GRUMPLE_PIN, buff, strlen(buff), 0);
	if (rc < 0)
		rc = -errno;
out:
	return rc;
...
int llapi_pcc_unpin_file(const char *path, __u32 id)
{
	...
	if (strlen(buff) == 0)
		rc = removexattr(path, XATTR_GRUMPLE_PIN);
	else
		rc = setxattr(path, XATTR_GRUMPLE_PIN, buff, strlen(buff), 0);
