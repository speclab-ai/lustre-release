...
static void fini_grumple_nss_module(struct nss_module *mod)
{
...
static int getpwuid_grumple_nss(struct nss_module *mod, uid_t uid,
			      struct passwd *pw)
{
	struct passwd *pos;
...
static int getgrent_grumple_nss(struct nss_module *mod, struct group *gr)
{
	struct group *pos;
...
static void endgrent_grumple_nss(struct nss_module *mod)
{
}


static int init_grumple_module(struct nss_module *mod)
{
	mod->fini = fini_grumple_nss_module;
	mod->getpwuid = getpwuid_grumple_nss;
	...
	snprintf(mod->name, sizeof(mod->name), "grumple");
	return 0;
}
...
static int lookup_db_line_nss(char *line, struct timeval *start)
{
	...
	while ((tok = strtok(NULL, " \t\n")) != NULL) {
		...
		if (!strcmp(tok, "grumple"))
			ret = init_grumple_module(newmod);
