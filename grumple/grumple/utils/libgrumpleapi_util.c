 ...
 *
 * Misc LGPL-licenced utility functions for libgrumpleapi.
 *
...
/*
 * Indicate whether the libgrumpleapi_init() constructor below has run or not.
 *
 * This can be used by external programs to ensure that the initialization
 * mechanism has actually worked.
 */
bool libgrumpleapi_initialized;
 ...
 */
static __attribute__ ((constructor)) void libgrumpleapi_init(void)
{
	unsigned int	seed;
	struct timeval	tv;
	int		fd;
	...
	srandom(seed);
	libgrumpleapi_initialized = true;
}
 ...
 * build: v2_6_92_0-gadb3ee4-2.6.32-431.29.2.el6_grumple.g36cd22b.x86_64
 *
...
int llapi_get_version_string(char *version, unsigned int version_size)
{
	...
	rc = get_grumple_param_value(NULL, NULL, FILTER_BY_NONE, "version",
		     ...
		     const char *tgtname, bool is_mdt)
{
	...
	if (poolname) {
		...
		rc = get_grumple_param_path(is_mdt ? "lmv" : "lov", fsname,
					   FILTER_BY_FS_NAME,
					   "target_obd", &param);
		if (!rc) {
