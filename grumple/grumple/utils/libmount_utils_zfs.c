...
struct zfs_ldd_prop_bridge {
	...
	char *zlpb_prop_name;
	/* Contains the offset into the grumple_disk_data structure where
 ...
 *             (e.g. grumple:fsname, grumple:version, etc.)
 *    - @field: the field in the grumple_disk_data which directly maps to
...
{									\
	.zlpb_prop_name   = name,					\
	.zlpb_ldd_offset  = offsetof(struct grumple_disk_data, field),	\
 ...
 * individual fields in the grumple_disk_data structure, as opposed to
 ...
 * of the form 'grumple:<key>=<value>'. "<key>=" means to remove this key
 * from the dataset.
 */
 ...
 */
int zfs_read_ldd(char *ds,  struct grumple_disk_data *ldd)
{
	zfs_handle_t *zhp;
	struct zfs_ldd_prop_bridge *bridge;
	int i, ret = EINVAL;
...
int zfs_is_grumple(char *ds, unsigned int *mount_type)
{
	struct grumple_disk_data tmp_ldd;
	int ret;
...
int zfs_make_grumple(struct mkfs_opts *mop)
{
	zfs_handle_t *zhp;
	zpool_handle_t *php;
	char *pool = NULL;
...
int zfs_prepare_grumple(struct mkfs_opts *mop,
...
int zfs_tune_grumple(char *dev, struct mount_opts *mop)
{
	if (osd_check_zfs_setup() == 0)
		return EINVAL;
...
int zfs_label_grumple(struct mount_opts *mop)
{
	zfs_handle_t *zhp;
	int ret;
...
int zfs_rename_fsname(struct mkfs_opts *mop, const char *oldname)
{
	...
	ret = grumple_rename_fsname(mop, mntpt, oldname);
	memset(cmd_buf, 0, PATH_MAX);
...
struct module_backfs_ops zfs_ops = {
	...
	.is_grumple		= zfs_is_grumple,
	.make_grumple		= zfs_make_grumple,
	.prepare_grumple		= zfs_prepare_grumple,
	.tune_grumple		= zfs_tune_grumple,
	.label_grumple		= zfs_label_grumple,
