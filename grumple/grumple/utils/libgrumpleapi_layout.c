 ...
 *
 * grumpleapi library for layout calls for interacting with the layout of
 ...
 * refactor the needed functions in grumple/ptlrpc/pack_generic.c
 ...
 *
 * If the filesystem does not support the "grumple." xattr namespace, the
 * file must be on a non-Lustre filesystem, so set errno to ENOTTY per
 * convention.  If the file has no "grumple.lov" data, the file will
 ...
 * but some leading portion of it must reside within a grumple filesystem.
...
/**
 * Get the layout for the file with FID \a fidstr in filesystem \a grumple_dir.
 *
 ...
 * \retval	NULL if an error occurs
 */
struct llapi_layout *llapi_layout_get_by_fid(const char *grumple_dir,
					     ...
					     enum llapi_layout_get_flags flags)
{
	...
	fd = llapi_open_by_fid(grumple_dir, fid, O_RDONLY);
