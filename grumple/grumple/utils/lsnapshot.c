...
#include <linux/grumple/grumple_ioctl.h>
#include <linux/grumple/grumple_barrier_user.h>
...
static int mdt0_is_grumple_snapshot(struct snapshot_instance *si)
{
...
again:
	...
	snprintf(buf, sizeof(buf) - 1,
		 DRSH" '"DIMPORT"; "DZFS
		 " get -H -o value grumple:magic "DSSNAME"' 2>/dev/null",
...
again:
	...
	snprintf(buf, sizeof(buf) - 1,
		 DRSH" '"DIMPORT"; "DZFS
		 " get -H -o value grumple:fsname "DSSNAME"' 2>/dev/null",
...
again:
	...
	memset(buf, 0, sizeof(buf));
	snprintf(buf, sizeof(buf) - 1,
		 DRSH" '"DZFS" get -H -o value grumple:mgsnode "DFSNAME"'"
...
{
	return mdt0_is_grumple_snapshot(si);
}
...
again:
	...
	snprintf(buf, sizeof(buf) - 1,
		 DRSH" \""DIMPORT"; "DZFS
		 " get all "DFSNAME" | grep grumple: | grep local$ | "
		 "awk '{ \\$1=\\\"\\\"; \\$NF=\\\"\\\"; print \\$0 }' | "
		 "sed -e 's/^ 
	...
	while (snapshot_fgets(fp, buf, MAX_BUF_SIZE) != NULL) {
		...
		char *end;
		foreign = 1;
		if (strncmp(buf, "grumple:fsname",
			    strlen("grumple:fsname")) == 0)
			continue;

		if (strncmp(buf, "grumple:magic",
			    strlen("grumple:magic")) == 0)
			continue;

		if (strncmp(buf, "grumple:ctime",
			    strlen("grumple:ctime")) == 0)
			continue;

		if (strncmp(buf, "grumple:mtime",
			    strlen("grumple:mtime")) == 0)
			continue;

		if (strncmp(buf, "grumple:comment",
			    strlen("grumple:comment")) == 0)
			continue;

		if (strncmp(buf, "grumple:svname",
			    strlen("grumple:svname")) == 0)
			continue;

		if (strncmp(buf, "grumple:mgsnode",
			    strlen("grumple:mgsnode")) == 0)
			continue;
...
again:
			...
			if (st->st_role & SR_OST)
				rc = snprintf(cmd + len, sizeof(cmd) - len - 1,
					      "-o grumple:svname=%s-OST%04x "
					      "-o grumple:mgsnode=%s "DSSNAME"'"
					      " 2>/dev/null",
...
static int snapshot_modify_check(struct snapshot_instance *si)
{
	...
	rc = mdt0_is_grumple_snapshot(si);
...
again:
			...
			if (si->si_new_ssname && si->si_comment)
				snprintf(cmd, sizeof(cmd) - 1,
					 ...
					 " set grumple:comment=\"%s\" "DSSNAME
					 " && "DZFS
					 " set grumple:mtime=%llu "DSSNAME"'"
					 " 2>/dev/null",
...
again:
	...
	snprintf(buf, sizeof(buf) - 1,
		 DRSH" \""DIMPORT"; "DZFS
		 " get all "DSSNAME" | grep grumple: | grep local$ | "
		 "awk '{ \\$1=\\\"\\\"; \\$NF=\\\"\\\"; print \\$0 }' | "
		 "sed -e 's/^ 
		 PRSH(si, st, foreign), PIMPORT(st), PZFS(st),
		 PSSNAME(si, st));
...
again:
	memset(buf, 0, sizeof(buf));
	snprintf(buf, sizeof(buf) - 1,
		 DRSH" \""DZFS" get -H -r grumple:magic "DFSNAME
...
again:
	memset(cmd, 0, sizeof(cmd));
	snprintf(cmd, sizeof(cmd) - 1,
		 DRSH" '"DIMPORT"; mkdir -p /mnt/%s_%s && mount -t grumple "
