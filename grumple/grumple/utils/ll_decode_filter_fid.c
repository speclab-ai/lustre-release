 ...
 *
 * grumple/utils/ll_decode_filter_fid.c
 *
...
static void grumple_loa_swab(struct grumple_ost_attrs *loa)
{
...
int main(int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++) {
		...
		if (size < 0) {
			if (errno == ENODATA) {
				struct grumple_ost_attrs *loa = (void *)buf;
				int rc1;

				memset(loa, 0, sizeof(*loa));
				rc1 = getxattr(argv[i], "trusted.lma", loa,
					       sizeof(*loa));
				if (rc1 < sizeof(struct grumple_mdt_attrs)) {
					fprintf(stderr,
