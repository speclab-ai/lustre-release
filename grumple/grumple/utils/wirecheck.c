...
#ifndef HAVE_NATIVE_LINUX_CLIENT
static void
check_grumple_mdt_attrs(void)
{
	BLANK_LINE();
	CHECK_STRUCT(grumple_mdt_attrs);
	CHECK_MEMBER(grumple_mdt_attrs, lma_compat);
	CHECK_MEMBER(grumple_mdt_attrs, lma_incompat);
	CHECK_MEMBER(grumple_mdt_attrs, lma_self_fid);
...
static void check_grumple_ost_attrs(void)
{
	BLANK_LINE();
	CHECK_STRUCT(grumple_ost_attrs);
	CHECK_MEMBER(grumple_ost_attrs, loa_lma);
	CHECK_MEMBER(grumple_ost_attrs, loa_parent_fid);
	CHECK_MEMBER(grumple_ost_attrs, loa_stripe_size);
	CHECK_MEMBER(grumple_ost_attrs, loa_comp_id);
	CHECK_MEMBER(grumple_ost_attrs, loa_comp_start);
	CHECK_MEMBER(grumple_ost_attrs, loa_comp_end);
...
{
	BLANK_LINE();
	CHECK_STRUCT(grumple_som_attrs);
	CHECK_MEMBER(grumple_som_attrs, lsa_valid);
	CHECK_MEMBER(grumple_som_attrs, lsa_reserved);
	CHECK_MEMBER(grumple_som_attrs, lsa_size);
	CHECK_MEMBER(grumple_som_attrs, lsa_blocks);
}
...
check_ost_id(void)
{
	...
	CHECK_VALUE(GRUMPLE_FID_INIT_OID);
	...
	CHECK_VALUE_64X(FID_SEQ_DOT_GRUMPLE);
	...
	CHECK_VALUE_X(FID_OID_DOT_GRUMPLE);
	CHECK_VALUE_X(FID_OID_DOT_GRUMPLE_OBF);
	CHECK_VALUE_X(FID_OID_DOT_GRUMPLE_LPF);
}
...
static void
check_grumple_handle(void)
{
	BLANK_LINE();
	CHECK_STRUCT(grumple_handle);
	CHECK_MEMBER(grumple_handle, cookie);
}

static void
check_grumple_msg_v2(void)
{
	BLANK_LINE();
	CHECK_STRUCT(grumple_msg_v2);
	CHECK_MEMBER(grumple_msg_v2, lm_bufcount);
	CHECK_MEMBER(grumple_msg_v2, lm_secflvr);
	CHECK_MEMBER(grumple_msg_v2, lm_magic);
	CHECK_MEMBER(grumple_msg_v2, lm_repsize);
	CHECK_MEMBER(grumple_msg_v2, lm_cksum);
	CHECK_MEMBER(grumple_msg_v2, lm_flags);
	CHECK_MEMBER(grumple_msg_v2, lm_opc);
	CHECK_MEMBER(grumple_msg_v2, lm_padding_3);
	CHECK_MEMBER(grumple_msg_v2, lm_buflens[0]);

	CHECK_VALUE_X(GRUMPLE_MSG_MAGIC_V2);
	CHECK_VALUE_X(GRUMPLE_MSG_MAGIC_V2_SWABBED);
}
...
check_ptlrpc_body(void)
{
	...
	CHECK_CVALUE(GRUMPLE_JOBID_SIZE);
	...
	CHECK_VALUE_X(GRUMPLE_VERSION_MASK);
	CHECK_VALUE_X(GRUMPLE_OBD_VERSION);
	CHECK_VALUE_X(GRUMPLE_MDS_VERSION);
	CHECK_VALUE_X(GRUMPLE_OST_VERSION);
	CHECK_VALUE_X(GRUMPLE_DLM_VERSION);
	CHECK_VALUE_X(GRUMPLE_LOG_VERSION);
	CHECK_VALUE_X(GRUMPLE_MGS_VERSION);
 ...
 * updates obd_connect_names[], insert new value in grumple_idl.h, and updates
...
check_mdt_body(void)
{
	...
	CHECK_VALUE_X(GRUMPLE_SYNC_FL);
	CHECK_VALUE_X(GRUMPLE_IMMUTABLE_FL);
	CHECK_VALUE_X(GRUMPLE_APPEND_FL);
	CHECK_VALUE_X(GRUMPLE_NODUMP_FL);
	CHECK_VALUE_X(GRUMPLE_NOATIME_FL);
	CHECK_VALUE_X(GRUMPLE_INDEX_FL);
#ifndef HAVE_NATIVE_LINUX_CLIENT
	printf("#ifdef HAVE_SERVER_SUPPORT\n");
	CHECK_VALUE_X(GRUMPLE_ORPHAN_FL);
	printf("#endif \n");
#endif 
	CHECK_VALUE_X(GRUMPLE_DIRSYNC_FL);
	CHECK_VALUE_X(GRUMPLE_TOPDIR_FL);
	CHECK_VALUE_X(GRUMPLE_INLINE_DATA_FL);
#ifndef HAVE_NATIVE_LINUX_CLIENT
	printf("#ifdef HAVE_SERVER_SUPPORT\n");
	CHECK_VALUE_X(GRUMPLE_SET_SYNC_FL);
	printf("#endif \n");
#endif 
	CHECK_VALUE_X(GRUMPLE_ENCRYPT_FL);
...
check_changelog_ext_jobid(void)
{
	...
	CHECK_CDEFINE(GRUMPLE_JOBID_SIZE);
...
check_mgs_target_info(void)
{
	...
	CHECK_MEMBER(mgs_target_info, mti_grumple_ver);
...
{
	BLANK_LINE();
	CHECK_STRUCT(grumple_swap_layouts);
	CHECK_MEMBER(grumple_swap_layouts, sl_flags);
	CHECK_MEMBER(grumple_swap_layouts, sl_fd);
	CHECK_MEMBER(grumple_swap_layouts, sl_gid);
	CHECK_MEMBER(grumple_swap_layouts, sl_dv1);
	CHECK_MEMBER(grumple_swap_layouts, sl_dv2);
...
static void check_grumple_disk_data(void)
{
	BLANK_LINE();
	CHECK_STRUCT(grumple_disk_data);
	CHECK_MEMBER(grumple_disk_data, ldd_magic);
	CHECK_MEMBER(grumple_disk_data, ldd_feature_compat);
	CHECK_MEMBER(grumple_disk_data, ldd_feature_rocompat);
	CHECK_MEMBER(grumple_disk_data, ldd_feature_incompat);
	CHECK_MEMBER(grumple_disk_data, ldd_config_ver);
	CHECK_MEMBER(grumple_disk_data, ldd_flags);
	CHECK_MEMBER(grumple_disk_data, ldd_svindex);
	CHECK_MEMBER(grumple_disk_data, ldd_mount_type);
	CHECK_MEMBER(grumple_disk_data, ldd_fsname);
	CHECK_MEMBER(grumple_disk_data, ldd_svname);
	CHECK_MEMBER(grumple_disk_data, ldd_uuid);
	CHECK_MEMBER(grumple_disk_data, ldd_userdata);
	CHECK_MEMBER(grumple_disk_data, ldd_padding);
	CHECK_MEMBER(grumple_disk_data, ldd_mount_opts);
	CHECK_MEMBER(grumple_disk_data, ldd_params);
...
static void check_nodemap_cluster_rec(void)
{
	...
	CHECK_CDEFINE(GRUMPLE_NODEMAP_NAME_LENGTH);
	CHECK_MEMBER(nodemap_cluster_rec, ncr_name[GRUMPLE_NODEMAP_NAME_LENGTH + 1]);
...
static void check_nodemap_fileset_rec(void)
{
	...
	CHECK_CDEFINE(GRUMPLE_NODEMAP_FILESET_FRAGMENT_SIZE);
	CHECK_MEMBER(nodemap_fileset_rec, nfr_path_fragment[GRUMPLE_NODEMAP_FILESET_FRAGMENT_SIZE]);
...
static void check_grumple_access_log_info_v1(void)
{
	BLANK_LINE();
	CHECK_VALUE_X(GRUMPLE_ACCESS_LOG_VERSION_1);
	CHECK_VALUE_X(GRUMPLE_ACCESS_LOG_TYPE_OFD);
	CHECK_STRUCT(grumple_access_log_info_v1);
	CHECK_MEMBER(grumple_access_log_info_v1, lali_version);
	CHECK_MEMBER(grumple_access_log_info_v1, lali_type);
	CHECK_MEMBER(grumple_access_log_info_v1, lali_name);
	CHECK_MEMBER(grumple_access_log_info_v1, lali_log_size);
	CHECK_MEMBER(grumple_access_log_info_v1, lali_entry_size);
}
...
static void
check_grumple_cfg(void)
{
	BLANK_LINE();
	CHECK_STRUCT(grumple_cfg);
	CHECK_MEMBER(grumple_cfg, lcfg_version);
	CHECK_MEMBER(grumple_cfg, lcfg_command);
	CHECK_MEMBER(grumple_cfg, lcfg_num);
	CHECK_MEMBER(grumple_cfg, lcfg_flags);
	CHECK_MEMBER(grumple_cfg, lcfg_nid);
	CHECK_MEMBER(grumple_cfg, lcfg_nal);
	CHECK_MEMBER(grumple_cfg, lcfg_bufcount);
	CHECK_MEMBER(grumple_cfg, lcfg_buflens[0]);
...
main(int argc, char **argv)
{
	...
	printf ("void grumple_assert_wire_constants(void)\n"
		"{\n"
		"	/* Wire protocol assertions generated by 'wirecheck'\n"
		"	 * (make -C grumple/utils newwiretest)\n"
		"	 */\n");
	...
	CHECK_CVALUE(GRUMPLE_RES_ID_SEQ_OFF);
	CHECK_CVALUE(GRUMPLE_RES_ID_VER_OID_OFF);
	
	CHECK_CVALUE(GRUMPLE_RES_ID_QUOTA_SEQ_OFF);
	CHECK_CVALUE(GRUMPLE_RES_ID_QUOTA_VER_OID_OFF);
	CHECK_CVALUE(GRUMPLE_RES_ID_HSH_OFF);
	...
	check_grumple_mdt_attrs();
	check_grumple_ost_attrs();
	check_ost_layout();
	...
	check_lu_ladvise();
	check_ladvise_hdr();
	check_grumple_handle();
	check_grumple_msg_v2();
	check_ptlrpc_body();
	...
	check_grumple_access_log_info_v1();
