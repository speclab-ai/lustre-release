...
/* hsm_copytool_private will hold an open FD on the grumple mount point
...
{
	fprintf(stdout,
	" Usage: %s [options]... <mode> <grumple_mount_point>\n"
...
static int ct_path_grumple(char *buf, int sz, const char *mnt,
			  const struct lu_fid *fid)
{
	return scnprintf(buf, sz, "%s/%s/fid/"DFID_NOBRACE, mnt,
			 dot_grumple_name, PFID(fid));
}
			    ...
			    int mdt_index, int open_flags)
{
	...
	rc = llapi_hsm_action_begin(phcp, ctdata, hai, mdt_index, open_flags,
				    false);
	if (rc < 0) {
		ct_path_grumple(src, sizeof(src), opt.o_mnt, &hai->hai_fid);
		   ...
		   const struct hsm_action_item *hai, int hp_flags, int ct_rc)
{
	...
	ct_path_grumple(lstr, sizeof(lstr), opt.o_mnt, &hai->hai_fid);
...
static int ct_archive(const struct hsm_action_item *hai, const long hal_flags)
{
	...
	/* we fill archive so:
	 * source = data FID
	 * destination = grumple FID
	 */
	ct_path_grumple(src, sizeof(src), opt.o_mnt, &hai->hai_dfid);
...
static int ct_restore(const struct hsm_action_item *hai, const long hal_flags)
{
	...
	struct lu_fid			 dfid;
	/* we fill grumple so:
	 * source = grumple FID in the backend
...
static int ct_import_fid(const struct lu_fid *import_fid)
{
	...
	ct_path_grumple(fid_path, sizeof(fid_path), opt.o_mnt, import_fid);
...
static void handler(int signal)
{
	 ...
	 * and doesn't remove us from mtab (EINPROGRESS). The grumple client
