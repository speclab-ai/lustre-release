...
#define PATH_FORMAT "/etc/grumple/mount.%s.params"
...
static void usage(FILE *out)
{
	...
	fprintf(out,
		...
		"\t\t\t<filesystem>: Lustre filesystem name (e.g. grumple1)\n"
		"\t\t\t<subdir>: subdirectory of the filesystem to mount\n"
		"\t<mountpt>: filesystem mountpoint (e.g. /mnt/grumple)\n"
		...
		"\t\texclude=<ostname>[:<ostname>]: list of inactive OSTs (e.g. grumple-OST0001)\n"
...
#endif
		"\t\t(no)user_fid2path: disable* or enable user $MOUNT/.grumple/fid access\n"
...
/*
 * 1  = don't pass on to grumple
 * 0  = pass on to grumple
 */
			 ...
			 int *flagp, size_t options_len)
{
	...
	*flagp = 0;
	 ...
	 * we should stop forcing MS_STRICTATIME in grumple mounts.
...
static int clear_update_ondisk(char *source, struct grumple_disk_data *ldd)
{
...
{
	struct grumple_disk_data *ldd = &mop->mo_ldd;
	char *cur, *start;
	char *temp_options;
	bool skip_mgsnode_param = false;
	int rc = 0;

	rc = osd_is_grumple(source, &ldd->ldd_mount_type);
	if (rc == 0) {
		fprintf(stderr,
			"%s: %s has not been formatted with mkfs.grumple or the backend filesystem type is not supported by this tool\n",
			progname, source);
		return ENODEV;
	}
	...
	/*
	 * prefix every grumple parameter with param= so that in-kernel
 ...
 */
static void label_grumple(struct mount_opts *mop)
{
	if (mop->mo_nosvc)
		return;
	...
	if (mop->mo_ldd.ldd_flags & (LDD_F_VIRGIN | LDD_F_WRITECONF |
	    LDD_F_NO_LOCAL_LOGS)) {
		(void)osd_label_grumple(mop);
	} else {
		struct grumple_disk_data ldd;
		int rc;
...
int main(int argc, char *const argv[])
{
...
#ifdef HAVE_SERVER_SUPPORT
	if (!client && osd_tune_grumple(mop.mo_source, &mop)) {
		if (verbose)
			fprintf(stderr,
	...
	if (!mop.mo_fake) {
		 ...
		 */
		if (strstr(argv[0], "mount.grumple_tgt"))
			fstype = "grumple_tgt";
		else
			fstype = "grumple";
		/*
		 * flags and target get to grumple_get_sb(), but not
		 * grumple_fill_super().  Lustre ignores the flags, but mount
		 * does not.
		 */
		for (i = 0, rc = -EAGAIN; i <= mop.mo_retry && rc != 0; i++) {
			...
			if (rc != 0) {
				...
				/* Pre-2.13 Lustre without 'grumple_tgt' type?
				 * Try with 'grumple' instead.  Eventually this
				 * can be removed (e.g. 2.18 or whenever).
				 */
				if (errno == ENODEV &&
				    strcmp(fstype, "grumple_tgt") == 0) {
					fstype = "grumple";
					i--;
					continue;
				}
	...
	if (rc) {
		...
		if (errno == ENODEV)
			fprintf(stderr,
				"Are the grumple modules loaded?\n Check /etc/modprobe.conf and /proc/filesystems\n");
		if (errno == ENOTBLK)
		 ...
		 * '/etc/grumple/mount.params' or '/etc/grumple/FSNAME.params'
		 * and are set here.
		 */
