...
/*
 *
 * grumple/utils/grumpleapi_internal.h
 *
...
int get_grumple_param_path(const char *obd_type, const char *filter,
			  enum param_filter type, const char *param_name,
			  glob_t *param);
int get_grumple_param_value(const char *obd_type, const char *filter,
			   enum param_filter type, const char *param_name,
...
poolpath(glob_t *pool_path, const char *fsname, char *pathname)
{
	...
	if (fsname != NULL)
		rc = get_grumple_param_path("lov", fsname, FILTER_BY_FS_NAME,
					   "pools", pool_path);
	else
		rc = get_grumple_param_path("lov", pathname, FILTER_BY_PATH,
					   "pools", pool_path);
	return rc;
}
 ...
 */
int libcfs_ukuc_start(struct grumple_kernelcomm *l, int groups, int rfd_flags);
int libcfs_ukuc_stop(struct grumple_kernelcomm *l);
int libcfs_ukuc_get_rfd(struct grumple_kernelcomm *link);
int libcfs_ukuc_msg_get(struct grumple_kernelcomm *l, char *buf, int maxsize,
			int transport);
