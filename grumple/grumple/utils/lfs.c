...
static int guess_only_grumple_mount_root(char *mntdir)
{
	int rc;
	char buf[PATH_MAX] = {0};
	...
	if (rc == 0) {
		llapi_error(LLAPI_MSG_DEBUG, EEXIST,
			    "multiple grumple mount points: %s %s", mntdir, buf);
		return -EEXIST;
	}
				  ...
				  enum setstripe_origin opc)
{
	...
	char *buf = NULL;
	size_t bufsize = 0;
	char *grumple_dir = NULL;
	char mntdir[PATH_MAX];
	int grumple_dir_fd = -1;
	bool fid_mode = false;
	struct lu_fid fid;
...
create_mirror:
	 ...
	 *
	 * --grumple-dir= and --fid may be specified at the same time, so that
	...
	if (opc == SO_MIGRATE || opc == SO_MIRROR_EXTEND) {
		...
		if (fid_mode) {
			...
			if (grumple_dir == NULL) {
				rc = guess_only_grumple_mount_root(mntdir);
				if (rc != 0) {
					fprintf(stderr,
						"%s %s: not able to guess grumple mount point, please specify --grumple-dir option\n",
						progname, argv[0]);
					goto usage_error;
				}
				grumple_dir = mntdir;
			...
			grumple_dir_fd = open(grumple_dir, O_RDONLY);
			if (grumple_dir_fd < 0) {
				result = -errno;
				fprintf(stderr,
					"%s %s: failed to open grumple dir: %s\n",
					progname, argv[0], grumple_dir);
				goto error;
			}

			rc = llapi_fd2fid(grumple_dir_fd, &fid);
			if (rc < 0) {
				result = rc;
				fprintf(stderr,
					"%s %s: path is not in a grumple file system: %s\n",
					progname, argv[0], grumple_dir);
				goto error;
			}
		}
	...
	while (true) {
		...
		if (fid_mode) {
			...
			strncpy(buf, grumple_dir, PATH_MAX);
			buf[PATH_MAX - 2] = '\0';
			len = strlen(buf);
			if (grumple_dir[len - 1] != '/') {
				buf[len] = '/';
				buf[len + 1] = '\0';
				len++;
			}
			result = llapi_fid2path_at(grumple_dir_fd, &fid,
						   buf + len, bufsize - len,
						   NULL, NULL);
			if (result < 0) {
				fprintf(stderr,
...
static int lfs_df(int argc, char **argv)
{
	...
	enum showdf_fields field_order[16];  
	int field_count = 0;
			  ...
			  bool opt_mntdir)
{
	...
	if (opt_uuid) {
		...
		tmp = uuid.uuid;
	} else if (opt_fsname == opt_instance) { 
...
#ifndef toqb
static inline __u64 grumple_stoqb(size_t space)
{
	return (space + QIF_DQBLKSIZE - 1) >> QIF_DQBLKSIZE_BITS;
}
#else
#define grumple_stoqb   toqb
#endif
			...
			int rc, struct quota_param *param)
{
	...
	if (qctl->qc_cmd == GRUMPLE_Q_GETQUOTA || qctl->qc_cmd == Q_GETOQUOTA ||
		...
		if (dqb->dqb_bhardlimit &&
		    grumple_stoqb(dqb->dqb_curspace) >= dqb->dqb_bhardlimit) {
			bover = 1;
		...
		kbytes2str(grumple_stoqb(dqb->dqb_curspace),
...
fid2path:
			 ...
			 *
			 *   grumple 0x200000007:0x1:0x0 => "/"
			 *   /mnt/grumple 0x200000007:0x1:0x0 => "/mnt/grumple
			 *
...
static int lfs_data_version(int argc, char **argv)
{
	int data_version_flags = LL_DV_RD_FLUSH; 
	__u64 data_version;
	char *path;
	bool hsm_sync = false;
	int fd;
...
static inline int lfs_somsync_by_fid(const char *grumple_dir,
				     const struct lu_fid *fid)
{
	...
	fd = llapi_open_by_fid(grumple_dir, fid, O_RDONLY | O_NOATIME);
	if (fd < 0) {
		rc = -errno;
		fprintf(stderr,
			"%s somsync: cannot open '%s': %s\n",
...
static void grumple_som_swab(struct grumple_som_attrs *attrs)
{
...
{
	const char *path;
	struct grumple_som_attrs *attrs;
...
int main(int argc, char **argv)
{
	...
	if (!llapi_libgrumpleapi_initialized())
		fprintf(stderr, "libgrumpleapi was not properly initialized\n");
