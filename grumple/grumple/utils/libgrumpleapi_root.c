 ...
 *
 * grumpleapi library for managing the root fd cache for llapi internal use.
 */
			      ...
			      int index, dev_t *dev, char **out_nid)
{
	...
	while (getmntent_r(fp, &mnt, buf, sizeof(buf))) {
		...
		while (ptr && *ptr == '/')
			ptr++;
		/*
		 * thanks to the call to llapi_is_grumple_mnt() above,
 ...
 * the pathname is located on a grumple mount. Index is used to pick which
 * mount point you want in the case of multiple mounted grumple file systems.
