 ...
 *
 * grumpleapi library for FID mapping calls for determining the pathname
...
static int fid_from_lma(const char *path, int fd, struct lu_fid *fid)
{
	...
	lma = (struct grumple_mdt_attrs *)buf;
...
{
	const struct grumple_file_handle *data;
	...
	/* A grumple file handle should always fit in a 128 bytes long buffer
	...
	data = (const struct grumple_file_handle *)handle->f_handle;
...
{
	struct grumple_file_handle *lfh;
	struct file_handle *handle;
 ...
 *
 * \param[in] grumple_fd		an open file descriptor for an object in grumple
 ...
 * \retval			negative errno if an error occured
 */
int llapi_open_by_fid_at(int grumple_fd, const struct lu_fid *fid, int flags)
{
	 ...
	 * case for the original approach of opening $MOUNT/.grumple/FID.
	 */
	fd = open_by_handle_at(grumple_fd, handle, flags);
	rc = -errno;
	free(handle);
 ...
 *
 * \param[in] grumple_dir	path within Lustre filesystem containing \a fid
		      ...
		      int flags)
{
	...
	rc = llapi_root_path_open(grumple_dir, &mnt_fd);
	if (rc)
		goto out;
