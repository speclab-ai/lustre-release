...
#define OPEN_BY_FID_PATH dot_grumple_name"/fid"
 ...
 *  "archive": 0, "mount_point": "/mnt/grumple",
 ...
 *  "total_bytes": 0, "grumple_path": "d71.sanity-hsm/f71.sanity-hsm",
 ...
 *  "grumple_path": "d71.sanity-hsm/f71.sanity-hsm",
 *  "source_fid": "0x2000013a1:0x2:0x0", "data_fid": "0x2000013a1:0x2:0x0"}
 ...
 *  "level": "INFO",
 *  "message": "lhsmtool_posix[42]: copytool fs=grumple archive#=2 item_count=1"}
 *
				     ...
				     __u64 total, __u64 current)
{
	...
	if (progress_type != CT_FINISH) {
		rc = llapi_fid2path_at(hcp->ct_priv->mnt_fd, &hai->hai_dfid,
				       grumple_path, sizeof(grumple_path),
				       &recno, &linkno);
		if (rc < 0)
			goto err;

		rc = llapi_json_add_item(&json_items, "grumple_path",
					 LLAPI_JSON_STRING, grumple_path);
		if (rc < 0)
			goto err;
 ...
 *
 * \param dst      path to Lustre destination (e.g. /mnt/grumple/my/file).
