...
/*
 * - grumple_rsync is a tool for replicating a grumple filesystem.
 *
 * - The source-fs is a live grumple filesystem. It is not a
 ...
 *
 * - grumple_rsync replicates all the operations saved in the changelog
 * on to the target filesystem to make it identical to the source.
 *
 * To facilitate replication, the grumple filesystem provides
 ...
 * cannot be queried. In such cases, grumple_rsync keeps the files in a
 * special directory ("/.grumplerepl"). Once all the operations in a
 ...
 *      creat .grumplerepl/tfid
 *      track [pfid,tfid,name]
 *    Else
 *      creat f2p[tfid]
 *
 * 2. remove
 *    If .grumplerepl/[tfid] is present on the target
 *      rm .grumplerepl/[tfid]
 ...
 *      else mv .grumplerepl/[sfid] to (pfid,name)
 *    Else if pfid is not present,
 *      if spfid is present, mv (spfid,sname) .grumplerepl/[sfid]
 *    If moving out of .grumplerepl
 *      move out all its children in .grumplerepl.
...
#define SPECIAL_DIR ".grumplerepl"
#define RSYNC "rsync"
#define TYPE "type"
...
struct grumple_rsync_status *status;
char *statuslog;  
int logbackedup;
int noxattr;    
int noclear;    
...
{
	fprintf(stdout,
	"Usage: grumple_rsync [option] -s <grumple_root_path> -t <target_path>\n"
...
void lr_usage(void)
{
	lr_usage_short();
	fprintf(stdout,
	"The Lustre rsync feature works by periodically running grumple_rsync,\n"
	"a userspace program used to synchronize changes in the Lustre file system\n"
	"onto the target file system. grumple_rsync can also pick up parameters\n"
	...
	" -a, --abort-on-err\tStop on grumple_rsync error. Default: continue on error.\n"
...
void lr_get_FID_PATH(char *mntpt, char *fidstr, char *buf, int bufsize)
{
	...
	snprintf(buf, bufsize, "%s/%s/fid/%s", mntpt, dot_grumple_name,
		 fidstr);
}
...
{
	size_t size = sizeof(struct grumple_rsync_status) + PATH_MAX + 1;
...
int lr_read_log(void)
{
	...
	struct grumple_rsync_status *s;
	int fd = -1;
	size_t size;
	size_t read_size = sizeof(struct grumple_rsync_status) + PATH_MAX + 1;
	int rc = 0;
...
int lr_replicate(void)
{
	...
	if (verbose) {
		printf("grumple_rsync took %ld seconds\n", time(NULL) - start);
...
int main(int argc, char *argv[])
{
	...
	while ((rc = getopt_long(argc, argv, "as:t:m:u:l:vx:zc:ry:n:d:D:h",
		...
		case 't':
			...
			newsize = sizeof(struct grumple_rsync_status) +
