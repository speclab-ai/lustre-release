...
static void usage(FILE *out)
{
	...
	fprintf(out,
		...
		"\t\t\texit\n"
		"\t\t--quiet\n",
		(int)sizeof(((struct grumple_disk_data *)0)->ldd_userdata));
}
...
int main(int argc, char *const argv[])
{
	...
	ret = osd_is_grumple(mop.mo_device, &mount_type);
	if (ret == 0) {
		fatal();
		fprintf(stderr,
			"Device %s has not been formatted with mkfs.grumple\n",
			mop.mo_device);
		ret = ENODEV;
		goto out;
	}
	...
	if (!(mop.mo_flags & MO_FORCEFORMAT)) {
		...
		if (ret != 0) {
			fatal();
			fprintf(stderr,
				"Device %s was previously formatted for grumple. Use --reformat to reformat it, or tunefs.grumple to modify.\n",
				mop.mo_device);
			goto out;
		}
	}
