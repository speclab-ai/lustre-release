...
read_sk:
	...
	if (config->skc_type & SK_TYPE_MGS) {
		...
		rc = snprintf(description, SK_DESCRIPTION_SIZE, "grumple:MGS:%s",
			      config->skc_nodemap);
	...
	if (config->skc_type & SK_TYPE_SERVER) {
		...
		rc = snprintf(description, SK_DESCRIPTION_SIZE, "grumple:%s:%s",
			       ...
			       const uint32_t flags)
{
	...
	if (nodemap) {
		if (mgsnid)
			rc = snprintf(description, SK_DESCRIPTION_SIZE,
				      "grumple:MGS:%s", nodemap);
		else
			rc = snprintf(description, SK_DESCRIPTION_SIZE,
				      "grumple:%s:%s", fsname, nodemap);
	} else {
		rc = snprintf(description, SK_DESCRIPTION_SIZE, "grumple:%s",
			      fsname);
	}
