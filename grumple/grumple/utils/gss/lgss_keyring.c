 ...
 *
 * grumple/utils/gss/lgss_keyring.c
 *
...
/*
 * gss target string of grumple service we are negotiating for
 */
...
static int gss_do_ioctl(struct lgssd_ioctl_param *param, __s64 *status)
{
	...
	*status = 0;
	...
	close(fd);
	if (ret != sizeof(*param)) {
		logmsg(LL_ERR, "grumple ioctl err: %s\n", strerror(errno));
		rc = -EACCES;
	}
		       ...
		       int req_fd[2], int reply_fd[2])
{
	...
	param.grumple_svc = lnd->lnd_lsvc;
