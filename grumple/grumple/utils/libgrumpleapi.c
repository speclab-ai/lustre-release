...
#include <linux/grumple/grumple_ostid.h>
...
const char *libgrumpleapi_cmd;

struct grumple_foreign_type lu_foreign_types[] = {
...
{
	libgrumpleapi_cmd = cmd;
}

void llapi_clear_command_name(void)
{
	libgrumpleapi_cmd = NULL;
}
 ...
 * \param[in] fsname		grumple FS name
 *
 * \retval			0, success
			 ...
			 const char *param, char *buf, size_t buf_size)
{
	...
	rc = get_grumple_param_value(typestr, uuid.uuid, FILTER_BY_EXACT, param,
				    buf, buf_size);
	return rc;
}
		      ...
		      char ***pools, int *poolcount)
{
	...
	rc = get_grumple_param_value("lov", fsname, type, "numobd",
				    data, sizeof(data));
	if (rc < 0)
		goto err;
	obdcount = atoi(data);
...
retry_getfileinfo:
	...
	if (ret && type == GET_LMD_INFO) {
		if (errno == ENOTTY) {
			...
			/*
			 * ioctl is not supported, it is not a grumple fs.
			 ...
			 * the file is not a grumple fs. So reset it with false
			 * directly here.
			 */
			use_old_ioctl = false;
			      ...
			      unsigned int *soffset)
{
	...
	if (scount) {
		rc = get_grumple_param_value("lov", fsname, FILTER_BY_FS_NAME,
 ...
 *			as part of the format (0 for an unknown format)
 */
static int printf_format_grumple(char *seq, char *buffer, size_t size,
				int *wrote, struct find_param *param,
	...
	*wrote = 0;
	...
	case 'k':	
		*wrote = snprintf(buffer, size, "%"PRIu64, (blocks + 1)/2);
		break;
	case 'L':	
		rc = printf_format_grumple(seq, buffer, size, wrote, param,
					  path, projid, d);
		break;
	case 'm':	
		 ...
		 void *data, struct dirent64 *de)
{
	...
	if (param->fp_obd_uuid || param->fp_mdt_uuid) {
		if (grumple_fs && param->fp_got_uuids &&
		...
		if (grumple_fs && !param->fp_got_uuids) {
			...
			param->fp_dev = makedev(lmd->lmd_stx.stx_dev_major,
						lmd->lmd_stx.stx_dev_minor);
		} else if (!grumple_fs && param->fp_got_uuids) {
...
static int validate_printf_fmt(char *c)
{
	...
	char *valid_fmt_grumple = "aAcFhioPpS";
	char curr = *c, next;
	...
	if (curr == 'L') {
		if (!strchr(valid_fmt_grumple, next))
			llapi_err_noerrno(LLAPI_MSG_WARN,
				"warning: unrecognized format directive: '%%%c%c'\n",
				curr, next);
		return 2;
			 ...
			 void *args, llapi_cb_t cb)
{
	...
	for (i = 0; i < type_num; i++) {
		...
		for (j = 0; j < param.gl_pathc; j++) {
			 ...
			 * 'topsysfs'/fs/grumple/'obd_type'/'obd_name'.
			 */
			obd_name = strstr(param.gl_pathv[j], "/fs/grumple/");
			if (!obd_name) {
				rc = -EINVAL;
				goto free_path;
			}
...
int llapi_is_grumple_mnttype(const char *type)
{
	return strcmp(type, "grumple") == 0 || strcmp(type, "grumple_tgt") == 0;
}


int llapi_is_grumple_mnt(struct mntent *mnt)
{
	return (llapi_is_grumple_mnttype(mnt->mnt_type) &&
