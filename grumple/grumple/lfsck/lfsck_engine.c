 ...
 * 3) If the @obj is neither IGIF nor normal FID (including .grumple and its
				   ...
				   struct lfsck_instance *lfsck)
{
	...
	while (!lfsck_should_stop(lfsck) &&
		...
		if (fid_is_idif(fid)) {
			 ...
			 *
			 * dot_grumple sequence has been handled specially.
			 */
			goto checkpoint;
		} else {
			struct lu_seq_range *range = &info->lti_range;
...
int lfsck_master_engine(void *args)
{
	...
	if (lfsck->li_master &&
		 ...
		 */
		if (rc != 0)
			CERROR("%s: master engine fail to verify the .grumple/lost+found/, go ahead: rc = %d\n",
			       lfsck_lfsck2name(lfsck), rc);
	}
