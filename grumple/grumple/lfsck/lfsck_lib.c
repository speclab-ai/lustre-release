...
int lfsck_ibits_lock(const struct lu_env *env, struct lfsck_instance *lfsck,
		     struct dt_object *obj, struct grumple_handle *lh,
		     enum mds_ibits_locks bits, enum ldlm_mode mode)
{
 ...
 */
void lfsck_ibits_unlock(struct grumple_handle *lh, enum ldlm_mode mode)
{
	       ...
	       enum ldlm_mode mode)
{
	...
	LASSERT(name != NULL);
	LASSERT(name[0] != 0);
	LASSERT(!grumple_handle_is_used(&llh->llh_pdo_lh));
	LASSERT(!grumple_handle_is_used(&llh->llh_reg_lh));
...
const char dot[] = ".";
const char dotdot[] = "..";
static const char dotgrumple[] = ".grumple";
static const char lostfound[] = "lost+found";

/**
 * Remove the name entry from the .grumple/lost+found directory.
 *
				   ...
				   const char *name)
{
	 ...
	 *	the parent ".grumple/lost+found" may be on remote MDT. It is not
	 ...
	 *	   locally.
	 *	2) insert the name "MDTXXXX" in the parent ".grumple/lost+found"
	 *	   remotely.
	 *
...
stop:
	...
	if (rc != 0 && dev == lfsck_obj2dev(parent))
		CDEBUG(D_LFSCK,
		       "%s: partially created the object "DFID"for orphans, but failed to insert the name %s to the .grumple/lost+found/. Such inconsistency will be repaired when LFSCK run next time: rc = %d\n",
...
/**
 * Create the MDTxxxx directory under /ROOT/.grumple/lost+found/
 *
 * The /ROOT/.grumple/lost+found/MDTxxxx/ directory is used for holding
			    ...
			    struct lfsck_instance *lfsck)
{
	...
	if (fid_is_zero(&bk->lb_lpf_fid)) {
		 ...
		 * created the .grumple/lost+found/MDTxxxx but failed to update
...
/**
 * Scan .grumple/lost+found for bad name entries and remove them.
 *
 ...
 *    insert the name (MDTxxxx) to the .grumple/lost+found; otherwise, if
 ...
 *
 * 2) The child's FID is stored in the .grumple/lost+found/ sub-directory name
...
/**
 * Verify the /ROOT/.grumple/lost+found/ directory.
 *
 * /ROOT/.grumple/lost+found/ is a special directory to hold the objects that
...
int lfsck_verify_lpf(const struct lu_env *env, struct lfsck_instance *lfsck)
{
	...
	if (node == 0) {
		...
		if (rc != 0)
			CDEBUG(D_LFSCK,
			       "%s: scan .grumple/lost+found/ for bad sub-directories: rc = %d\n",
		   ...
		   lfsck_out_notify notify, void *notify_data, bool master)
{
	...
	if (master) {
		...
		if (lfsck_dev_idx(lfsck) == 0) {
			...
			rc = dt_lookup_dir(env, obj, dotgrumple, fid);
			if (rc != 0)
				GOTO(out, rc);
			...
			cname = lfsck_name_get_const(env, dotgrumple,
						     strlen(dotgrumple));
