			     ...
			     struct dt_object *obj, const struct lu_fid *fid)
{
	...
	rc = dt_xattr_get(env, obj, lfsck_buf_get(env, loa, sizeof(*loa)),
			  XATTR_NAME_LMA);
	if (rc >= (int)sizeof(struct grumple_mdt_attrs)) {
		grumple_lma_swab(&loa->loa_lma);
				...
				struct lfsck_component *com)
{
	...
	while (!list_empty(&llsd->llsd_master_list)) {
		...
		exp = grumple_find_lwp_by_index(lfsck->li_obd->obd_name,
					       llst->llst_index);
		if (exp == NULL) {
			lfsck_layout_llst_del(llsd, llst);
			kref_put(&llst->llst_ref, lfsck_layout_llst_put);
					  ...
					  struct lu_fid *pfid, __u32 comp_id)
{
	...
	exp = grumple_find_lwp_by_index(obd->obd_name, range->lsr_index);
