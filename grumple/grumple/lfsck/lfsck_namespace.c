...
enum lfsck_nameentry_check {
	LFSCK_NAMEENTRY_DEAD		= 1, 
	LFSCK_NAMEENTRY_REMOVED		= 2, 
	LFSCK_NAMEENTRY_RECREATED	= 3, 
};
...
/**
 * Insert orphan into .grumple/lost+found/MDTxxxx/ locally.
 *
 * Add the specified orphan MDT-object to the .grumple/lost+found/MDTxxxx/
 ...
 * under the .grumple/lost+found/MDTxxxx/ for the orphan MDT-object.
 *
 * \param[in] env	pointer to the thread context
 * \param[in] com	pointer to the lfsck component
 ...
 * an orphan and insert it into .grumple/lost+found/MDTxxxx/ directory
 ...
 * it in the directory .grumple/lost+found/MDTxxxx/ with the name:
 ...
 * then remove it and insert the @child in the .grumple/lost+found/MDTxxxx/
 * as an orphan.
 *
 * \param[in] env	pointer to the thread context
 * \param[in] com	pointer to the lfsck component
			   ...
			   bool *retry, bool *unknown)
{
	...
	if (rc != 0) {
		if (!grumple_handle_is_used(lh) && retry != NULL)
			*retry = true;
		else
...
again:
	while (ldata->ld_lee != NULL) {
		...
		/* If current dotdot is the .grumple/lost+found/MDTxxxx/,
...
next:
	 ...
	 * still under the .grumple/lost+found/MDTxxxx/ or not.
	 */
	if (lpf) {
		lpf = false;
		goto again;
 ...
 *    in the .grumple/lost+found/MDTxxxx/ with the name:
 ...
 * .grumple/lost+found/MDTxxxx/; if the remote parent is lost, then recreate
...
out:
	...
	if (count == 0) {
		...
		if (!(ns->ln_flags & LF_INCOMPLETE) &&
			 ...
			 * the global .grumple/lost+found/MDTxxxx directory.
			 */
			rc = lfsck_namespace_insert_orphan(env, com, child,
							   "", "O", &count);
			if (rc < 0)
 ...
 * /ROOT/.grumple/lost+found/MDTxxxx/
 *
 * \param[in] env	pointer to the thread context
 * \param[in] com	pointer to the lfsck component
 */
